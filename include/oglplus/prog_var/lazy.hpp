...
class LazyImpl<tag::ProgVar, ProgVar_> : public ProgVar_ {
private:
    String _identifier;
...
public:
    ...
    ProgVar_ operator[](std::size_t offset) {
        Init();
        return ProgVar_(
          ProgramName(this->_program), GLuint(this->_location + offset));
    }
    ...
    void Set(T0&& v0, T1&& v1) {
        Init();
        ProgVar_::Set(std::forward<T0>(v0), std::forward<T1>(v1));
    }
    ...
    void Set(T0&& v0, T1&& v1, T2&& v2) {
        Init();
        ProgVar_::Set(
          std::forward<T0>(v0), std::forward<T1>(v1), std::forward<T1>(v2));
    }
