...
struct GLSLtoCppTypeMatcher_Vec {
	static std::size_t _type_idx(bool*) {
		return 0;
	}
	static std::size_t _type_idx(GLint*) {
		return 1;
	}
	static std::size_t _type_idx(GLuint*) {
		return 2;
	}
	static std::size_t _type_idx(GLfloat*) {
		return 3;
	}
#if defined(GL_DOUBLE)
	static std::size_t _type_idx(GLdouble*) {
		return 4;
	}
#endif

	static bool _does_match(
	  GLenum sl_type, std::size_t type_idx, std::size_t dim);
};
...
struct GLSLtoCppTypeMatcher<oglplus::Vector<T, N>>
  : public GLSLtoCppTypeMatcher_Vec {
	...
	static T* _type_sel(void) {
		return nullptr;
	}

	static bool _matches(GLenum sl_type) {
		return GLSLtoCppTypeMatcher_Vec::_does_match(
		  sl_type, GLSLtoCppTypeMatcher_Vec::_type_idx(_type_sel()), N);
	}
};
...
struct GLSLtoCppTypeMatcher_Mat {
	...
	static bool _does_match(
	  GLenum sl_type, std::size_t type_idx, std::size_t rows, std::size_t cols);
};
...
struct GLSLtoCppTypeMatcher<oglplus::Matrix<T, Rows, Cols>>
  : public GLSLtoCppTypeMatcher_Mat {
	...
	static bool _matches(GLenum sl_type) {
		return GLSLtoCppTypeMatcher_Mat::_does_match(sl_type,
...
private:
	...
	static void _do_check(bool (*)(GLenum),
	  GLenum var_type,
	  ProgramName program,
	  GLint location,
	  StrCRef identifier);
...
public:
	...
	void CheckType(
	  ProgramName program, GLint location, StrCRef identifier) const {
		_do_check(_type_matches,
		  ProgVarTypeOps<tag::Uniform>::GetType(program, location, identifier),
		  program,
		  location,
		  identifier);
	}
...
public:
	ProgVarTypecheck(void*) noexcept {
	}
