...
private:
    ...
    static void _do_set_v(
      _set_cont, GLuint program, LI base_location, LI location, const V* v) {
        std::integral_constant<std::size_t, 4> nparam;
        Callers::_call_set_v(program, location, Setters::_fns_v(nparam, v), v);
        OGLPLUS_CHECK_CTXT(
          ProgVarError, Program(ProgramName(program)).Index(base_location));
        _do_set_v<N - 4, LI, V>(
          _set_mode<N - 4>(), program, base_location, location + 1, v + 4);
    }

    template <std::size_t N, typename LI, typename V>
    static void _do_set_v(
      _set_done, GLuint program, LI base_location, LI location, const V* v) {
    ...
    static void _do_set_n(
      ...
      const V* v) {
        ...
        Callers::_call_set_vn(
          program, location, n, Setters::_fns_v(nparam, v), v);
        OGLPLUS_CHECK_CTXT(
          ProgVarError, Program(ProgramName(program)).Index(base_location));
    }
    ...
    static void _do_set_t(
      ...
      V... v) {
        std::integral_constant<std::size_t, 4> nparam;
        Callers::_call_set_t(
          program, location, Setters::_fns_t(nparam, &v0), v0, v1, v2, v3);
        OGLPLUS_CHECK_CTXT(
          ProgVarError, Program(ProgramName(program)).Index(base_location));
        _do_set_t(
          _set_mode<sizeof...(V)>(),
    ...
    static void _do_set_t(
      _set_done, GLuint program, LI base_location, LI location, V... v) {
        std::integral_constant<std::size_t, sizeof...(V)> nparam;
        Callers::_call_set_t(
          program, location, Setters::_fns_t(nparam, &v...), v...);
        OGLPLUS_CHECK_CTXT(
          ProgVarError, Program(ProgramName(program)).Index(base_location));
    }
...
protected:
    ...
    static void _do_set(GLuint program, LI location, V... v) {
        ...
        _do_set_t(_set_mode<sizeof...(V)>(), program, location, location, v...);
        OGLPLUS_CHECK_CTXT(
          ProgVarError, Program(ProgramName(program)).Index(location));
    }
    ...
    static void _do_set(GLuint program, LI location, const V* v) {
        ...
        _do_set_v<Cols, LI, V>(
          _set_mode<Cols>(), program, location, location, v);
    }
    ...
    static void _do_set_many(GLuint prog, LI location, GLsizei n, const V* v) {
        static_assert(
          ...
          "The number of elements must be between 1 and M");
        _do_set_n<Cols, LI, V>(
          _set_mode<Cols>(), prog, location, location, n, v);
    }
};
...
protected:
    ...
    static void _do_set_mat(
      GLuint program, LI location, GLsizei count, Boolean transpose, V* v) {
        static_assert(
          (Cols > 0) && (Cols <= 4),
    ...
    static void _do_set_mat_p(
      GLuint program, LI location, bool transpose, V v, P... p) {
        static_assert(
          (Cols > 0) && (Cols <= 4),
          "The number of columns must be between 1 and 4");
        static_assert(
          (sizeof...(P) + 1) % Cols == 0, "Not enough values for the last row");
        V values[] = {v, V(p)...};
        _do_set_mat<Cols, (sizeof...(P) + 1) / Cols, LI, V>(
          program, location, 1, transpose, values);
    }
};
