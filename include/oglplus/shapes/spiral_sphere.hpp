...
private:
    ...
    template <typename T>
    void _make_tangents(std::vector<T>& dest, unsigned& k, double sign) const;

    template <typename T>
    void _make_bitangents(std::vector<T>& dest, unsigned& k, double sign) const;
...
public:
    ...
    std::vector<GLfloat> _bitangents() const;
    ...
    void BoundingSphere(oglplus::Sphere<T>& bounding_sphere) const {
        bounding_sphere =
          oglplus::Sphere<T>(T(0), T(0), T(0), T(_radius + _thickness));
    }
