...
public:
    ...
    Grid(
      const Vec3f p, const Vec3f u, const Vec3f v, unsigned udiv, unsigned vdiv)
      : _point(p)
      , _u(u)
      , _v(v)
      , _udiv(udiv)
    ...
    inline Vec3f Tangential() const {
        return Normalized(_u);
    }

    inline Vec3f Bitangential() const {
        return Normalized(_v);
    }

    Vec4f PlaneEquation() const {
    ...
    void BoundingSphere(oglplus::Sphere<T>& bounding_sphere) const {
        bounding_sphere = oglplus::Sphere<T>(
          T(_point.x()), T(_point.y()), T(_point.z()), T(Length(_u + _v)));
    }
