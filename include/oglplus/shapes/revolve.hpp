...
/// Class providing attributes and instructions for rendering of a revolved
/// shape
...
private:
    ...
    static Vector<Type, 3> _mix(
      const Vector<Type, 3>& a, const Vector<Type, 3>& b, Type factor) {
        if(factor < Type(0))
            factor = Type(0);
        if(factor > Type(1))
            factor = Type(1);
    ...
    Vector<Type, 3> _get_position(std::size_t ring, std::size_t section) const {
        return _mix(
          _positions_0[ring], _positions_1[ring], _section_factors[section]);
    }

    Vector<Type, 3> _get_normal(std::size_t ring, std::size_t section) const {
        return _mix(
          _normals_0[ring], _normals_1[ring], _section_factors[section]);
    }

    Vector<Type, 3> _get_tex_coord(
      std::size_t ring, std::size_t section) const {
        return _mix(
          _tex_coords_0[ring], _tex_coords_1[ring], _section_factors[section]);
    }
...
public:
     ...
     */
    using VertexAttribs = VertexAttribsInfo<RevolveY>;
#else
    ...
    void BoundingSphere(oglplus::Sphere<T>& bounding_sphere) const {
        // TODO
        bounding_sphere = oglplus::Sphere<T>(T(0), T(0), T(0), T(_radius));
    }
