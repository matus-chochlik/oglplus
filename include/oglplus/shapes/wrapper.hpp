...
/// Wraps instructions and VAO+VBOs used to render a shape built by a
/// ShapeBuilder
...
public:
    ...
    ShapeWrapperBase(
      ...
      , _names(std::size_t(std::distance(names_begin, names_end))) {
        this->_init(builder, builder.Indices(selector), names_begin, names_end);
    }
...
/// Wraps instructions and VBOs and VAO used to render a shape built by a
/// ShapeBuilder
...
public:
    ...
    template <class ShapeBuilder>
    ShapeWrapperTpl(
      const GLchar* name, const ShapeBuilder& builder, const ProgramOps& prog)
      : ShapeWrapperBase(&name, (&name) + 1, builder, _sel()) {
        UseInProgram(prog);
    }
};
