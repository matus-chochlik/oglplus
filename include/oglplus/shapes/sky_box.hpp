...
public:
    ...
    GLuint Positions(std::vector<T>& dest) const {
        const T _positions[8 * 3] = {T(-1), T(-1), T(-1), T(+1), T(-1), T(-1),
    ...
    IndexArray Indices(Default = Default()) const {
        ...
        const GLushort _indices[6 * 5] = {1, 3, 5, 7, 9, 4, 6, 0, 2, 9,
                                          2, 6, 3, 7, 9, 4, 0, 5, 1, 9,
                                          5, 7, 4, 6, 9, 0, 2, 1, 3, 9};
        return IndexArray(_indices, _indices + 6 * 5);
#else
        const GLushort _indices[6 * 5 + 5] = {
          1, 3, 5, 7, 7, 4, 4, 6, 0, 2, 2, 2, 2, 6, 3, 7, 7, 4,
          4, 0, 5, 1, 1, 5, 5, 7, 4, 6, 6, 0, 0, 2, 1, 3, 3};
