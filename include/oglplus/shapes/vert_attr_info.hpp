...
private:
    template <typename T>
    static typename _getter_proc<T>::type _find_getter(
      TypeTag<T>,
      StrCRef,
      std::integral_constant<std::size_t, N>,
      std::integral_constant<std::size_t, N>) {
...
protected:
    template <typename T>
    static typename _getter_proc<T>::type _find_getter(
      TypeTag<T> selector, StrCRef name) {
        return _find_getter(
          selector,
          name,
          std::integral_constant<std::size_t, 0>(),
...
public:
    ...
    template <typename T>
    static typename _base::template _getter_proc<T>::type VertexAttribGetter(
      const std::vector<T>& /*selector*/, StrCRef name) {
