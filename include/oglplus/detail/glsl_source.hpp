...
struct GLSLSourceWrapper {
    virtual ~GLSLSourceWrapper() {
    }

    virtual GLsizei Count() const = 0;

    virtual const GLchar* const* Parts() const = 0;

    virtual const GLint* Lengths() const = 0;
};
...
public:
    ...
    const GLchar* const* Parts() const override {
        return &_ptr;
    }

    const GLint* Lengths() const override {
        return &_size;
    }
};
