/*
 *  .file oglplus/enums/ext/nv_path_transform_type.ipp
 *
 *  Automatically generated header file. DO NOT modify manually,
 *  edit 'source/enums/ext/nv_path_transform_type.txt' instead.
 *
 *  Copyright 2010-2012 Matus Chochlik. Distributed under the Boost
 *  Software License, Version 1.0. (See accompanying file
 *  LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
 */

#if OGLPLUS_DOCUMENTATION_ONLY

/// NONE
None,
/// TRANSLATE_X_NV
TranslateX,
/// TRANSLATE_Y_NV
TranslateY,
/// TRANSLATE_2D_NV
Translate2D,
/// TRANSLATE_3D_NV
Translate3D,
/// AFFINE_2D_NV
Affine2D,
/// AFFINE_3D_NV
Affine3D,
/// TRANSPOSE_AFFINE_2D_NV
TransposeAffine2D,
/// TRANSPOSE_AFFINE_3D_NV
TransposeAffine3D

#else // !OGLPLUS_DOCUMENTATION_ONLY

#ifdef OGLPLUS_LIST_NEEDS_COMMA
# undef OGLPLUS_LIST_NEEDS_COMMA
#endif

#if defined GL_NONE
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(None)
#  pragma push_macro("None")
#  undef None
   OGLPLUS_ENUM_CLASS_VALUE(None, GL_NONE)
#  pragma pop_macro("None")
# else
   OGLPLUS_ENUM_CLASS_VALUE(None, GL_NONE)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_TRANSLATE_X_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(TranslateX)
#  pragma push_macro("TranslateX")
#  undef TranslateX
   OGLPLUS_ENUM_CLASS_VALUE(TranslateX, GL_TRANSLATE_X_NV)
#  pragma pop_macro("TranslateX")
# else
   OGLPLUS_ENUM_CLASS_VALUE(TranslateX, GL_TRANSLATE_X_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_TRANSLATE_Y_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(TranslateY)
#  pragma push_macro("TranslateY")
#  undef TranslateY
   OGLPLUS_ENUM_CLASS_VALUE(TranslateY, GL_TRANSLATE_Y_NV)
#  pragma pop_macro("TranslateY")
# else
   OGLPLUS_ENUM_CLASS_VALUE(TranslateY, GL_TRANSLATE_Y_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_TRANSLATE_2D_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(Translate2D)
#  pragma push_macro("Translate2D")
#  undef Translate2D
   OGLPLUS_ENUM_CLASS_VALUE(Translate2D, GL_TRANSLATE_2D_NV)
#  pragma pop_macro("Translate2D")
# else
   OGLPLUS_ENUM_CLASS_VALUE(Translate2D, GL_TRANSLATE_2D_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_TRANSLATE_3D_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(Translate3D)
#  pragma push_macro("Translate3D")
#  undef Translate3D
   OGLPLUS_ENUM_CLASS_VALUE(Translate3D, GL_TRANSLATE_3D_NV)
#  pragma pop_macro("Translate3D")
# else
   OGLPLUS_ENUM_CLASS_VALUE(Translate3D, GL_TRANSLATE_3D_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_AFFINE_2D_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(Affine2D)
#  pragma push_macro("Affine2D")
#  undef Affine2D
   OGLPLUS_ENUM_CLASS_VALUE(Affine2D, GL_AFFINE_2D_NV)
#  pragma pop_macro("Affine2D")
# else
   OGLPLUS_ENUM_CLASS_VALUE(Affine2D, GL_AFFINE_2D_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_AFFINE_3D_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(Affine3D)
#  pragma push_macro("Affine3D")
#  undef Affine3D
   OGLPLUS_ENUM_CLASS_VALUE(Affine3D, GL_AFFINE_3D_NV)
#  pragma pop_macro("Affine3D")
# else
   OGLPLUS_ENUM_CLASS_VALUE(Affine3D, GL_AFFINE_3D_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_TRANSPOSE_AFFINE_2D_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(TransposeAffine2D)
#  pragma push_macro("TransposeAffine2D")
#  undef TransposeAffine2D
   OGLPLUS_ENUM_CLASS_VALUE(TransposeAffine2D, GL_TRANSPOSE_AFFINE_2D_NV)
#  pragma pop_macro("TransposeAffine2D")
# else
   OGLPLUS_ENUM_CLASS_VALUE(TransposeAffine2D, GL_TRANSPOSE_AFFINE_2D_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#if defined GL_TRANSPOSE_AFFINE_3D_NV
# if OGLPLUS_LIST_NEEDS_COMMA
    OGLPLUS_ENUM_CLASS_COMMA
# endif
# if OGLPLUS_NO_SCOPED_ENUMS && defined(TransposeAffine3D)
#  pragma push_macro("TransposeAffine3D")
#  undef TransposeAffine3D
   OGLPLUS_ENUM_CLASS_VALUE(TransposeAffine3D, GL_TRANSPOSE_AFFINE_3D_NV)
#  pragma pop_macro("TransposeAffine3D")
# else
   OGLPLUS_ENUM_CLASS_VALUE(TransposeAffine3D, GL_TRANSPOSE_AFFINE_3D_NV)
# endif
# ifndef OGLPLUS_LIST_NEEDS_COMMA
#  define OGLPLUS_LIST_NEEDS_COMMA 1
# endif
#endif
#ifdef OGLPLUS_LIST_NEEDS_COMMA
# undef OGLPLUS_LIST_NEEDS_COMMA
#endif

#endif // !OGLPLUS_DOCUMENTATION_ONLY

