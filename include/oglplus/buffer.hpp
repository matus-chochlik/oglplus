...
protected:
	...
	static Boolean IsA(GLuint name) {
		Boolean result(OGLPLUS_GLFUNC(IsBuffer)(name), std::nothrow);
...
public:
	...
	static void Bind(Target target, BufferName buffer) {
		OGLPLUS_GLFUNC(BindBuffer)(GLenum(target), GetGLName(buffer));
		OGLPLUS_VERIFY(
		  BindBuffer, ObjectError, Object(buffer).BindTarget(target));
	}
	...
	static void BindBase(
	  IndexedTarget target, GLuint index, BufferName buffer) {
		OGLPLUS_GLFUNC(BindBufferBase)
		(GLenum(target), index, GetGLName(buffer));
		OGLPLUS_VERIFY(
		  BindBufferBase, ObjectError, Object(buffer).BindTarget(target));
	}
	 ...
	 *  @throws Error
	 */
	static void BindRange(IndexedTarget target,
	  GLuint index,
	  BufferName buffer,
	  BufferSize offset,
	  BufferSize size) {
		...
		(GLenum(target),
		  ...
		  GLsizeiptr(size.Get()));
		OGLPLUS_VERIFY(
		  BindBufferRange, ObjectError, Object(buffer).BindTarget(target));
	}

#if OGLPLUS_DOCUMENTATION_ONLY || GL_VERSION_4_4 || GL_ARB_multi_bind
	static void BindBase(BufferIndexedTarget target,
	  GLuint first,
	  SizeType count,
	  const GLuint* names) {
		OGLPLUS_GLFUNC(BindBuffersBase)(GLenum(target), first, count, names);
		OGLPLUS_VERIFY(BindBuffersBase, ObjectError, BindTarget(target));
	}
	...
	static void BindBase(BufferIndexedTarget target,
	  ...
	  const Sequence<BufferName>& buffers) {
		BindBase(target, first, GLsizei(buffers.size()), GetGLNames(buffers));
	}

	static void BindRange(BufferIndexedTarget target,
	  GLuint first,
	  GLsizei count,
	  const GLuint* names,
	  ...
	  const GLsizeiptr* sizes) {
		...
		(GLenum(target), first, count, names, offsets, sizes);
		OGLPLUS_VERIFY(BindBuffersRange, ObjectError, BindTarget(target));
	}
...
public:
	...
	void BindBaseTransformFeedback(
	  TransformFeedbackBufferBindingPoint index) const {
  ...
  || GL_ARB_shader_storage_buffer_object
	 ...
	 */
	void BindRange(IndexedTarget target,
	  GLuint index,
	  BufferSize offset,
	...
	void InvalidateSubData(BufferSize offset, BufferSize size) {
		...
		(_obj_name(), GLintptr(offset.Get()), GLsizeiptr(size.Get()));
		OGLPLUS_CHECK(InvalidateBufferSubData, ObjectError, Object(*this));
	}
#endif
};
...
public:
	...
	static Boolean Mapped(Target target) {
		return Boolean(GetIntParam(target, GL_BUFFER_MAPPED), std::nothrow);
	}
	...
	static void Resize(Target target,
	  ...
	  BufferUsage usage = BufferUsage::StaticDraw) {
		...
		(GLenum(target), size.Get(), nullptr, GLenum(usage));
		OGLPLUS_CHECK(
		  BufferData, ObjectError, ObjectBinding(target).EnumParam(usage));
	}
	 ...
	 *  @throws Error
	 */
	static void Data(Target target,
	 ...
	 *  @throws Error
	 */
	static void RawData(Target target,
	  BufferSize size,
	  const GLvoid* data,
	  BufferUsage usage = BufferUsage::StaticDraw) {
	...
	static void SubData(
	  Target target, BufferSize offset, const BufferData& data) {
		OGLPLUS_GLFUNC(BufferSubData)
		(GLenum(target),
		  GLintptr(offset.Get()),
		  GLsizei(data.Size()),
		  data.Data());
		OGLPLUS_CHECK(BufferSubData, ObjectError, ObjectBinding(target));
	}
	...
	template <typename GLtype>
	static void SubData(
	  Target target, BufferSize offset, SizeType count, const GLtype* data) {
	 ...
	 */
	static inline void CopySubData(BufferTarget readtarget,
	  ...
	  BufferSize size) {
		...
		(GLenum(readtarget),
		  ...
		  GLsizeiptr(size.Get()));
		OGLPLUS_CHECK(CopyBufferSubData,
		  ObjectPairError,
		  SubjectBinding(readtarget).ObjectBinding(writetarget));
	}
#endif // copy buffer
	 ...
	 */
	template <typename GLtype>
	static void ClearData(Target target,
	  ...
	  const GLtype* data) {
		...
		(GLenum(target),
		  ...
		  data);
		OGLPLUS_CHECK(ClearBufferData, ObjectError, ObjectBinding(target));
	}
	 ...
	 */
	template <typename GLtype>
	static void ClearSubData(Target target,
	  PixelDataInternalFormat internal_format,
	  BufferSize offset,
	  BufferSize size,
	  PixelDataFormat format,
	  const GLtype* data) {
		...
		(GLenum(target),
		  ...
		  GLenum(GetDataType<GLtype>()),
		  data);
		OGLPLUS_CHECK(ClearBufferSubData,
		  ObjectError,
	 ...
	 */
	static void Storage(
	  Target target, const BufferData& data, Bitfield<BufferStorageBit> flags) {
		OGLPLUS_GLFUNC(BufferStorage)
		(GLenum(target),
		  GLsizeiptr(data.Size()),
		  data.Data(),
		  GLbitfield(flags));
		OGLPLUS_CHECK(BufferStorage, ObjectError, ObjectBinding(target));
	}
	...
	static Boolean ImmutableStorage(Target target) {
		return Boolean(
		  GetIntParam(target, GL_BUFFER_IMMUTABLE_STORAGE), std::nothrow);
	}
	...
	static Bitfield<BufferStorageBit> StorageFlags(Target target) {
		return Bitfield<BufferStorageBit>(
		  GLbitfield(GetIntParam(target, GL_BUFFER_STORAGE_FLAGS)));
	}
#endif
	...
	static void PageCommitment(
	  Target target, BufferSize offset, BufferSize size, Boolean commit) {
		...
		(GLenum(target),
		  ...
		  commit._get());
		OGLPLUS_VERIFY(
		  BufferPageCommitmentARB, ObjectError, ObjectBinding(target));
	}
	...
	static SizeType Size(Target target) {
		return MakeSizeType(GetIntParam(target, GL_BUFFER_SIZE), std::nothrow);
	}
	...
	static void MakeResident(Target target, AccessSpecifier access) {
		...
		OGLPLUS_CHECK(MakeBufferResidentNV, ObjectError, ObjectBinding(target));
	}
	...
	static void MakeNonResident(Target target) {
		...
		OGLPLUS_CHECK(
		  MakeBufferNonResidentNV, ObjectError, ObjectBinding(target));
	}
	...
	static BufferGPUAddress GPUAddress(Target target) {
		...
		(GLenum(target), GL_BUFFER_GPU_ADDRESS_NV, &value);
		OGLPLUS_CHECK(
		  GetBufferParameterui64vNV, ObjectError, ObjectBinding(target));
...
inline BufferOpsAndIdxTgt operator<<(
  const BufferOps& buf, BufferIndexedTarget target) {
...
// Bind
inline BufferTarget operator<<(const BufferOps& buf, BufferTarget target) {
	buf.Bind(target);
	return target;
}
...
inline const BufferOps& operator<<(
  const BufferOpsAndIdxTgt& bat, GLuint index) {
	bat.buf.BindBase(bat.target, index);
	return bat.buf;
}

// Data
inline BufferTarget operator<<(BufferTarget target, const BufferData& data) {
...
inline BufferTarget operator<<(
  BufferTargetAndUsage&& tau, const BufferData& data) {
...
// SubData
inline BufferTarget operator<<(
  BufferTargetAndOffset&& tao, const BufferData& data) {
