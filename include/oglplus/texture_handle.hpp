...
public:
	...
	TextureHandle(TextureName texture, SamplerName sampler)
	  : _handle(OGLPLUS_GLFUNC(GetTextureSamplerHandleARB)(
		  GetGLName(texture), GetGLName(sampler))) {
	...
	Boolean IsResident(void) const {
		Boolean result(
		  OGLPLUS_GLFUNC(IsTextureHandleResidentARB)(_handle), std::nothrow);
...
inline GLuint64 GetGLHandle(TextureHandle th) noexcept {
	return th._handle;
}
...
public:
	...
	Boolean IsResident(void) const {
		Boolean result(
		  OGLPLUS_GLFUNC(IsImageHandleResidentARB)(_handle), std::nothrow);
...
inline GLuint64 GetGLHandle(ImageHandle ih) noexcept {
	return ih._handle;
}
