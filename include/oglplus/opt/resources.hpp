...
inline bool OpenResourceFile(
  ...
  const std::string& name,
  const char* ext) {
    return FindResourceFile(file, category, name, &ext, 1) == 0;
}

class ResourceFile : public std::ifstream {
public:
    std::ifstream& stream() {
        return *this;
    }
    ...
    ResourceFile(
      const std::string& category, const std::string& name, const char* ext);
};
