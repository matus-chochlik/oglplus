...
public:
	...
	void Resize(BufferSize size, BufferUsage usage = BufferUsage::StaticDraw) {
		...
		OGLPLUS_CHECK(
		  NamedBufferDataEXT, ObjectError, Object(*this).EnumParam(usage));
	}
	...
	void SubData(BufferSize offset, const BufferData& data) const {
		...
		(_obj_name(),
		  ...
		  GLsizei(data.Size()),
		  data.Data());
		OGLPLUS_CHECK(NamedBufferSubDataEXT, ObjectError, Object(*this));
	}
	...
	static void CopySubData(BufferName readbuffer,
	  ...
	  BufferSize size) {
		...
		(GetGLName(readbuffer),
		  ...
		  GLsizeiptr(size.Get()));
		OGLPLUS_CHECK(NamedCopyBufferSubDataEXT,
		  ObjectPairError,
	...
	void ClearData(PixelDataInternalFormat internal_format,
	  ...
	  const GLtype* data) const {
		...
		(_obj_name(),
		  ...
		  data);
		OGLPLUS_CHECK(ClearNamedBufferDataEXT,
		  ObjectError,
	...
	void ClearSubData(PixelDataInternalFormat internal_format,
	  ...
	  const GLtype* data) const {
		...
		(_obj_name(),
		  ...
		  GLenum(GetDataType<GLtype>()),
		  data);
		OGLPLUS_CHECK(ClearNamedBufferSubDataEXT,
		  ObjectError,
		  Object(*this).EnumParam(internal_format));
	}
#endif
...
inline DSABufferOpsAndUsageEXT operator<<(
  DSABufferOpsEXT& buf, BufferUsage usage) {
...
inline DSABufferOpsAndIdxTgtEXT operator<<(
  DSABufferOpsEXT& buf, BufferIndexedTarget target) {
...
inline DSABufferOpsAndOffsetEXT operator+(
  DSABufferOpsEXT& buf, GLintptr offset) {
...
// Bind
inline DSABufferOpsEXT& operator<<(DSABufferOpsEXT& buf, BufferTarget target) {
	buf.Bind(target);
	return buf;
}
...
inline DSABufferOpsEXT& operator<<(
  const DSABufferOpsAndIdxTgtEXT& bat, GLuint index) {
...
template <typename GLtype>
inline DSABufferOpsEXT& operator<<(
  DSABufferOpsEXT& buf, const std::vector<GLtype>& data) {
	buf.Data(data);
	return buf;
}
...
template <typename GLtype>
inline DSABufferOpsEXT& operator<<(
  DSABufferOpsAndUsageEXT&& bau, const std::vector<GLtype>& data) {
...
inline DSABufferOpsEXT& operator<<(
  DSABufferOpsEXT& buf, const GLtype (&data)[Count]) {
	buf.Data(data);
	return buf;
}
...
template <typename GLtype, std::size_t Count>
inline DSABufferOpsEXT& operator<<(
  DSABufferOpsAndUsageEXT&& bau, const GLtype (&data)[Count]) {
	bau.buf.Data(data, bau.usage);
	return bau.buf;
}
...
template <typename GLtype>
inline DSABufferOpsEXT& operator<<(
  DSABufferOpsAndOffsetEXT&& bao, const std::vector<GLtype>& data) {
...
template <typename GLtype, std::size_t Count>
inline DSABufferOpsEXT& operator<<(
  DSABufferOpsAndOffsetEXT&& bao, const GLtype (&data)[Count]) {
	bao.buf.SubData(bao.offset, data);
	return bao.buf;
}
