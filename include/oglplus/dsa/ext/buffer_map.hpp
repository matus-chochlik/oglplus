...
public:
    ...
    DSABufferRawMapEXT(
      ...
      , _ptr(OGLPLUS_GLFUNC(MapNamedBufferRangeEXT)(
          GetGLName(buffer), _offset, _size, GLbitfield(access)))
      , _buf_name(GetGLName(buffer)) {
        OGLPLUS_CHECK(MapNamedBufferRangeEXT, ObjectError, Object(buffer));
    }
     ...
     *  @throws Error
     */
    DSABufferRawMapEXT(BufferName buffer, Bitfield<BufferMapAccess> access)
      : _offset(0)
      , _size(_get_size(GetGLName(buffer)))
      , _ptr(OGLPLUS_GLFUNC(MapNamedBufferEXT)(
          GetGLName(buffer), _translate(GLbitfield(access))))
      , _buf_name(GetGLName(buffer)) {
        OGLPLUS_CHECK(MapNamedBufferEXT, ObjectError, Object(buffer));
    }
...
class DSABufferTypedMapEXT : public DSABufferRawMapEXT {
public:
    /// Maps a range of the buffer
    /**
     *  @param buffer use the specified buffer
     ...
     *  @throws Error
     */
    DSABufferTypedMapEXT(BufferName buffer, Bitfield<BufferMapAccess> access)
