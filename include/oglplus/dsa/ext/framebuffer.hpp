...
public:
	...
	FramebufferStatus Status(Target fbo_target) const {
		GLenum result = OGLPLUS_GLFUNC(CheckNamedFramebufferStatusEXT)(
		  _obj_name(), GLenum(fbo_target));
		if(result == 0)
			OGLPLUS_CHECK(CheckNamedFramebufferStatusEXT,
			  ObjectError,
			  Object(*this).BindTarget(fbo_target));
	...
	void AttachRenderbuffer(
	  Property::Attachment attachment, RenderbufferName renderbuffer) {
		OGLPLUS_GLFUNC(NamedFramebufferRenderbufferEXT)
		(_obj_name(),
		  GLenum(attachment),
		  GL_RENDERBUFFER,
		  GetGLName(renderbuffer));
		OGLPLUS_CHECK(NamedFramebufferRenderbufferEXT,
		  ObjectPairError,
		  Subject(renderbuffer).Object(*this));
	}
	 ...
	 *  @glfunref{FramebufferRenderbuffer}
	 */
	void AttachColorRenderbuffer(FramebufferColorAttachmentNumber attachment_no,
	 ...
	 */
	void AttachTexture(
	  Property::Attachment attachment, TextureName texture, GLint level) {
		OGLPLUS_GLFUNC(NamedFramebufferTextureEXT)
		(_obj_name(), GLenum(attachment), GetGLName(texture), level);
		OGLPLUS_CHECK(NamedFramebufferTextureEXT,
		  ObjectPairError,
		  Subject(texture).Object(*this).Index(level));
	}
	 ...
	 *  @glfunref{FramebufferTexture}
	 */
	void AttachColorTexture(FramebufferColorAttachmentNumber attachment_no,
	  TextureName texture,
	  GLint level) {
		OGLPLUS_GLFUNC(NamedFramebufferTextureEXT)
		(_obj_name(),
	 ...
	 */
	void AttachTexture1D(Property::Attachment attachment,
	  ...
	  GLint level) {
		...
		(_obj_name(),
		  ...
		  GetGLName(texture),
		  level);
		OGLPLUS_CHECK(NamedFramebufferTexture1DEXT,
		  ObjectPairError,
		  Subject(texture).Object(*this).Index(level));
	}
	 ...
	 */
	void AttachTexture2D(Property::Attachment attachment,
	  TextureTarget textarget,
	  TextureName texture,
	  GLint level) {
		...
		(_obj_name(),
		  ...
		  GetGLName(texture),
		  level);
		OGLPLUS_CHECK(NamedFramebufferTexture2DEXT,
		  ObjectPairError,
		  Subject(texture).Object(*this).Index(level));
	}
	 ...
	 */
	void AttachTexture3D(Property::Attachment attachment,
	  TextureTarget textarget,
	  TextureName texture,
	  GLint level,
	  GLint layer) {
		...
		(_obj_name(),
		  ...
		  level,
		  layer);
		OGLPLUS_CHECK(NamedFramebufferTexture3DEXT,
		  ObjectPairError,
		  Subject(texture).Object(*this).Index(level));
	}
	 ...
	 */
	void AttachTextureLayer(Property::Attachment attachment,
	  TextureName texture,
	  GLint level,
	  GLint layer) {
		OGLPLUS_GLFUNC(NamedFramebufferTextureLayerEXT)
		(_obj_name(), GLenum(attachment), GetGLName(texture), level, layer);
		OGLPLUS_CHECK(NamedFramebufferTextureLayerEXT,
		  ObjectPairError,
		  Subject(texture).Object(*this).Index(level));
	}
	...
	void DrawBuffer(ColorBuffer buffer) {
		OGLPLUS_GLFUNC(FramebufferDrawBufferEXT)(_obj_name(), GLenum(buffer));
		OGLPLUS_VERIFY(FramebufferDrawBufferEXT, ObjectError, Object(*this));
	}
	...
	void DrawBuffers(const EnumArray<ColorBuffer>& buffers) {
		...
		(_obj_name(), GLint(buffers.Count()), buffers.Values());
		OGLPLUS_VERIFY(FramebufferDrawBuffersEXT, ObjectError, Object(*this));
	}
	...
	void ReadBuffer(ColorBuffer buffer) {
		OGLPLUS_GLFUNC(FramebufferReadBufferEXT)(_obj_name(), GLenum(buffer));
		OGLPLUS_VERIFY(FramebufferReadBufferEXT, ObjectError, Object(*this));
	}
};
...
inline DSAFramebufferOpsAndAttchEXT operator<<(
  DSAFramebufferOpsEXT& fbo, DSAFramebufferOpsEXT::Property::Attachment attch) {
	return DSAFramebufferOpsAndAttchEXT(fbo, attch);
}

// Bind
inline DSAFramebufferOpsEXT& operator<<(
  DSAFramebufferOpsEXT& fbo, FramebufferTarget target) {
	fbo.target = target;
	fbo.Bind();
	return fbo;
}
...
// AttachTexture
inline DSAFramebufferOpsEXT& operator<<(
  DSAFramebufferOpsAndAttchEXT&& faa, TextureName tex) {
...
// AttachRenderbuffer
inline DSAFramebufferOpsEXT& operator<<(
  DSAFramebufferOpsAndAttchEXT&& faa, RenderbufferName rbo) {
...
// Complete
inline DSAFramebufferOpsEXT& operator<<(
  DSAFramebufferOpsEXT& fbo, FramebufferComplete) {
	fbo.Complete();
	return fbo;
}
