...
public:
	...
	void Resize(BufferSize size, BufferUsage usage = BufferUsage::StaticDraw) {
		...
		(_obj_name(), size.Get(), nullptr, GLenum(usage));
		OGLPLUS_CHECK(
	...
	void Data(const BufferData& data,
	  BufferUsage usage = BufferUsage::StaticDraw) const {
		OGLPLUS_GLFUNC(NamedBufferData)
		(_obj_name(), GLsizei(data.Size()), data.Data(), GLenum(usage));
		OGLPLUS_CHECK(
		  NamedBufferData, ObjectError, Object(*this).EnumParam(usage));
	}
	...
	void SubData(BufferSize offset, const BufferData& data) const {
		...
		(_obj_name(),
		  ...
		  GLsizei(data.Size()),
		  data.Data());
		OGLPLUS_CHECK(NamedBufferSubData, ObjectError, Object(*this));
	}
	 ...
	 */
	static void CopySubData(BufferName readbuffer,
	  ...
	  BufferSize size) {
		...
		(GetGLName(readbuffer),
		  ...
		  GLsizeiptr(size.Get()));
		OGLPLUS_CHECK(CopyNamedBufferSubData,
		  ObjectPairError,
		  Subject(readbuffer).Object(writebuffer));
	}
#endif // copy buffer
	 ...
	 */
	template <typename GLtype>
	void ClearData(PixelDataInternalFormat internal_format,
	  ...
	  const GLtype* data) const {
		...
		(_obj_name(),
		  ...
		  data);
		OGLPLUS_CHECK(ClearNamedBufferData,
		  ObjectError,
	 ...
	 */
	template <typename GLtype>
	void ClearSubData(PixelDataInternalFormat internal_format,
	  BufferSize offset,
	  BufferSize size,
	  PixelDataFormat format,
	  const GLtype* data) const {
		...
		(_obj_name(),
		  ...
		  GLenum(GetDataType<GLtype>()),
		  data);
		OGLPLUS_CHECK(ClearNamedBufferSubData,
		  ObjectError,
		  Object(*this).EnumParam(internal_format));
	}

	void Storage(
	  const BufferData& data, Bitfield<BufferStorageBit> flags) const {
		OGLPLUS_GLFUNC(NamedBufferStorage)
		(_obj_name(), GLsizeiptr(data.Size()), data.Data(), GLbitfield(flags));
		OGLPLUS_CHECK(NamedBufferStorage, ObjectError, Object(*this));
	}

	void Storage(BufferSize size,
	  const void* data,
	  Bitfield<BufferStorageBit> flags) const {
	...
	void MakeResident(AccessSpecifier access) const {
		OGLPLUS_GLFUNC(MakeNamedBufferResidentNV)(_obj_name(), GLenum(access));
		OGLPLUS_CHECK(MakeNamedBufferResidentNV,
		  ObjectError,
		  Object(*this).EnumParam(access));
	}
	...
	BufferGPUAddress GPUAddress(void) const {
		...
		OGLPLUS_CHECK(
		  GetNamedBufferParameterui64vNV, ObjectError, Object(*this));
...
inline DSABufferOpsAndIdxTgt operator<<(
  DSABufferOps& buf, BufferIndexedTarget target) {
...
// Bind
inline DSABufferOps& operator<<(DSABufferOps& buf, BufferTarget target) {
	buf.Bind(target);
	return buf;
}
...
inline DSABufferOps& operator<<(
  const DSABufferOpsAndIdxTgt& bat, GLuint index) {
...
template <typename GLtype>
inline DSABufferOps& operator<<(
  DSABufferOps& buf, const std::vector<GLtype>& data) {
	buf.Data(data);
	return buf;
}
...
template <typename GLtype>
inline DSABufferOps& operator<<(
  DSABufferOpsAndUsage&& bau, const std::vector<GLtype>& data) {
...
inline DSABufferOps& operator<<(
  DSABufferOps& buf, const GLtype (&data)[Count]) {
	buf.Data(data);
	return buf;
}
...
template <typename GLtype, std::size_t Count>
inline DSABufferOps& operator<<(
  DSABufferOpsAndUsage&& bau, const GLtype (&data)[Count]) {
	bau.buf.Data(data, bau.usage);
	return bau.buf;
}
...
template <typename GLtype>
inline DSABufferOps& operator<<(
  DSABufferOpsAndOffset&& bao, const std::vector<GLtype>& data) {
...
template <typename GLtype, std::size_t Count>
inline DSABufferOps& operator<<(
  DSABufferOpsAndOffset&& bao, const GLtype (&data)[Count]) {
	bao.buf.SubData(bao.offset, data);
	return bao.buf;
}
