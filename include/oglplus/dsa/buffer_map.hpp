...
private:
    ...
    static GLsizeiptr _get_size(GLuint buf_name) {
        ...
        (buf_name, GL_BUFFER_SIZE, &value);
        OGLPLUS_CHECK(
          GetNamedBufferParameteriv, ObjectError, Object(BufferName(buf_name)));
...
public:
    ...
    DSABufferRawMap(
      ...
      , _ptr(OGLPLUS_GLFUNC(MapNamedBufferRange)(
          GetGLName(buffer), _offset, _size, GLbitfield(access)))
      , _buf_name(GetGLName(buffer)) {
        OGLPLUS_CHECK(MapNamedBufferRange, ObjectError, Object(buffer));
    }
     ...
     *  @throws Error
     */
    DSABufferRawMap(BufferName buffer, Bitfield<BufferMapAccess> access)
      : _offset(0)
      , _size(_get_size(GetGLName(buffer)))
      , _ptr(OGLPLUS_GLFUNC(MapNamedBuffer)(
          GetGLName(buffer), _translate(GLbitfield(access))))
      , _buf_name(GetGLName(buffer)) {
        OGLPLUS_CHECK(MapNamedBuffer, ObjectError, Object(buffer));
    }
    ...
    void FlushRange(BufferSize offset, BufferSize length) {
        ...
        (_buf_name, GLintptr(offset.Get()), GLsizeiptr(length.Get()));
        OGLPLUS_CHECK(
...
class DSABufferTypedMap : public DSABufferRawMap {
public:
    /// Maps a range of the buffer
    /**
     *  @param target use the buffer bound to the target specified
