...
public:
	...
	{
		OGLPLUS_GLFUNC(NamedFramebufferTexture1D)(
			_name,
			GLenum(attachment),
			GLenum(textarget),
			GetGLName(texture),
	...
	{
		OGLPLUS_GLFUNC(FramebufferDrawBuffer)(
			_name,
			GLenum(buffer)
		);
		OGLPLUS_VERIFY(
	...
	{
		OGLPLUS_GLFUNC(FramebufferDrawBuffers)(
			_name,
			buffers.Count(),
			buffers.Values()
		);
	...
	{
		OGLPLUS_GLFUNC(FramebufferReadBuffer)(_name, GLenum(buffer));
		OGLPLUS_VERIFY(
