...
private:
    ...
    static GLsizeiptr _get_size(BufferTarget target) {
        ...
        (GLenum(target), GL_BUFFER_SIZE, &value);
        OGLPLUS_CHECK(GetBufferParameteriv, ObjectError, ObjectBinding(target));
...
public:
    ...
    BufferRawMap(
      ...
      , _ptr(OGLPLUS_GLFUNC(MapBufferRange)(
          GLenum(target), _offset, _size, GLbitfield(access)))
      , _target(target) {
        OGLPLUS_CHECK(MapBufferRange, Error, EnumParam(target));
    }
    ...
    BufferRawMap(BufferTarget target, Bitfield<BufferMapAccess> access)
      ...
      , _ptr(OGLPLUS_GLFUNC(MapBuffer)(
          GLenum(target), _translate(GLbitfield(access))))
      , _target(target) {
        OGLPLUS_CHECK(MapBuffer, ObjectError, ObjectBinding(_target));
    }
    ...
    void FlushRange(BufferSize offset, BufferSize length) {
        ...
        (GLenum(_target), GLintptr(offset.Get()), GLsizeiptr(length.Get()));
        OGLPLUS_CHECK(
          FlushMappedBufferRange, ObjectError, ObjectBinding(_target));
    }
};

/// Typed mapping of the buffer to the client address space
template <typename Type>
class BufferTypedMap : public BufferRawMap {
public:
    /// Maps a range of the buffer
    /**
     *  @param target use the buffer bound to the target specified
     ...
     */
    void FlushElements(
      BufferTypedSize<Type> start, BufferTypedSize<Type> count) {
