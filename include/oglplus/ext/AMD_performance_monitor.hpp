...
public:
	...
	String GetString(void) const {
		...
		OGLPLUS_CHECK(GetPerfMonitorCounterStringAMD, Error, Index(_group));

		std::vector<GLchar> buffer(length);
		OGLPLUS_GLFUNC(GetPerfMonitorCounterStringAMD)
		(_group, _counter, GLsizei(buffer.size()), &length, buffer.data());
		OGLPLUS_CHECK(GetPerfMonitorCounterStringAMD, Error, Index(_group));
	...
	PerfMonitorAMDType Type(void) const {
		...
		OGLPLUS_CHECK(GetPerfMonitorCounterInfoAMD, Error, Index(_group));
...
public:
	...
	String GetString(void) const {
		...
		OGLPLUS_CHECK(GetPerfMonitorGroupStringAMD, Error, Index(_group));

		std::vector<GLchar> buffer(length);
		OGLPLUS_GLFUNC(GetPerfMonitorGroupStringAMD)
		(_group, GLsizei(buffer.size()), &length, buffer.data());
		OGLPLUS_CHECK(GetPerfMonitorGroupStringAMD, Error, Index(_group));
		return String(buffer.data(), buffer.size());
	}
	 ...
	 */
	void GetCounters(GLint& max_active_counters,
	  std::vector<PerfMonitorAMDCounter>& counters) const {
		GLint count = 0;
		OGLPLUS_GLFUNC(GetPerfMonitorCountersAMD)
		(_group, &count, &max_active_counters, 0, nullptr);
		OGLPLUS_CHECK(GetPerfMonitorCountersAMD, Error, Index(_group));
	 ...
	 *  @glfunref{GetPerfMonitorCountersAMD}
	 */
	std::vector<PerfMonitorAMDCounter> GetCounters(
	  GLint& max_active_counters) const {
...
private:
	...
	PerfMonitorAMDResult(PerfMonitorAMDCounter counter, GLuint lo, GLuint hi)
	  : _counter(counter) {
		_un._qw._lo = lo;
		_un._qw._hi = hi;
	}
...
public:
	 ...
	 */
	void SelectCounters(Boolean enable,
	  const std::vector<PerfMonitorAMDCounter>& counters) const {
		if(counters.empty())
			return;
		...
		(this->_obj_name(),
		  ...
		  GLint(list.size()),
		  list.data());
		OGLPLUS_CHECK(SelectPerfMonitorCountersAMD, ObjectError, Object(*this));
	}
	...
	bool ResultAvailable(void) const {
		...
		(this->_obj_name(),
		  ...
		  &result,
		  nullptr);
		OGLPLUS_CHECK(GetPerfMonitorCounterDataAMD, ObjectError, Object(*this));
		return result != 0;
	}
...
public:
	...
	void GetGroups(std::vector<PerfMonitorAMDGroup>& groups) const {
		GLint count = 0;
		OGLPLUS_GLFUNC(GetPerfMonitorGroupsAMD)(&count, 0, nullptr);
		OGLPLUS_CHECK_SIMPLE(GetPerfMonitorGroupsAMD);

		std::vector<GLuint> buffer(count);
		OGLPLUS_GLFUNC(GetPerfMonitorGroupsAMD)
		(&count, GLsizei(buffer.size()), buffer.data());
		OGLPLUS_CHECK_SIMPLE(GetPerfMonitorGroupsAMD);
