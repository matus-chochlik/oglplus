...
public:
    ...
    /// Enables/disables messages with specific parameters
    static void Control(
      DebugSource source, DebugType type, DebugSeverity severity, bool enable) {
    ...
    public:
        ...
        LogSink(Callback callback)
          ...
          , _prev_context(nullptr) {
            ...
            OGLPLUS_GLFUNC(GetPointerv)(GL_DEBUG_CALLBACK_FUNCTION, _tmp_ptr);
            OGLPLUS_IGNORE(GetPointerv);
            _prev_callback = _tmp_callback;

            // get the previous context
            OGLPLUS_GLFUNC(GetPointerv)
            (GL_DEBUG_CALLBACK_USER_PARAM, &_prev_context);
            OGLPLUS_IGNORE(GetPointerv);

            OGLPLUS_GLFUNC(DebugMessageCallback)
            (GLDEBUGPROC(&LogSink::_gl_debug_proc), static_cast<void*>(this));
    ...
    static void InsertMessage(
      ...
      const GLchar* buffer) {
        ...
        (GLenum(source), GLenum(type), id, GLenum(severity), length, buffer);
    ...
    static void InsertMessage(
      ...
      StrCRef message) {
        InsertMessage(
          source, type, id, severity, message.size(), message.c_str());
    }
    ...
    static void InsertMessage(
      ...
      const GLchar (&message)[N]) {
        InsertMessage(source, type, id, severity, N, message);
    }

    /// Pushes a debug group
    static void PushGroup(
      DebugSource source, GLuint id, GLsizei length, const GLchar* message) {
        OGLPLUS_GLFUNC(PushDebugGroup)(GLenum(source), id, length, message);
    ...
    template <std::size_t N>
    static void PushGroup(
      DebugSource source, GLuint id, const GLchar (&message)[N]) {
