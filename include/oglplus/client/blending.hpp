...
private:
    static context::BlendEquationSeparate _do_get(DrawBufferIndex index) {
        if(GLuint(index) == 0) {
            return context::BlendingState::BlendEquationSeparate();
        } else {
            return context::BlendingState::BlendEquationSeparate(index);
        }
    }

    static void _do_set(
      context::BlendEquationSeparate value, DrawBufferIndex index) {
        if(GLuint(index) == 0) {
            if(value.Separate()) {
            ...
            if(value.Separate()) {
                context::BlendingState::BlendEquationSeparate(index, value);
            } else {
                context::BlendingState::BlendEquation(index, value.Alpha());
            }
        }
    }
...
private:
    static context::BlendFunctionSeparate _do_get(DrawBufferIndex index) {
        if(GLuint(index) == 0) {
            return context::BlendingState::BlendFuncSeparate();
        } else {
            return context::BlendingState::BlendFuncSeparate(index);
        }
    }

    static void _do_set(
      context::BlendFunctionSeparate value, DrawBufferIndex index) {
        if(GLuint(index) == 0) {
            if(value.Separate()) {
                context::BlendingState::BlendFuncSeparate(value);
            } else {
                context::BlendingState::BlendFunc(
                  value.SrcAlpha(), value.DstAlpha());
            }
        } else {
            if(value.Separate()) {
                context::BlendingState::BlendFuncSeparate(index, value);
            } else {
                context::BlendingState::BlendFunc(
                  index, value.SrcAlpha(), value.DstAlpha());
            }
        }
    }
...
private:
    static context::BlendEquationSeparate _do_get(Nothing) {
        return context::BlendingState::BlendEquationSeparate();
    }

    static void _do_set(context::BlendEquationSeparate value, Nothing) {
        if(value.Separate()) {
            context::BlendingState::BlendEquationSeparate(value);
        } else {
            context::BlendingState::BlendEquation(value.Alpha());
...
private:
    static context::BlendFunctionSeparate _do_get(Nothing) {
        return context::BlendingState::BlendFuncSeparate();
    }

    static void _do_set(context::BlendFunctionSeparate value, Nothing) {
        if(value.Separate()) {
            context::BlendingState::BlendFuncSeparate(value);
        } else {
            context::BlendingState::BlendFunc(
...
class BlendColor : public SettingStack<context::RGBAValue, Nothing> {
private:
