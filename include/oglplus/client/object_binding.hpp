...
struct CurrentObject {
    ...
    public:
        WithTarget(void)
            : SettingStack<GLuint, Nothing>(&_do_get, &_do_bind), BoundObjOps<ObjTag>(ObjTgt)
        {
        }
...
private:
    static BufferNameAndRange _do_get(GLuint index)
    {
        ...
        result._buffer = GetGLName(ObjBindingOps<tag::Buffer>::Binding(BufTgt, index));
        result._offset = 0;
        result._size = 0;
        return result;
    }
...
public:
    ...
    typedef SettingHolder<BufferNameAndRange, GLuint> Holder;
