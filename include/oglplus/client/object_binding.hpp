...
struct CurrentObject {
	...
	private:
		static GLuint _do_get(Nothing) {
			return GetGLName(ObjBindingOps<ObjTag>::Binding(ObjTgt));
		}

		static void _do_bind(GLuint obj, Nothing) {
...
class CurrentObjectsWithTarget
  : public oglplus::enums::EnumToClass<Nothing,
...
template <typename ObjTag>
class CurrentObjectWithoutTarget : public SettingStack<GLuint, Nothing> {
private:
	static GLuint _do_get(Nothing) {
	...
	static void _do_bind(GLuint obj, Nothing) {
		ObjBindingOps<ObjTag>::Bind(ObjectName<ObjTag>(obj));
	}
...
class CurrentIndexBuffers : public SettingStack<BufferNameAndRange, GLuint> {
private:
	static BufferNameAndRange _do_get(GLuint index) {
	...
	static void _do_bind(BufferNameAndRange bnr, GLuint index) {
		if(bnr._size == 0) {
			ObjBindingOps<tag::Buffer>::BindBase(BufTgt, index, bnr.Buffer());
		} else {
			ObjBindingOps<tag::Buffer>::BindRange(
			  BufTgt, index, bnr.Buffer(), bnr.Offset(), bnr.Size());
		}
	}
...
public:
	...
	Holder Push(BufferName obj, BufferSize offset, BufferSize size) {
		BufferNameAndRange bnr = {
		  GetName(obj), GLintptr(offset), GLsizei(size)};
		return this->_push(bnr);
	}
...
private:
	...
	static void _active_tex(GLuint tex_unit) {
		...
		OGLPLUS_VERIFY(ActiveTexture, Error, Index(tex_unit));
	}
	...
	static void _do_bind(GLuint obj, GLuint tex_unit) {
		_active_tex(tex_unit);
		ObjBindingOps<ObjTag>::Bind(ObjTgt, ObjectName<ObjTag>(obj));
	}
...
private:
	...
	static void _do_bind(GLuint obj, GLuint tex_unit) {
		ObjBindingOps<ObjTag>::Bind(tex_unit, ObjectName<ObjTag>(obj));
	}
