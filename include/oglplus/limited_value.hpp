...
private:
	...
	static inline GLuint _check_u(GLuint value, const char* query_name) {
		OGLPLUS_HANDLE_ERROR_IF(!(value < _limit()),
		  GL_INVALID_VALUE,
		  LimitError::Message(),
		  LimitError,
		  Value(value).Limit(_limit()).EnumParam(Query, query_name));
		return value;
	}

	static inline GLuint _check_i(GLint value, const char* query_name) {
		OGLPLUS_HANDLE_ERROR_IF((value < 0),
		  GL_INVALID_VALUE,
		  LimitError::MessageNeg(),
		  LimitError,
		  Limit(_limit()).EnumParam(Query, query_name));
		OGLPLUS_HANDLE_ERROR_IF(!(GLuint(value) < _limit()),
		  GL_INVALID_VALUE,
		  LimitError::Message(),
		  LimitError,
		  Value(GLuint(value)).Limit(_limit()).EnumParam(Query, query_name));
		return GLuint(value);
	}
