...
struct ViewportPosition {
	...
	/// The x-coordinate
	GLint X(void) const noexcept {
		return _v[0];
	}

	/// The y-coordinate
	GLint Y(void) const noexcept {
		return _v[1];
	}
};
...
struct ViewportSize {
	...
	GLint Width(void) const noexcept {
		return _v[0];
	}

	/// The height of the viewport
	GLint Height(void) const noexcept {
		return _v[1];
	}
};
...
struct ViewportExtents {
	...
	ViewportExtents(void) noexcept {
	}

	ViewportExtents(GLint x, GLint y, GLint w, GLint h) noexcept {
		_v[0] = x;
		_v[1] = y;
		_v[2] = w;
		_v[3] = h;
	...
	/// The x-coordinate
	GLint Left(void) const noexcept {
		return _v[0];
	}

	/// The y-coordinate
	GLint Bottom(void) const noexcept {
		return _v[1];
	}
	...
	friend bool operator==(
	  const ViewportExtents& a, const ViewportExtents& b) noexcept {
...
struct BoundsRange {
	...
	/// The min limit
	GLint Min(void) const noexcept {
		return _v[0];
	}

	/// The max limit
	GLint Max(void) const noexcept {
		return _v[1];
	}
};
...
struct ViewportDepthRange {
	...
	ViewportDepthRange(void) noexcept {
	}

	ViewportDepthRange(GLfloat near_val, GLfloat far_val) noexcept {
		_v[0] = near_val;
		_v[1] = far_val;
	}

	/// The near limit
	GLfloat Near(void) const noexcept {
		return _v[0];
	}

	/// The far limit
	GLfloat Far(void) const noexcept {
		return _v[1];
	}
};
...
public:
	...
	static void Viewport(const ViewportExtents& vp) {
		OGLPLUS_GLFUNC(Viewport)(vp.X(), vp.Y(), vp.Width(), vp.Height());
		OGLPLUS_CHECK_SIMPLE(Viewport);
	}
...
#if OGLPLUS_DOCUMENTATION_ONLY || GL_ES_VERSION_3_0 || GL_VERSION_4_1 \
  || GL_ARB_ES2_compatibility
	 ...
	 */
	static void Viewport(ViewportIndex viewport,
	  GLfloat x,
	  GLfloat y,
	  GLfloat width,
	  GLfloat height) {
		OGLPLUS_GLFUNC(ViewportIndexedf)(GLuint(viewport), x, y, width, height);
		OGLPLUS_CHECK(ViewportIndexedf, Error, Index(GLuint(viewport)));
	}
	...
	static void Viewport(ViewportIndex viewport, const GLfloat* extents) {
		...
		OGLPLUS_CHECK(ViewportIndexedfv, Error, Index(GLuint(viewport)));
	}
	...
	static void ViewportArray(
	  GLuint first, SizeType count, const GLfloat* extents) {
	 ...
	 *  @glfunref{ViewportIndexedf}
	 */
	static void Viewport(ViewportIndex viewport, const ViewportExtents& vp) {
	...
	static ViewportExtents Viewport(ViewportIndex viewport) {
		ViewportExtents result;
		OGLPLUS_GLFUNC(GetIntegeri_v)(GL_VIEWPORT, GLuint(viewport), result._v);
	...
	static void DepthRange(
	  ViewportIndex viewport, GLclampd near_val, GLclampd far_val) {
		OGLPLUS_GLFUNC(DepthRangeIndexed)(GLuint(viewport), near_val, far_val);
		OGLPLUS_CHECK(DepthRangeIndexed, Error, Index(GLuint(viewport)));
	}

	static void DepthRange(
	  ViewportIndex viewport, const ViewportDepthRange& vdr) {
	...
	static void DepthRangeArray(
	  GLuint first, SizeType count, const GLclampd* v) {
	...
	static ViewportDepthRange DepthRange(ViewportIndex viewport) {
		...
		(GL_DEPTH_RANGE, GLuint(viewport), result._v);
		OGLPLUS_CHECK(GetFloati_v, Error, Index(GLuint(viewport)));
		return result;
	}
#endif
};
...
public:
	...
	static ViewportSize MaxViewportDims(void) {
		ViewportSize result;
		OGLPLUS_GLFUNC(GetIntegerv)(GL_MAX_VIEWPORT_DIMS, result._v);
	...
	static BoundsRange ViewportBoundsRange(void) {
		BoundsRange result;
		OGLPLUS_GLFUNC(GetIntegerv)(GL_VIEWPORT_BOUNDS_RANGE, result._v);
		OGLPLUS_VERIFY_SIMPLE(GetIntegerv);
		return result;
	}
