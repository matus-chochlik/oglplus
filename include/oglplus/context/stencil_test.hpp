...
struct StencilFuncArgs {
	...
	StencilFuncArgs(void) noexcept {
	}

	StencilFuncArgs(CompareFunction func,
	  GLint refv = GLint(0),
	...
	CompareFunction Func(void) const noexcept {
	...
	GLuint ValueMask(void) const noexcept {
		return _mask;
	}

	friend bool operator==(
	  const StencilFuncArgs& a, const StencilFuncArgs& b) noexcept {
		return (a._func == b._func) && (a._refv == b._refv)
			   && (a._mask == b._mask);
	}
...
struct StencilOperations {
	...
	StencilOperations(void) noexcept {
	}

	StencilOperations(StencilOperation sfail,
	...
	StencilOperation StencilFail(void) const noexcept {
		return _sfail;
	}

	StencilOperation DepthFail(void) const noexcept {
		return _dfail;
	}

	StencilOperation DepthPass(void) const noexcept {
		return _dpass;
	}

	friend bool operator==(
	  const StencilOperations& a, const StencilOperations& b) noexcept {
		return (a._sfail == b._sfail) && (a._dfail == b._dfail)
			   && (a._dpass == b._dpass);
	}
...
public:
	...
	static void StencilFunc(
	  CompareFunction func, GLint ref = GLint(0), GLuint mask = ~GLuint(0)) {
		OGLPLUS_GLFUNC(StencilFunc)(GLenum(func), ref, mask);
		OGLPLUS_VERIFY(StencilFunc, Error, EnumParam(func));
	}
	...
	static void StencilFuncSeparate(Face face,
	  ...
	  GLuint mask = ~GLuint(0)) {
		...
		(GLenum(face), GLenum(func), ref, mask);
		OGLPLUS_VERIFY(StencilFuncSeparate, Error, EnumParam(func));
	}

	static void StencilFuncSeparate(Face face, const StencilFuncArgs& fa) {
		return StencilFuncSeparate(face, fa.Func(), fa.Ref(), fa.ValueMask());
	}

	static void StencilFuncSeparateSingle(SingleFace face,
	  CompareFunction func,
	  GLint ref = GLint(0),
	  GLuint mask = ~GLuint(0)) {
		OGLPLUS_GLFUNC(StencilFuncSeparate)
	...
	static void StencilFuncSeparateSingle(
	  SingleFace face, const StencilFuncArgs& fa) {
		return StencilFuncSeparateSingle(
		  face, fa.Func(), fa.Ref(), fa.ValueMask());
	}
	...
	static CompareFunction StencilFunc(bool backface = false) {
		...
		(backface ? GL_STENCIL_BACK_FUNC : GL_STENCIL_FUNC, &result);
	...
	static GLint StencilRef(bool backface = false) {
		GLint result;
		OGLPLUS_GLFUNC(GetIntegerv)
		(backface ? GL_STENCIL_BACK_REF : GL_STENCIL_REF, &result);
		OGLPLUS_VERIFY_SIMPLE(GetIntegerv);
		return result;
	}
	...
	static GLuint StencilValueMask(bool backface = false) {
		GLint result;
		OGLPLUS_GLFUNC(GetIntegerv)
		(backface ? GL_STENCIL_BACK_VALUE_MASK : GL_STENCIL_VALUE_MASK,
		  &result);
		OGLPLUS_VERIFY_SIMPLE(GetIntegerv);
	...
	static oglplus::context::StencilFuncArgs StencilFuncArgs(
	  bool backface = false) {
		return oglplus::context::StencilFuncArgs(StencilFunc(backface),
		  StencilRef(backface),
		  StencilValueMask(backface));
	}

	static oglplus::context::StencilFuncArgs StencilFuncArgs(Face face) {
		return StencilFuncArgs(face == Face::Back);
	}

	static oglplus::context::StencilFuncArgs StencilFuncArgsSingle(
	  SingleFace face) {
	...
	static void StencilOp(
	  StencilOperation sfail, StencilOperation dfail, StencilOperation dpass) {
	...
	static void StencilOpSeparateSingle(SingleFace face,
	  StencilOperation sfail,
	  StencilOperation dfail,
	  StencilOperation dpass) {
		OGLPLUS_GLFUNC(StencilOpSeparate)
	...
	static void StencilOpSeparate(Face face, const StencilOperations& ops) {
		StencilOpSeparate(face, ops._sfail, ops._dfail, ops._dpass);
	}

	static void StencilOpSeparateSingle(
	  SingleFace face, const StencilOperations& ops) {
		StencilOpSeparateSingle(face, ops._sfail, ops._dfail, ops._dpass);
	}
	...
	static StencilOperation StencilFail(bool backface = false) {
		GLint result;
		OGLPLUS_GLFUNC(GetIntegerv)
		(backface ? GL_STENCIL_BACK_FAIL : GL_STENCIL_FAIL, &result);
		OGLPLUS_VERIFY_SIMPLE(GetIntegerv);
	...
	static StencilOperation StencilPassDepthPass(bool backface = false) {
		GLint result;
		OGLPLUS_GLFUNC(GetIntegerv)
		(backface ? GL_STENCIL_BACK_PASS_DEPTH_PASS
	...
	static StencilOperations StencilOps(bool backface = false) {
		return StencilOperations(StencilFail(backface),
