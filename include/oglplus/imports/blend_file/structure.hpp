 ...
 *  @brief Class providing information about a structure defined in a .blend
 * file
 *
...
private:
    ...
    BlendFileStructField(
      BlendFileSDNA* sdna, std::size_t struct_index, std::size_t field_index)
      : _sdna(sdna)
...
public:
    BlendFileStructField Get(std::size_t index) const {
        return BlendFileStructField(_sdna, _struct_index, index);
    }
};
...
public:
    ...
    BlendFileStructFieldRange Fields() const {
        return BlendFileStructFieldRange(_sdna, _struct_index);
    }

    inline BlendFileFlattenedStruct Flattened() const;
};

BlendFileStruct BlendFileType::AsStructure() const {
