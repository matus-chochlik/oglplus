...
private:
    ...
    std::size_t _invalid_type_index() const {
    ...
    public:
        ...
        std::size_t _field_count() const {
            assert(_field_names.size() == _field_structs.size());
            assert(_field_names.size() == _field_indices.size());
            assert(_field_names.size() == _field_offsets.size());
    ...
    struct _struct_info {
        ...
        std::size_t _field_count() const {
            assert(_field_type_indices.size() == _field_name_indices.size());
            assert(_field_type_indices.size() == _field_elem_counts.size());
            assert(_field_type_indices.size() == _field_ptr_flags.size());
            assert(_field_type_indices.size() == _field_ptr2_flags.size());
            assert(_field_type_indices.size() == _field_array_flags.size());
    ...
    std::size_t _invalid_struct_index() const {
    ...
    const std::shared_ptr<_flat_struct_info>& _struct_flatten_fields(
      std::size_t struct_index);

    // a sequence for assigning unique integer identifiers to
    // C++ types
    static std::size_t& _type_id_seq() {
    ...
    void _init_type_id_index(const std::string& type_name) {
        ...
        if(_type_id_to_type_index.size() <= tid) {
            _type_id_to_type_index.resize(tid + 4, _invalid_type_index());
        }
        ...
        if(pos == _type_map.end())
            ...
            _type_id_to_type_index[tid] = pos->second;
    }

    template <typename T>
    std::size_t _find_type_index() const {
        std::size_t tid = _type_id(TypeTag<T>());
        if(_type_id_to_type_index.size() > tid)
            return _type_id_to_type_index[tid];
        else
            return _invalid_type_index();
    }

    template <typename T>
    bool _is_known_type() const {
