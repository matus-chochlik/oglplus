...
protected:
    ...
    BlendFilePointerTpl<Level> _do_get(
      ...
      std::size_t field_element) const {
        return _block_data_ref->template _get_pointer<Level>(
          _flat_field, block_element, field_element, _offset);
    }
    ...
    typename std::enable_if<std::is_integral<Int>::value, Int>::type _do_get(
...
/// Convenience class combining functionality of FlattenedStruct, Block and
/// BlockData
...
public:
    ...
    const BlendFileFlattenedStruct& Structure() const {
        return _flat_struct;
    }
    ...
    std::size_t StructureSize() const {
        return Structure().Size();
    }

    /// Returns a range of fields of the block's flattened structure
    BlendFileFlattenedStructFieldRange StructureFields() const {
    ...
    /// Returns the block
    const BlendFileBlock& Block() const {
        return _block;
    }

    /// Returns the code of the block
    std::string BlockCode() const {
        return _block.Code();
    }
    ...
    void BlockDataValueVisit(
      ...
      std::size_t field_element = 0) const {
        BlockData().ValueVisit(
          visitor, flat_field, block_element, field_element, _offset);
    }
    ...
    BlendFileFlatStructTypedFieldData<T> Field(
      const std::string& field_name) const {
        return BlendFileFlatStructTypedFieldData<T>(
          Structure().FieldByName(field_name), BlockData(), _offset);
    }
    ...
    template <typename T>
    typename _adjust_type<T>::type TryGet(
      const std::string& field_name, T default_value) {
