...
public:
    ...
    const BlendFileBlock& GlobalBlock() const {
        return _blocks[_glob_block_index];
    }

    /// Returns a pointer to the global block
    BlendFilePointer GlobalBlockPointer() const {
    ...
    BlendFileStructGlobBlock StructuredGlobalBlock() {
        return BlendFileStructGlobBlock(
          StructuredBlockByPointer(GlobalBlockPointer()));
    }

    /// Returns a range allowing the traversal of file blocks
    BlendFileBlockRange Blocks() const {
