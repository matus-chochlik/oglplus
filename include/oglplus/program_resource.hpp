...
private:
    ...
    void QueryParams(
      GLenum property, GLsizei bufsize, GLsizei* written, GLint* params) const {
    ...
    GLint GetParam(GLenum property) const {
        ...
        OGLPLUS_VERIFY(GetProgramResourceiv, Error, EnumParam(property));
        return res;
    }
...
public:
    ProgramResource(aux::ProgramInterfaceContext& context, GLuint index);
    ...
    Boolean GetBoolParam(ProgramResourceProperty property) const {
        return Boolean(GetParam(GLenum(property)), std::nothrow);
    }
    ...
    ProgramInterface Interface() const {
    ...
    Boolean ReferencedBy(ShaderType shader_type) const {
        return Boolean(
          GetParam(ReferencedByProperty(shader_type)), std::nothrow);
    }
    ...
    Boolean IsPerPatch() const {
        return Boolean(GetParam(GL_IS_PER_PATCH), std::nothrow);
    }
