...
protected:
    ...
    VectorBase(
      ...
      typename std::enable_if<(N <= M) && (!std::is_same<T, U>::value)>::type* =
        nullptr) {
...
template <typename T, std::size_t N>
inline Vector<T, 1> Extract(const Vector<T, N>& a, std::size_t d0) {
    return Vector<T, 1>(a[d0]);
}

template <typename T, std::size_t N>
inline Vector<T, 2> Extract(
  const Vector<T, N>& a, std::size_t d0, std::size_t d1) {
    return Vector<T, 2>(a[d0], a[d1]);
}

template <typename T, std::size_t N>
inline Vector<T, 3> Extract(
  const Vector<T, N>& a, std::size_t d0, std::size_t d1, std::size_t d2) {
...
inline
  typename std::enable_if<std::is_convertible<V, T>::value, Vector<T, N>>::type
...
inline Vector<T, Cols> operator*(
  const Vector<T, N>& v, const Matrix<T, N, Cols>& m) {
    T tmp[Cols];
...
inline Vector<T, Rows> operator*(
  const Matrix<T, Rows, N>& m, const Vector<T, N>& v) {
    T tmp[Rows];
