...
public:
    ...
    unsigned SegmentCount() const {
        ...
        if(_connected)
            return unsigned((_points.size() - 1) / Order);
        else
            return unsigned(_points.size() / (Order + 1));
    }

    /// Returns the contol points of the curve
    const ::std::vector<Type>& ControlPoints() const {
        return _points;
    }
    ...
    Type Position01(Parameter t) const {
        ...
        return math::Bezier<Type, Parameter, Order>::Position(
          _points.data() + poffs, _points.size() - poffs, t_sub);
    }
    ...
    BezierCurves<Type, Parameter, Order - 1> Derivative() const {
        unsigned sstep = SegmentStep();
        unsigned s = SegmentCount();
...
class CubicBezierLoop : public BezierCurves<Type, Parameter, 3> {
private:
    template <typename StdRange>
    static std::vector<Type> _make_cpoints(
      const StdRange& points, Parameter r) {
