...
public:
    ...
    friend bool Equal(const Quaternion& q1, const Quaternion& q2) {
        return (q1._a == q2._a) && (q1._x == q2._x) && (q1._y == q2._y) &&
               (q1._z == q2._z);
    }
    ...
    static Quaternion Inverse(const Quaternion& q1) {
        ...
        assert(id != T(0));
        id *= T(1) / id;
        return Quaternion(+q1._a * id, -q1._x * id, -q1._y * id, -q1._z * id);
    }
    ...
    static Vector<T, 3> RotateVector(
      const Quaternion& q, const Vector<T, 3>& v) {
...
template <typename T>
inline Quaternion<T> Multiply(
  const Quaternion<T>& q1, const Quaternion<T>& q2) {
 ...
 */
template <typename T>
class QuaternionSLERP : public BaseSLERP<Quaternion<T>, T> {
public:
     ...
     */
    QuaternionSLERP(
      const Quaternion<T>& q1, const Quaternion<T>& q2, T eps = 0.001)
