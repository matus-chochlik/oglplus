...
protected:
    ...
    struct _op_add {
        ...
        void operator()(Matrix& t) const {
            for(std::size_t i = 0; i != Rows; ++i)
                for(std::size_t j = 0; j != Cols; ++j)
                    t._m._elem[i][j] = a._m._elem[i][j] + b._m._elem[i][j];
        }
    };
            ...
            for(std::size_t i = 0; i != Rows; ++i)
                for(std::size_t j = 0; j != Cols; ++j) {
                    ...
                    for(std::size_t k = 1; k != N; ++k) {
                        t._m._elem[i][j] += a.At(i, k) * b.At(k, j);
                    }
                }
        }
    };
    ...
    struct _op_extract {
        ...
        void operator()(Matrix<T, R, C>& t) const {
            static_assert(I + R <= Rows, "Invalid row for this matrix type");
            static_assert(J + C <= Cols, "Invalid column for this matrix type");
    ...
    struct _op_copy {
        ...
        void operator()(Matrix& t) const {
            static_assert(Rows <= R, "Invalid row for this matrix type");
            static_assert(Cols <= C, "Invalid column for this matrix type");
            for(std::size_t i = 0; i != Rows; ++i)
                for(std::size_t j = 0; j != Cols; ++j)
...
#include <oglplus/math/matrix_4_ctr.ipp>
    ...
    friend Matrix Multiplied(
      const Matrix<T, Rows, N>& a, const Matrix<T, N, Cols>& b) {
    ...
    friend void RowSwap(Matrix& m, std::size_t a, std::size_t b) {
        assert(a < Rows);
        assert(b < Rows);
        ::std::swap_ranges(m._m._elem[a], m._m._elem[a] + Cols, m._m._elem[b]);
    }
...
inline bool Close(const Matrix<T, R, C>& a, const Matrix<T, R, C>& b, T eps) {
    assert(eps >= T(0));
    for(std::size_t i = 0; i != R; ++i)
        for(std::size_t j = 0; j != C; ++j) {
            T u = a.At(i, j);
...
public:
    ...
    ModelMatrix(RotationA_, const Vector<T, 3>& axis, Angle<T> angle)
      : Base(oglplus::Nothing()) {
        ...
        const T xx = x * x, xy = x * y, xz = x * z, yy = y * y, yz = y * z,
                zz = z * z;
        InitMatrix4x4(
          *this,
          xx * _cf + cf,
...
public:
    ...
    CameraMatrix(
      Perspective_, T x_left, T x_right, T y_bottom, T y_top, T z_near, T z_far)
      : Base(oglplus::Nothing()) {
    ...
    static inline CameraMatrix Perspective(
      T x_left, T x_right, T y_bottom, T y_top, T z_near, T z_far) {
        return CameraMatrix(
          Perspective_(), x_left, x_right, y_bottom, y_top, z_near, z_far);
    }
    ...
    static inline CameraMatrix PerspectiveX(
      Angle<T> xfov, T aspect, T z_near, T z_far) {
        assert(aspect > T(0));
    ...
    static inline CameraMatrix PerspectiveX(
      Angle<T> xfov, T width, T height, T z_near, T z_far) {
        assert(height > 0);
    ...
    static inline CameraMatrix PerspectiveY(
      Angle<T> yfov, T aspect, T z_near, T z_far) {
        assert(aspect > T(0));
    ...
    static inline CameraMatrix PerspectiveY(
      Angle<T> yfov, T width, T height, T z_near, T z_far) {
        assert(height > 0);
    ...
    CameraMatrix(
      Ortho_, T x_left, T x_right, T y_bottom, T y_top, T z_near, T z_far)
      : Base(oglplus::Nothing()) {
    ...
    static inline CameraMatrix Ortho(
      T x_left, T x_right, T y_bottom, T y_top, T z_near, T z_far) {
        return CameraMatrix(
          Ortho_(), x_left, x_right, y_bottom, y_top, z_near, z_far);
    }
     ...
     *  x/y @p aspect ratio and z-axis @p z_near and @p z_far planes
     */
    static inline CameraMatrix OrthoX(T width, T aspect, T z_near, T z_far) {
        assert(aspect > T(0));
        assert(width > T(0));
     ...
     *  x/y @p aspect ratio and z-axis @p z_near and @p z_far planes
     */
    static inline CameraMatrix OrthoY(T height, T aspect, T z_near, T z_far) {
        assert(aspect > T(0));
        assert(height > T(0));
    ...
    CameraMatrix(ScreenStretch_, T x_left, T x_right, T y_bottom, T y_top)
      : Base(oglplus::Nothing()) {
    ...
    static inline CameraMatrix ScreenStretch(
      T x_left, T x_right, T y_bottom, T y_top) {
        return CameraMatrix(ScreenStretch_(), x_left, x_right, y_bottom, y_top);
    }
    ...
    static inline CameraMatrix ScreenTile(
      unsigned x, unsigned y, unsigned nx, unsigned ny) {
        assert(x < nx);
        assert(y < ny);
    ...
    CameraMatrix(
      LookingAt_, const Vector<T, 3>& eye, const Vector<T, 3>& target)
      : Base(oglplus::Nothing()) {
        assert(eye != target);
    ...
    static inline CameraMatrix LookingAt(
      const Vector<T, 3>& eye, const Vector<T, 3>& target) {
    ...
    CameraMatrix(
      ...
      : Base(oglplus::Nothing()) {
        ...
        Vector<T, 3> y = Normalized(dupz != 0.0 ? up - z * dupz : up);
        Vector<T, 3> x = Cross(y, z);
    ...
    CameraMatrix(
      ...
      Angle<T> elevation) {
        ...
        Vector<T, 3> x(-Sin(azimuth), T(0), -Cos(azimuth));
    ...
    static inline CameraMatrix Orbiting(
      ...
      Angle<T> azimuth,
      Angle<T> elevation) {
        return CameraMatrix(Orbiting_(), target, radius, azimuth, elevation);
    }
