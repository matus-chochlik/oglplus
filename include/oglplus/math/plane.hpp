...
public:
    ...
    static inline Plane FromTriangle(
      const Vector<T, 3>& p0, const Vector<T, 3>& p1, const Vector<T, 3>& p2) {
        return Plane(FromTriangle_(), p0, p1, p2);
    }
    ...
    Plane(
      ...
      const Vector<T, 3>& v2)
      : _equation(
          Normalized(Cross(v1, v2)), -Dot(Normalized(Cross(v1, v2)), p)) {}

    /// Constructs a plane defined by point p0 and vectors v1 and v2
    static inline Plane FromPointAndVectors(
      const Vector<T, 3>& p, const Vector<T, 3>& v1, const Vector<T, 3>& v2) {
        return Plane(FromPointAndVectors_(), p, v1, v2);
    }
