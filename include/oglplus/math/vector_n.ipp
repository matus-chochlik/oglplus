...
private:
    ...
    struct _op_negate {
        void operator()(Vector& t, const Vector& a) const {
    ...
    struct _op_add {
        void operator()(Vector& t, const Vector& a, const Vector& b) const {
            for(std::size_t i = 0; i != N; ++i)
    ...
    struct _op_mult_c {
        void operator()(Vector& t, const Vector& a, T v) const {
            for(std::size_t i = 0; i != N; ++i)
...
public:
    ...
    Vector(T v0, T v1, P... vn)
      : Base(oglplus::Nothing()) {
        static_assert(2 + sizeof...(P) == N, "Invalid number of arguments!");
    ...
    friend Vector<T, Cols> operator*(
      const Vector& v, const Matrix<T, N, Cols>& m);
