...
public:
    /// Finds the shader-storage block location, throws on failure if
    /// active_only
    ...
    static GLint GetLocation(
      ProgramName program, StrCRef identifier, bool active_only) {
        GLuint result = OGLPLUS_GLFUNC(GetProgramResourceIndex)(
          GetGLName(program), GL_SHADER_STORAGE_BLOCK, identifier.c_str());
        OGLPLUS_CHECK(
...
public:
    ...
    static GLuint MaxIn(ShaderType shader_type) {
        GLint result;
        OGLPLUS_GLFUNC(GetIntegerv)(_translate_max(shader_type), &result);
        OGLPLUS_VERIFY(
          GetIntegerv, Error, EnumParam(_translate_max(shader_type)));
        assert(result >= 0);
        return GLuint(result);
    }

    /// Returns true if this shader-storage block is referenced by @p
    /// shader_type
    /**
     *  @glsymbols
     ...
     *  @glfunref{GetActiveUniformBlock}
     */
    GLuint64 DataSize() const {
        GLint64 result;
        OGLPLUS_GLFUNC(GetInteger64i_v)
        (_block_index(), GL_SHADER_STORAGE_BUFFER_SIZE, &result);
        OGLPLUS_VERIFY(
          GetInteger64i_v,
          Error,
    ...
    void Binding(ShaderStorageBufferBindingPoint binding) {
        ...
        (this->_program, _block_index(), GLuint(binding));
        OGLPLUS_VERIFY(
          ShaderStorageBlockBinding,
          ProgVarError,
          Program(ProgramName(this->_program)).Index(GLuint(binding)));
    }
};
