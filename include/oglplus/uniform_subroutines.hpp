...
public:
     ...
     */
    GLint GetLocation(
      ProgramName program, StrCRef identifier, bool active_only) const {
        return GetLocation(program, this->_stage, identifier, active_only);
    }
...
private:
    ...
    static std::size_t _get_location_count(
      ProgramName program, ShaderType stage) {
        GLint result = 0;
        ...
        (GetGLName(program),
         ...
         &result);
        OGLPLUS_VERIFY(
          GetProgramStageiv, ObjectError, Object(program).EnumParam(stage));

        assert(!(result < 0));
        return std::size_t(result);
    }

    std::vector<GLuint>& _get_indices() {
        if(_indices.empty()) {
