...
public:
    ...
    void SetValues(std::size_t n, const T* values) {
        ...
        this->template _do_set_many<N>(
          this->_program, this->_location, GLsizei(n / N), values);
    }
...
public:
    void SetValue(const Matrix<T, R, C>& value) {
        this->template _do_set_mat<C, R>(
          this->_program, this->_location, 1, true, Data(value));
    }
    ...
    void SetValues(
      std::size_t n, const Matrix<T, R, C>* values, std::true_type) {
        const T* temp = reinterpret_cast<const T*>(values);
        SetValues(n * R * C, true, temp);
    }

    void SetValues(
      std::size_t n, const Matrix<T, R, C>* values, std::false_type) {
        std::vector<T> temp;
        temp.reserve(n * R * C);
        for(std::size_t i = 0; i != n; ++i) {
...
public:
    ...
    template <typename T>
    void SetValue(T value) {
        ProgVar<OpsTag, tag::Uniform, tag::NoTypecheck, T>(*this).SetValue(
          value);
    }

    template <typename T>
    void SetValues(std::size_t n, T values) {
        ProgVar<OpsTag, tag::Uniform, tag::NoTypecheck, T>(*this).SetValues(
          n, values);
    }
