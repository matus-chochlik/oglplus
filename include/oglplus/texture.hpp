...
protected:
	...
	static Boolean IsA(GLuint name) {
		Boolean result(OGLPLUS_GLFUNC(IsTexture)(name), std::nothrow);
...
public:
	...
	static void Bind(Target target, TextureName texture) {
		OGLPLUS_GLFUNC(BindTexture)(GLenum(target), GetGLName(texture));
		OGLPLUS_VERIFY(
		  BindTexture, ObjectError, Object(texture).BindTarget(target));
	}
  ...
  || GL_ARB_shader_image_load_store
	 ...
	 */
	static void BindImage(ImageUnitSelector unit,
	  TextureName texture,
	  GLint level,
	  Boolean layered,
	  GLint layer,
	  ...
	  ImageUnitFormat format) {
		...
		(GLuint(unit),
		  ...
		  GLenum(access),
		  GLenum(format));
		OGLPLUS_CHECK(BindImageTexture,
		  ObjectError,
		  Object(texture).EnumParam(format).Index(level));
	}
#endif

#if OGLPLUS_DOCUMENTATION_ONLY || GL_VERSION_4_4 || GL_ARB_multi_bind
	static void Bind(GLuint first, SizeType count, const GLuint* names) {
		OGLPLUS_GLFUNC(BindTextures)(first, count, names);
		OGLPLUS_VERIFY_SIMPLE(BindTextures);
	}

	static void BindImage(GLuint first, SizeType count, const GLuint* names) {
		OGLPLUS_GLFUNC(BindImageTextures)(first, count, names);
	...
	static void Bind(GLuint first, const Sequence<TextureName>& textures) {
		Bind(first, SizeType(textures.size()), GetGLNames(textures));
	}
	...
	static void BindImage(GLuint first, const Sequence<TextureName>& textures) {
		BindImage(first, SizeType(textures.size()), GetGLNames(textures));
	}
#endif
};
...
public:
	...
	static void Active(TextureUnitSelector index) {
		OGLPLUS_GLFUNC(ActiveTexture)(GLenum(GL_TEXTURE0 + GLuint(index)));
		OGLPLUS_VERIFY(ActiveTexture, Error, Index(GLuint(index)));
	}
	...
	static GLint Active(void) {
		...
		OGLPLUS_VERIFY(
		  GetIntegerv, Error, EnumParam(GLenum(GL_ACTIVE_TEXTURE)));
  ...
  || GL_ARB_shader_image_load_store
	 ...
	 *  @glfunref{BindImageTexture}
	 */
	void BindImage(ImageUnitSelector unit,
	  GLint level,
	  bool layered,
	  GLint layer,
	  AccessSpecifier access,
	  ImageUnitFormat format) const {
	...
	void InvalidateImage(GLint level) {
		...
		OGLPLUS_CHECK(
		  InvalidateTexImage, ObjectError, Object(*this).Index(level));
	}
	...
	void InvalidateSubImage(GLint level,
	  ...
	  SizeType depth) {
		...
		(_obj_name(), level, xoffs, yoffs, zoffs, width, height, depth);
		OGLPLUS_CHECK(
		  InvalidateTexSubImage, ObjectError, Object(*this).Index(level));
	}
#endif
	...
	void ClearImage(GLint level, PixelDataFormat format, const GLtype* data) {
		...
		(_obj_name(),
		  ...
		  data);
		OGLPLUS_CHECK(ClearTexImage, ObjectError, Object(*this).Index(level));
	}
	...
	void View(Target target,
	  ...
	  GLuint num_layers) {
		...
		(_obj_name(),
		  ...
		  num_layers);
		OGLPLUS_CHECK(
		  TextureView, ObjectError, Object(*this).BindTarget(target));
	}
#endif
};
...
public:
	...
	ObjZeroOps(ObjZeroOps&& temp) noexcept
	  : _base(static_cast<_base&&>(temp)) {
	}

	ObjZeroOps(const ObjZeroOps& that) noexcept
	  : _base(static_cast<const _base&>(that)) {
	}

	ObjZeroOps& operator=(ObjZeroOps&& temp) noexcept {
		_base::operator=(static_cast<_base&&>(temp));
		return *this;
	}

	ObjZeroOps& operator=(const ObjZeroOps& that) noexcept {
	...
	static SizeType Width(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_WIDTH), std::nothrow);
	}
	...
	static SizeType Height(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_HEIGHT), std::nothrow);
	}
	...
	static SizeType Depth(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_DEPTH), std::nothrow);
	}
	...
	static PixelDataType RedType(Target target, GLint level = 0) {
		return PixelDataType(GetIntParam(target, level, GL_TEXTURE_RED_TYPE));
	}
	...
	static PixelDataType GreenType(Target target, GLint level = 0) {
		return PixelDataType(GetIntParam(target, level, GL_TEXTURE_GREEN_TYPE));
	}
	...
	static PixelDataType BlueType(Target target, GLint level = 0) {
		return PixelDataType(GetIntParam(target, level, GL_TEXTURE_BLUE_TYPE));
	}
	...
	static PixelDataType AlphaType(Target target, GLint level = 0) {
		return PixelDataType(GetIntParam(target, level, GL_TEXTURE_ALPHA_TYPE));
	}
	...
	static PixelDataType DepthType(Target target, GLint level = 0) {
		return PixelDataType(GetIntParam(target, level, GL_TEXTURE_DEPTH_TYPE));
	}
	...
	static SizeType RedSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_RED_SIZE), std::nothrow);
	}
	...
	static SizeType GreenSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_GREEN_SIZE), std::nothrow);
	}
	...
	static SizeType BlueSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_BLUE_SIZE), std::nothrow);
	}
	...
	static SizeType AlphaSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_ALPHA_SIZE), std::nothrow);
	}
	...
	static SizeType DepthSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_DEPTH_SIZE), std::nothrow);
	}
	...
	static SizeType StencilSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_STENCIL_SIZE), std::nothrow);
	}
	...
	static SizeType SharedSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_SHARED_SIZE), std::nothrow);
	}
	...
	static SizeType CompressedImageSize(Target target, GLint level = 0) {
		return MakeSizeType(
		  GetIntParam(target, level, GL_TEXTURE_COMPRESSED_IMAGE_SIZE),
		  std::nothrow);
	}
	...
	static PixelDataInternalFormat InternalFormat(
	  Target target, GLint level = 0) {
		return PixelDataInternalFormat(
		  GetIntParam(target, level, GL_TEXTURE_INTERNAL_FORMAT));
	}
	 ...
	 */
	static void GetImage(Target target,
	  GLint level,
	  PixelDataFormat format,
	...
	static void GetCompressedImage(
	  Target target, GLint level, const OutputData& dest);
	 ...
	 */
	static void GetCompressedImage(
	  Target target, GLint level, SizeType size, GLubyte* buffer) {
	 ...
	 */
	static void GetCompressedImage(
	  Target target, GLint level, std::vector<GLubyte>& dest);
	 ...
	 *  @glfunref{TexImage3D}
	 */
	static void Image3D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	  ...
	  const void* data) {
		...
		(GLenum(target),
		  ...
		  GLenum(type),
		  data);
		OGLPLUS_CHECK(TexImage3D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format).Index(level));
	}
	 ...
	 */
	static void SubImage3D(Target target,
	  GLint level,
	  GLint xoffs,
	  GLint yoffs,
	  GLint zoffs,
	  ...
	  const void* data) {
		...
		(GLenum(target),
		  ...
		  GLenum(type),
		  data);
		OGLPLUS_CHECK(TexSubImage3D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(format).Index(level));
	}
	 ...
	 *  @glfunref{TexImage2D}
	 */
	static void Image2D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	 ...
	 */
	static void ImageCM(GLuint face,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	 ...
	 */
	static void SubImage2D(Target target,
	  GLint level,
	  GLint xoffs,
	  GLint yoffs,
	  SizeType width,
	 ...
	 *  @glfunref{TexImage1D}
	 */
	static void Image1D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  GLint border,
	 ...
	 */
	static void SubImage1D(Target target,
	  GLint level,
	  GLint xoffs,
	  SizeType width,
	  PixelDataFormat format,
	...
	static void SubImage1D(
	  Target target, const images::Image& image, GLint xoffs, GLint level = 0);
#endif // GL_VERSION_3_0
	 ...
	 *  @glfunref{TexImage1D}
	 */
	static void Image(Target target,
	  const images::Image& image,
	  GLint level = 0,
	  GLint border = 0);
	 ...
	 */
	static void CopyImage2D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  GLint x,
	  GLint y,
	 ...
	 */
	static void CopyImage1D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  GLint x,
	  GLint y,
	  ...
	  GLint border) {
		...
		(GLenum(target), level, GLenum(internal_format), x, y, width, border);
		OGLPLUS_CHECK(CopyTexImage1D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format).Index(level));
	}
#endif // GL_VERSION_3_0
	 ...
	 */
	static void CopySubImage3D(Target target,
	  GLint level,
	  GLint xoffs,
	  GLint yoffs,
	  GLint zoffs,
	  ...
	  SizeType height) {
		...
		(GLenum(target), level, xoffs, yoffs, zoffs, x, y, width, height);
		OGLPLUS_CHECK(
		  CopyTexSubImage3D, ObjectError, ObjectBinding(target).Index(level));
	}
	 ...
	 */
	static void CopySubImage2D(Target target,
	  GLint level,
	  GLint xoffs,
	  GLint yoffs,
	  GLint x,
	  ...
	  SizeType height) {
		...
		(GLenum(target), level, xoffs, yoffs, x, y, width, height);
		OGLPLUS_CHECK(
		  CopyTexSubImage2D, ObjectError, ObjectBinding(target).Index(level));
	}
	 ...
	 */
	static void CopySubImage1D(Target target,
	  GLint level,
	  GLint xoffs,
	  GLint x,
	  GLint y,
	  SizeType width) {
		...
		(GLenum(target), level, xoffs, x, y, width);
		OGLPLUS_CHECK(
		  CopyTexSubImage1D, ObjectError, ObjectBinding(target).Index(level));
	}
#endif // GL_VERSION_3_0
	 ...
	 */
	static void CompressedImage3D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	  ...
	  const void* data) {
		...
		(GLenum(target),
		  ...
		  image_size,
		  data);
		OGLPLUS_CHECK(CompressedTexImage3D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format).Index(level));
	}
	 ...
	 */
	static void CompressedImage2D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	  ...
	  const void* data) {
		...
		(GLenum(target),
		  ...
		  image_size,
		  data);
		OGLPLUS_CHECK(CompressedTexImage2D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format).Index(level));
	}
	 ...
	 */
	static void CompressedImage1D(Target target,
	  GLint level,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  GLint border,
	  ...
	  const void* data) {
		...
		(GLenum(target),
		  ...
		  image_size,
		  data);
		OGLPLUS_CHECK(CompressedTexImage1D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format).Index(level));
	}
#endif // GL_VERSION_3_0
	 ...
	 */
	static void CompressedSubImage3D(Target target,
	  GLint level,
	  GLint xoffs,
	  GLint yoffs,
	  GLint zoffs,
	  ...
	  const void* data) {
		...
		(GLenum(target),
		  ...
		  image_size,
		  data);
		OGLPLUS_CHECK(CompressedTexSubImage3D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(format).Index(level));
	}
	 ...
	 */
	static void CompressedSubImage2D(Target target,
	  GLint level,
	  GLint xoffs,
	  GLint yoffs,
	  SizeType width,
	  ...
	  const void* data) {
		...
		(GLenum(target),
		  ...
		  image_size,
		  data);
		OGLPLUS_CHECK(CompressedTexSubImage2D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(format).Index(level));
	}
	 ...
	 */
	static void CompressedSubImage1D(Target target,
	  GLint level,
	  GLint xoffs,
	  SizeType width,
	  PixelDataFormat format,
	  ...
	  const void* data) {
		...
		(GLenum(target), level, xoffs, width, GLenum(format), image_size, data);
		OGLPLUS_CHECK(CompressedTexSubImage1D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(format).Index(level));
	}
#endif
	 ...
	 */
	static void Image3DMultisample(Target target,
	  SizeType samples,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	  ...
	  Boolean fixed_sample_locations) {
		...
		OGLPLUS_CHECK(TexImage3DMultisample,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format));
	}
	 ...
	 */
	static void Image2DMultisample(Target target,
	  SizeType samples,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	  Boolean fixed_sample_locations) {
		...
		(GLenum(target),
		  ...
		  height,
		  fixed_sample_locations._get());
		OGLPLUS_CHECK(TexImage2DMultisample,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format));
	}
	 ...
	 *  @glfunref{TexBuffer}
	 */
	static void Buffer(Target target,
	  PixelDataInternalFormat internal_format,
	  BufferName buffer) {
		OGLPLUS_GLFUNC(TexBuffer)
		(GLenum(target), GLenum(internal_format), GetGLName(buffer));
		OGLPLUS_CHECK(TexBuffer,
		  ObjectPairError,
		  Subject(buffer).ObjectBinding(target).EnumParam(internal_format));
	}
#endif
	 ...
	 */
	static void BufferRange(Target target,
	  PixelDataInternalFormat internal_format,
	  BufferName buffer,
	  GLintptr offset,
	  BigSizeType size) {
	 ...
	 */
	static void Storage1D(Target target,
	  SizeType levels,
	  PixelDataInternalFormat internal_format,
	  SizeType width) {
		OGLPLUS_GLFUNC(TexStorage1D)
		(GLenum(target), levels, GLenum(internal_format), width);
		OGLPLUS_CHECK(TexStorage1D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format));
	}
	 ...
	 */
	static void Storage2D(Target target,
	  SizeType levels,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height) {
		OGLPLUS_GLFUNC(TexStorage2D)
		(GLenum(target), levels, GLenum(internal_format), width, height);
		OGLPLUS_CHECK(TexStorage2D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format));
	}
	 ...
	 */
	static void Storage3D(Target target,
	  SizeType levels,
	  PixelDataInternalFormat internal_format,
	  SizeType width,
	  SizeType height,
	  SizeType depth) {
		...
		(GLenum(target), levels, GLenum(internal_format), width, height, depth);
		OGLPLUS_CHECK(TexStorage3D,
		  ObjectError,
		  ObjectBinding(target).EnumParam(internal_format));
	}
#endif
	...
	static void BaseLevel(Target target, GLint level) {
		...
		(GLenum(target), GL_TEXTURE_BASE_LEVEL, level);
		OGLPLUS_CHECK(
		  TexParameteri, ObjectError, ObjectBinding(target).Index(level));
	}
	...
	static Vector<GLfloat, 4> BorderColor(Target target, TypeTag<GLfloat>) {
		...
		(GLenum(target), GL_TEXTURE_BORDER_COLOR, result);
		OGLPLUS_CHECK(GetTexParameterfv, ObjectError, ObjectBinding(target));
	...
	static void BorderColor(Target target, Vector<GLfloat, 4> color) {
		...
		(GLenum(target), GL_TEXTURE_BORDER_COLOR, Data(color));
		OGLPLUS_CHECK(TexParameterfv, ObjectError, ObjectBinding(target));
	}
	...
	static Vector<GLint, 4> BorderColor(Target target, TypeTag<GLint>) {
		...
		(GLenum(target), GL_TEXTURE_BORDER_COLOR, result);
		OGLPLUS_CHECK(GetTexParameterIiv, ObjectError, ObjectBinding(target));
	...
	static void BorderColor(Target target, Vector<GLint, 4> color) {
		...
		(GLenum(target), GL_TEXTURE_BORDER_COLOR, Data(color));
		OGLPLUS_CHECK(TexParameterIiv, ObjectError, ObjectBinding(target));
	}
	...
	static Vector<GLuint, 4> BorderColor(Target target, TypeTag<GLuint>) {
		...
		(GLenum(target), GL_TEXTURE_BORDER_COLOR, result);
		OGLPLUS_CHECK(GetTexParameterIuiv, ObjectError, ObjectBinding(target));
	...
	static void BorderColor(Target target, Vector<GLuint, 4> color) {
		...
		(GLenum(target), GL_TEXTURE_BORDER_COLOR, Data(color));
		OGLPLUS_CHECK(TexParameterIuiv, ObjectError, ObjectBinding(target));
	}
#endif // GL_VERSION_3_0
	...
	static TextureCompareMode CompareMode(Target target) {
		return TextureCompareMode(GetIntParam(target, GL_TEXTURE_COMPARE_MODE));
	}
	...
	static void CompareMode(Target target, TextureCompareMode mode) {
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GL_TEXTURE_COMPARE_MODE, GLint(mode));
		OGLPLUS_CHECK(
		  TexParameteri, ObjectError, ObjectBinding(target).EnumParam(mode));
	}
	...
	static CompareFunction CompareFunc(Target target) {
		return CompareFunction(GetIntParam(target, GL_TEXTURE_COMPARE_FUNC));
	}
	...
	static void CompareFunc(Target target, CompareFunction func) {
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GL_TEXTURE_COMPARE_FUNC, GLint(func));
		OGLPLUS_CHECK(
		  TexParameteri, ObjectError, ObjectBinding(target).EnumParam(func));
	}
	...
	static void LODBias(Target target, GLfloat value) {
		...
		(GLenum(target), GL_TEXTURE_LOD_BIAS, value);
		OGLPLUS_CHECK(TexParameterf, ObjectError, ObjectBinding(target));
	}
#endif // GL_VERSION_3_0
	...
	static void Filter(Target target, TextureFilter filter) {
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GL_TEXTURE_MIN_FILTER, GLint(filter));
		OGLPLUS_CHECK(
		  TexParameteri, ObjectError, ObjectBinding(target).EnumParam(filter));
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GL_TEXTURE_MAG_FILTER, GLint(filter));
		OGLPLUS_CHECK(
		  TexParameteri, ObjectError, ObjectBinding(target).EnumParam(filter));
	}
	...
	static TextureMagFilter MagFilter(Target target) {
		return TextureMagFilter(GetIntParam(target, GL_TEXTURE_MAG_FILTER));
	}
	...
	static TextureMinFilter MinFilter(Target target) {
		return TextureMinFilter(GetIntParam(target, GL_TEXTURE_MIN_FILTER));
	}
	...
	static void MinLOD(Target target, GLfloat value) {
		OGLPLUS_GLFUNC(TexParameterf)
		(GLenum(target), GL_TEXTURE_MIN_LOD, value);
		OGLPLUS_CHECK(TexParameterf, ObjectError, ObjectBinding(target));
	}
	...
	static void MaxLOD(Target target, GLfloat value) {
		OGLPLUS_GLFUNC(TexParameterf)
		(GLenum(target), GL_TEXTURE_MAX_LOD, value);
		OGLPLUS_CHECK(TexParameterf, ObjectError, ObjectBinding(target));
	}
	...
	static void MaxLevel(Target target, GLint value) {
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GL_TEXTURE_MAX_LEVEL, value);
		OGLPLUS_CHECK(TexParameteri, ObjectError, ObjectBinding(target));
	}
	...
	static GLfloat MaxAnisotropy(Target target) {
		...
		return GetFloatParam(target, GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT);
#else
	...
	static GLfloat Anisotropy(Target target) {
		...
		return GetFloatParam(target, GL_TEXTURE_MAX_ANISOTROPY_EXT);
#else
		OGLPLUS_FAKE_USE(target);
		return GLfloat(1);
#endif
...
#ifdef GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT
		OGLPLUS_GLFUNC(TexParameterf)
		(GLenum(target), GL_TEXTURE_MAX_ANISOTROPY_EXT, value);
		OGLPLUS_CHECK(TexParameterf, ObjectError, ObjectBinding(target));
#else
		OGLPLUS_FAKE_USE(target);
	...
	static TextureSwizzle Swizzle(Target target, TextureSwizzleCoord coord) {
		return TextureSwizzle(GetIntParam(target, GLenum(coord)));
	}
	 ...
	 */
	static void Swizzle(
	  Target target, TextureSwizzleCoord coord, TextureSwizzle mode) {
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GLenum(coord), GLint(mode));
		OGLPLUS_CHECK(
		  TexParameteri, ObjectError, ObjectBinding(target).EnumParam(mode));
	}
	...
	static TextureSwizzleTuple SwizzleRGBA(Target target) {
		...
		(GLenum(target), GL_TEXTURE_SWIZZLE_RGBA, result.Values());
		OGLPLUS_CHECK(GetTexParameteriv, ObjectError, ObjectBinding(target));
		return result;
	}
	...
	static void SwizzleRGBA(Target target, TextureSwizzle mode) {
		...
		(GLenum(target), GL_TEXTURE_SWIZZLE_RGBA, params);
		OGLPLUS_CHECK(
		  TexParameteriv, ObjectError, ObjectBinding(target).EnumParam(mode));
	}
	...
	static void SwizzleRGBA(Target target,
	  ...
	  TextureSwizzle mode_a) {
		...
		OGLPLUS_GLFUNC(TexParameteriv)
		(GLenum(target), GL_TEXTURE_SWIZZLE_RGBA, params);
		OGLPLUS_CHECK(TexParameteriv, ObjectError, ObjectBinding(target));
	}
	...
	static void SwizzleRGBA(Target target, const TextureSwizzleTuple& modes) {
		OGLPLUS_GLFUNC(TexParameteriv)
		(GLenum(target), GL_TEXTURE_SWIZZLE_RGBA, modes.Values());
		OGLPLUS_CHECK(TexParameteriv, ObjectError, ObjectBinding(target));
	}
	...
	static TextureWrap Wrap(Target target, TextureWrapCoord coord) {
		return TextureWrap(GetIntParam(target, GLenum(coord)));
	}
	...
	static void Wrap(Target target, TextureWrap mode) {
		switch(TextureTargetDimensions(target)) {
			...
			case 0:
				break;
			default:
				OGLPLUS_ABORT("Invalid texture wrap dimension");
		}
	}
	...
	static PixelDataFormat DepthStencilMode(Target target) {
		return PixelDataFormat(
		  GetIntParam(target, GL_DEPTH_STENCIL_TEXTURE_MODE));
	}
	...
	static void DepthStencilMode(Target target, PixelDataFormat mode) {
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GL_DEPTH_STENCIL_TEXTURE_MODE, GLint(mode));
		OGLPLUS_CHECK(
		  TexParameteri, ObjectError, ObjectBinding(target).EnumParam(mode));
	}
#endif
	...
	static Boolean Seamless(Target target) {
		return Boolean(
		  GetIntParam(target, GL_TEXTURE_CUBE_MAP_SEAMLESS), std::nothrow);
	}
	...
	static void Seamless(Target target, Boolean enable) {
		OGLPLUS_GLFUNC(TexParameteri)
		(GLenum(target), GL_TEXTURE_CUBE_MAP_SEAMLESS, enable._get());
		OGLPLUS_CHECK(TexParameteri, ObjectError, ObjectBinding(target));
	}
#endif

#if OGLPLUS_DOCUMENTATION_ONLY || GL_VERSION_4_5 || GL_ARB_texture_barrier \
  || GL_NV_texture_barrier
	...
	static void GenerateMipmap(Target target) {
		...
		OGLPLUS_CHECK(GenerateMipmap, ObjectError, ObjectBinding(target));
	}
};
...
inline TextureUnitAndTarget operator|(
  TextureUnitSelector unit, TextureTarget tex) {
	return TextureUnitAndTarget(unit, tex);
}

// Bind
inline TextureTarget operator<<(
  const DefaultTextureOps& tex, TextureTarget target) {
	tex.Bind(target);
	return target;
}
...
inline TextureTargetAndSlot operator<<(
  TextureTargetAndSlot tas, TextureWrap wrap) {
	switch(tas.slot) {
		case 0:
			DefaultTextureOps::WrapS(tas.target, wrap);
			break;
		case 1:
			DefaultTextureOps::WrapT(tas.target, wrap);
			break;
		case 2:
			DefaultTextureOps::WrapR(tas.target, wrap);
			break;
		default:
...
// Swizzle
inline TextureTargetAndSlot operator<<(
  TextureTargetAndSlot tas, TextureSwizzle swizzle) {
	switch(tas.slot) {
		case 0:
			DefaultTextureOps::SwizzleR(tas.target, swizzle);
			break;
		case 1:
			DefaultTextureOps::SwizzleG(tas.target, swizzle);
			break;
		case 2:
			DefaultTextureOps::SwizzleB(tas.target, swizzle);
			break;
		case 3:
			DefaultTextureOps::SwizzleA(tas.target, swizzle);
			break;
		default:
			OGLPLUS_ABORT("Invalid texture swizzle slot");
	}
	return tas;
}
#endif
...
// Image
inline TextureTarget operator<<(
  TextureTarget target, const images::Image& image) {
...
// Image
inline TextureTarget operator<<(
  TextureTarget target, const images::ImageSpec& image_spec) {
...
// Image + Level
inline TextureTarget operator<<(
  TextureTargetAndSlot tas, const images::Image& image) {
...
// Image + Level
inline TextureTarget operator<<(
  TextureTargetAndSlot tas, const images::ImageSpec& image_spec) {
	DefaultTextureOps::Image(tas.target, image_spec, tas.slot);
	return tas.target;
}

// GenerateMipmap
inline TextureTarget operator<<(TextureTarget target, TextureMipmap) {
