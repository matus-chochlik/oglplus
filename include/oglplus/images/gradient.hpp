...
private:
    ...
    static void _apply_gradient(
      const std::vector<Vector<T, N>>& grad0, GLubyte* dp, GLubyte* de) {
...
public:
    struct AddComponents {
        template <typename T, std::size_t N>
        Vector<T, N> operator()(
          const Vector<T, N>& a, const Vector<T, N>& b) const {
            return a + b;
        }
    ...
    LinearGradient(
      ...
      : Image(width, 1, 1, N, &TypeTag<GLubyte>()) {
        ...
        _make_gradient(background, width, x_points, x_gradient);
    ...
    LinearGradient(
      ...
      : Image(width, height, 1, N, &TypeTag<GLubyte>()) {
        ...
        _make_gradient(background, height, y_points, y_gradient);

        std::vector<Vector<T, N>> x_gradient((std::size_t(width)));
        _make_gradient(background, width, x_points, x_gradient);

        _apply_gradient(
          combine, y_gradient, x_gradient, _begin_ub(), _end_ub());
    }
    ...
    LinearGradient(
      ...
      : Image(width, height, depth, N, &TypeTag<GLubyte>()) {
        ...
        _make_gradient(background, depth, z_points, z_gradient);
        ...
        _apply_gradient(
          combine, z_gradient, y_gradient, x_gradient, _begin_ub(), _end_ub());
    }
};
