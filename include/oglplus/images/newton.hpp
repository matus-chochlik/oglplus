      ...
      Vector<float, N> c2) {
        ...
        for(GLsizei i = 0; i != width; ++i)
            for(GLsizei j = 0; j != height; ++j) {
                ...
                for(n = 0; n != max; ++n) {
                    Vec2f zn = z - _cdiv(Function::f(z), Function::df(z));
                    if(Distance(zn, z) < 0.00001f)
                        break;
                    z = zn;
                }
                Vector<float, N> c =
                  _mix(c1, c2, float(mixer(float(n) / float(max - 1))));
                for(n = 0; n != N; ++n) {
...
public:
    ...
    struct X3Minus1 {
        ...
        static Vec2f df(Vec2f n) {
            return 3.0f *
                   Vec2f(n.x() * n.x() - n.y() * n.y(), 2.0f * n.x() * n.y());
        }
    };
    ...
    struct X4Minus1 {
        static Vec2f f(Vec2f n) {
            return Vec2f(
              n.x() * n.x() * n.x() * n.x() + n.y() * n.y() * n.y() * n.y() -
    ...
    template <typename Function, typename Mixer>
    NewtonFractal(SizeType width, SizeType height, Function func, Mixer mixer)
