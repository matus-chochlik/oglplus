...
private:
	...
	template <typename T>
	static double _do_convert(void* ptr) noexcept {
		assert(ptr != nullptr);
	...
	bool _is_initialized(void) const noexcept;

	static PixelDataFormat _get_def_pdf(unsigned N) noexcept;

	static PixelDataInternalFormat _get_def_pdif(unsigned N) noexcept;
...
protected:
	...
	template <typename T>
	static T _one(TypeTag<T>) noexcept {
		return std::numeric_limits<T>::max();
	}

	static float _one(TypeTag<float>) noexcept {
		return 1.0f;
	}

	static double _one(TypeTag<double>) noexcept {
		return 1.0;
	}

	template <typename T>
	bool _type_ok(void) const noexcept {
		return _type == PixelDataType(GetDataType<T>());
	}

	template <typename T>
	T* _begin(void) noexcept {
		assert(_is_initialized());
		assert(_type_ok<T>());
		return static_cast<T*>(_storage.begin());
	}

	unsigned char* _begin_ub(void) noexcept {
		return _begin<unsigned char>();
	}

	template <typename T>
	T* _end(void) noexcept {
		assert(_is_initialized());
		assert(_type_ok<T>());
		return static_cast<T*>(_storage.end());
	}

	unsigned char* _end_ub(void) noexcept {
...
public:
	...
	Image(SizeType width,
	  ...
	  , _storage(oglplus::aux::AlignedPODArray(
		  data, std::size_t(_width * _height * _depth * _channels)))
	 ...
	 *  3: Channels
	 */
	SizeType Dimension(std::size_t i) const noexcept {
		if(i == 0)
			return Width();
		if(i == 1)
			return Height();
	...
	SizeType Width(void) const noexcept {
		return MakeSizeType(_width, std::nothrow);
	}

	/// Returns the height of the image
	SizeType Height(void) const noexcept {
		return MakeSizeType(_height, std::nothrow);
	}

	/// Returns the depth of the image
	SizeType Depth(void) const noexcept {
		return MakeSizeType(_depth, std::nothrow);
	}

	/// Returns the number of channels
	SizeType Channels(void) const noexcept {
		return MakeSizeType(_channels, std::nothrow);
	}

	/// Returns the pixel data type
	PixelDataType Type(void) const noexcept {
		assert(_is_initialized());
		return _type;
	}

	/// Return the pixel data format
	PixelDataFormat Format(void) const noexcept {
		assert(_is_initialized());
		return _format;
	}

	/// Return a suitable pixel data internal format
	PixelDataInternalFormat InternalFormat(void) const noexcept {
		assert(_is_initialized());
		return _internal;
	}
	...
	const void* RawData(void) const noexcept {
		assert(_is_initialized());
		return _storage.begin();
	}

	/// Returns the size of data in bytes
	std::size_t DataSize(void) const noexcept {
		assert(_is_initialized());
		return _storage.size();
	}

	std::size_t PixelPos(
	  SizeType width, SizeType height, SizeType depth) const {
		assert(_is_initialized());
		assert(width >= 0);
		assert(height >= 0);
		assert(depth >= 0);
	...
	std::size_t ComponentPos(SizeType width,
	  SizeType height,
	  SizeType depth,
	...
	double Component(SizeType width,
	  SizeType height,
	  SizeType depth,
	  SizeType component) const {
		...
		assert(_convert);
		return _convert(
		  _storage.at(ComponentPos(width, height, depth, component)));
	}

	/// Returns the component of the pixel at the specified coordinates
	template <typename T>
	T ComponentAs(SizeType width,
	  SizeType height,
	  SizeType depth,
	  SizeType component) const {
		assert(_type_ok<T>());
