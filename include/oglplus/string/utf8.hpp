...
void ConvertCodePointsToUTF8(
  const UnicodeCP* cps, std::size_t len, std::vector<char>& result);

std::size_t CodePointsRequired(const char* str, std::size_t len);

UnicodeCP ConvertUTF8ToCodePoint(
  const char* str, std::size_t len, std::size_t& cp_len);

void ConvertUTF8ToCodePoints(
  const char* str, std::size_t len, std::vector<UnicodeCP>& result);
