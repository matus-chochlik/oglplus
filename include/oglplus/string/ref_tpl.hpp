...
private:
	...
	static const Char* _ecs(void) noexcept {
		return "";
	}

	void _validate(void) const noexcept {
...
public:
	...
	StrCRefTpl(const Char* cstr, std::size_t ssize) noexcept
	  : _c_str(cstr)
	  , _size(ssize) {
		_validate();
	}
	...
	StrCRefTpl(const Char (&cary)[N]) noexcept
	  : _c_str(cary)
	  , _size(N) {
		_validate();
	}
	...
	StrCRefTpl(const std::basic_string<Char>& sstr) noexcept
	  : _c_str(sstr.c_str())
	  , _size(sstr.size()) {
		_validate();
	}
	...
	StrCRefTpl(const std::vector<Char>& cvec) noexcept
	  : _c_str(cvec.data())
	  , _size(cvec.size()) {
		_validate();
	}
	...
	template <std::size_t N>
	StrCRefTpl(const std::array<Char, N>& cvec) noexcept
	  : _c_str(cvec.data())
	  , _size(cvec.size()) {
		_validate();
	}
...
template <typename Char>
inline StrCRefChainTpl<Char, StrCRefTpl<Char>, StrCRefTpl<Char>> operator+(
  StrCRefTpl<Char> left, StrCRefTpl<Char> right) {
	return StrCRefChainTpl<Char, StrCRefTpl<Char>, StrCRefTpl<Char>>(
	  left, right);
}

template <typename Char, typename Left, typename Right>
inline StrCRefChainTpl<Char,
  StrCRefChainTpl<Char, Left, Right>,
  StrCRefTpl<Char>>
operator+(StrCRefChainTpl<Char, Left, Right> left, StrCRefTpl<Char> right) {
	return StrCRefChainTpl<Char,
	  StrCRefChainTpl<Char, Left, Right>,
	  StrCRefTpl<Char>>(left, right);
}
