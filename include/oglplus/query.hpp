...
protected:
    ...
    static Boolean IsA(GLuint name) {
        Boolean result(OGLPLUS_GLFUNC(IsQuery)(name), std::nothrow);
...
public:
    ...
    void Begin(Target target) {
        ...
        OGLPLUS_VERIFY(
          BeginQuery, ObjectError, Object(*this).EnumParam(target));
    }
    ...
    void BeginConditionalRender(ConditionalRenderMode mode) {
        ...
        OGLPLUS_VERIFY(
          BeginConditionalRender, ObjectError, Object(*this).EnumParam(mode));
    }
  ...
  (GL_VERSION_3_1 && GL_ARB_timer_query)
    ...
    void Counter(Target target) {
        ...
        OGLPLUS_VERIFY(
          QueryCounter, ObjectError, Object(*this).EnumParam(target));
    }
    ...
    void Result(GLuint& result) const {
        OGLPLUS_GLFUNC(GetQueryObjectuiv)
        (_obj_name(), GL_QUERY_RESULT, &result);
  ...
  (GL_VERSION_3_1 && GL_ARB_timer_query)
    ...
    void Result(GLint64& result) const {
        ...
        (_obj_name(), GL_QUERY_RESULT, &result);
        OGLPLUS_VERIFY(GetQueryObjecti64v, ObjectError, Object(*this));
    }
    ...
    void Result(GLuint64& result) const {
        ...
        (_obj_name(), GL_QUERY_RESULT, &result);
        OGLPLUS_VERIFY(GetQueryObjectui64v, ObjectError, Object(*this));
    }
#endif // timer query
...
inline QueryActivator ObjectOps<tag::DirectState, tag::Query>::Activate(
  Target target) {
    return QueryActivator(*this, target);
}

template <typename ResultType>
inline QueryExecution<ResultType>
ObjectOps<tag::DirectState, tag::Query>::Execute(
  QueryTarget target, ResultType& result) {
