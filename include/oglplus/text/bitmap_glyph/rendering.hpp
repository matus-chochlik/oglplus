...
protected:
    ...
    friend void BitmapGlyphAllocateLayoutData(
      BitmapGlyphRenderingBase& that, BitmapGlyphLayoutData& layout_data);
...
inline void BitmapGlyphAllocateLayoutData(
  BitmapGlyphRenderingBase& that, BitmapGlyphLayoutData& layout_data) {
    auto i = that._layout_storage.begin(), e = that._layout_storage.end();
    while(i != e) {
        if(i->Allocate(layout_data))
            return;
        ++i;
    }
    that._layout_storage.emplace_back(
      that, that._config.layout_storage_page, that._config.layout_storage_unit);
...
inline void BitmapGlyphInitializeLayoutData(
  ...
  SizeType length) {
    ...
    _storage.Initialize(layout_data, width, x_offsets, cps, length);
}
...
class BitmapGlyphRenderingTpl : public BitmapGlyphRenderingBase {
public:
