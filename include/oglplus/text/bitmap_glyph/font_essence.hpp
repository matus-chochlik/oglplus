...
private:
    ...
    void _do_load_pages(
      PageGetter get_page, const Element* elem, GLsizei size) {
        _page_storage.Bind();
        ...
        for(GLsizei i = 0; i != size; ++i) {
            ...
            if(!_pager.UsePage(page)) {
                ...
                _page_storage.LoadPage(
                  frame, _load_page_bitmap(page), _load_page_metric(page));
...
public:
    BitmapGlyphFontEssence(
      ...
      , _pager(parent, pg_map_tex_unit, frames)
    ...
    TextureUnitSelector BitmapTexUnit() const {
        return _page_storage.BitmapTexUnit();
    }

    TextureUnitSelector MetricTexUnit() const {
        return _page_storage.MetricTexUnit();
    }

    TextureUnitSelector PageMapTexUnit() const {
    ...
    GLfloat QueryXOffsets(
      const CodePoint* cps,
      SizeType size,
      std::vector<GLfloat>& x_offsets) const;
