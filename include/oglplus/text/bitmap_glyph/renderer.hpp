...
private:
    ...
    ProgramUniform<GLint> _bitmap_sampler, _metric_sampler, _pg_map_sampler;
...
protected:
    const Program& _get_program() const {
        return _program;
    }
...
public:
     ...
     */
    BitmapGlyphRenderer(
      BitmapGlyphRenderingBase& parent, const Sequence<ShaderName>& shaders);
    ...
    void Render(const BitmapGlyphLayoutTpl<BitmapFont>& layout) {
        ...
        layout._font._essence->LoadPages(
          layout._pages.data(), layout._pages.size());
        ...
        Context gl;
        gl.DrawArrays(
          PrimitiveType::Points, layout._data._offset, layout._data._length);
    }
};
