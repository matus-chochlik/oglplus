...
private:
    ...
    static aux::GLSLSourceWrapper* make_impl(P1&& p1, P2&& p2) {
        return new Impl(std::forward<P1>(p1), std::forward<P2>(p2));
    }
...
public:
    ...
    GLSLSource(const std::vector<StrCRef>& lits)
      : _impl(make_impl<aux::StrCRefsGLSLSrcWrap>(lits.begin(), lits.end())) {
    }

    template <size_t N>
    GLSLSource(const StrCRef (&lits)[N])
      : _impl(make_impl<aux::StrCRefsGLSLSrcWrap>(lits, lits + N)) {
    }

    GLSLSource(const std::vector<String>& strs)
      : _impl(make_impl<aux::StrsGLSLSrcWrap>(strs.begin(), strs.end())) {
    }
