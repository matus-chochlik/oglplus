...
private:
    ...
    bool _initialize() {
        bool result = EGLPLUS_EGLFUNC(Initialize)(
                        GetEGLHandle(_display), &_major, &_minor) == EGL_TRUE;
...
public:
    ...
    const Display& AssociatedDisplay() const {
        return _display;
    }
    ...
    const char* QueryString(StringQuery query) const {
        assert(_initialized);
        const char* result =
          EGLPLUS_EGLFUNC(QueryString)(GetEGLHandle(_display), EGLint(query));
     ...
     */
    Range<String> ClientAPIs() const;
#else
    aux::SepStrRange ClientAPIs() const {
     ...
     */
    Range<String> Extensions() const;
#else
    aux::SepStrRange Extensions() const {
    ...
    static bool HasClientExtensions() {
        const char* ext_str =
          EGLPLUS_EGLFUNC(QueryString)(EGL_NO_DISPLAY, EGLint(EGL_EXTENSIONS));
     ...
     *  @egldefref{EGL_EXTENSIONS}
     */
    static Range<String> ClientExtensions();
#else
    static aux::SepStrRange ClientExtensions() {
        const char* ext_str =
          EGLPLUS_EGLFUNC(QueryString)(EGL_NO_DISPLAY, EGLint(EGL_EXTENSIONS));
        EGLPLUS_VERIFY_SIMPLE(QueryString);
     ...
     */
    static Boolean ReleaseThread() {
        Boolean result(EGLPLUS_EGLFUNC(ReleaseThread)(), std::nothrow);
