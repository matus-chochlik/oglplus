...
private:
	...
	{
		oglplus::aux::UTF8Validator<const char*> valid_utf8;
...
public:
	...
	explicit StrLit(const char* literal);
#else
	template <std::size_t N>
	explicit StrLit(const char (&lit)[N])
	EGLPLUS_NOEXCEPT(true)
	 : _lit(lit)
	 , _size(N-1)
	{ _check(); }
	...
	explicit StrLit(const char* lit, std::size_t lit_size)
	EGLPLUS_NOEXCEPT(true)
	 : _lit(lit)
	 , _size(lit_size)
	{
