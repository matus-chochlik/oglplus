...
struct ContextValueTypeToContextAttrib {
    static std::integral_constant<int, -1> MaxValueType();
};
...
protected:
    ...
    ContextOps(::ALCdevice* device, ::ALCcontext* context)
      : _device(device)
      , _context(context) {
        assert(_device);
        assert(_context);
...
public:
    ...
    static const char* Vendor() {
        return reinterpret_cast<const char*>(GetString(StringQuery::Vendor));
    }
    ...
    static const char* Version() {
        return reinterpret_cast<const char*>(GetString(StringQuery::Version));
    }
    ...
    static const char* Renderer() {
        return reinterpret_cast<const char*>(GetString(StringQuery::Renderer));
    }
    ...
    static aux::SepStrRange Extensions() {
        return aux::SepStrRange(
          reinterpret_cast<const char*>(GetString(StringQuery::Extensions)));
    }
#endif
     ...
     */
    static oalplus::DistanceModel DistanceModel() {
        ALint result;
        OALPLUS_ALFUNC(GetIntegerv(AL_DISTANCE_MODEL, &result));
    ...
    static ALfloat DopplerFactor() {
        ALfloat result;
        OALPLUS_ALFUNC(GetFloatv(AL_DOPPLER_FACTOR, &result));
    ...
    static ALfloat SpeedOfSound() {
        ALfloat result;
        OALPLUS_ALFUNC(GetFloatv(AL_SPEED_OF_SOUND, &result));
        OALPLUS_VERIFY_SIMPLE(GetFloatv);
        return result;
    }
};
...
public:
    ...
    Context(const Device& device, const FinishedContextAttribs& attribs)
      : ContextOps(
          device._device,
          OALPLUS_ALCFUNC(CreateContext)(device._device, attribs.Get())) {
        OALPLUS_CHECK_SIMPLE_ALC(_device, CreateContext);
    }
...
class ContextMadeCurrent : public Context {
public:
     ...
     */
    ContextMadeCurrent(
      const Device& device, const FinishedContextAttribs& attribs)
