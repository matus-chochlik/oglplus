...
eglplus_egl_glx_SurfaceImpl::eglplus_egl_glx_SurfaceImpl(
  int drawable_type, ::GLXDrawable drawable)
...
void eglplus_egl_glx_SurfaceImpl::_cleanup(::Display* display) {
    if(_do_cleanup)
        _do_cleanup(display, _glx_drawable);
...
bool eglplus_egl_valid_surface(EGLSurface surface) {
    ::pthread_mutex_lock(&eglplus_egl_Surfaces_mutex);
    bool result =
      eglplus_egl_glx_Surfaces.find(surface) != eglplus_egl_glx_Surfaces.end();
    ::pthread_mutex_unlock(&eglplus_egl_Surfaces_mutex);
...
EGLAPI EGLSurface EGLAPIENTRY eglCreateWindowSurface(
  ...
  const EGLint* egl_attrib_list) {
    ...
    if(
      ::XGetWindowAttributes(display->_x_open_display, window, &win_attr) ==
      0) {
        return EGL_NO_SURFACE;
    }

    try {
        return new eglplus_egl_glx_SurfaceImpl(GLX_WINDOW_BIT, window);
    } catch(...) {
...
EGLAPI EGLSurface EGLAPIENTRY eglCreatePbufferSurface(
  EGLDisplay display, EGLConfig config, const EGLint* egl_attrib_list) {
    if(!eglplus_egl_valid_display(display)) {
        eglplus_egl_SetErrorCode(EGL_BAD_DISPLAY);
        return EGL_NO_SURFACE;
    }
    ...
    if(*egl_attrib_list != EGL_NONE) {
        ...
        while(*tmp_attrib_list != EGL_NONE) {
            switch(*tmp_attrib_list++) {
                case EGL_WIDTH: {
                    glx_attrib_list[glx_attrib_count++] = GLX_PBUFFER_WIDTH;
                    glx_attrib_list[glx_attrib_count++] =
                      int(*tmp_attrib_list++);
                    break;
                }
                case EGL_HEIGHT: {
                    glx_attrib_list[glx_attrib_count++] = GLX_PBUFFER_HEIGHT;
                    glx_attrib_list[glx_attrib_count++] =
                      int(*tmp_attrib_list++);
                    break;
                }
                case EGL_LARGEST_PBUFFER: {
                    glx_attrib_list[glx_attrib_count++] = GLX_LARGEST_PBUFFER;
                    glx_attrib_list[glx_attrib_count++] =
                      (*tmp_attrib_list++ == EGL_TRUE) ? True : False;
                    break;
                }
                default:;
            }
    ...
    try {
        return new eglplus_egl_glx_SurfaceImpl(
          GLX_PBUFFER_BIT, pbuffer, &::glXDestroyPbuffer);
    } catch(...) {
        eglplus_egl_SetErrorCode(EGL_BAD_ALLOC);
        return EGL_NO_SURFACE;
    }
...
EGLAPI EGLSurface EGLAPIENTRY eglCreatePixmapSurface(
  ...
  const EGLint* egl_attrib_list) {
    ...
    try {
        return new eglplus_egl_glx_SurfaceImpl(GLX_PIXMAP_BIT, pixmap);
    } catch(...) {
        eglplus_egl_SetErrorCode(EGL_BAD_ALLOC);
        return EGL_NO_SURFACE;
    }
...
EGLAPI EGLBoolean EGLAPIENTRY eglQuerySurface(
  ...
  EGLint* egl_attrib_value) {
    ...
    switch(egl_attribute) {
        ...
        default:
            *egl_attrib_value = EGLint(glx_attrib_value);
    }
    return EGL_TRUE;
}
//------------------------------------------------------------------------------
...
eglSwapBuffers(EGLDisplay display, EGLSurface surface) {
    ...
    ::glXSwapBuffers(display->_x_open_display, surface->_glx_drawable);
...
EGLAPI EGLBoolean EGLAPIENTRY eglCopyBuffers(
  EGLDisplay display, EGLSurface surface, EGLNativePixmapType target) {
    if(!eglplus_egl_valid_display(display)) {
        eglplus_egl_SetErrorCode(EGL_BAD_DISPLAY);
        return EGL_FALSE;
    }
...
eglSwapInterval(EGLDisplay display, EGLint interval) {
    ...
    eglplus_egl_glXSwapIntervalEXT(
      display->_x_open_display, glx_drawable, interval);
    return EGL_TRUE;
}
//------------------------------------------------------------------------------
