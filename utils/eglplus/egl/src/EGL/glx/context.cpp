...
eglplus_egl_glx_ContextImpl::eglplus_egl_glx_ContextImpl(
  ::GLXContext context, ::GLXPbuffer empty_surf)
...
bool eglplus_egl_valid_context(EGLContext context) {
    ::pthread_mutex_lock(&eglplus_egl_Contexts_mutex);
    bool result =
      eglplus_egl_glx_Contexts.find(context) != eglplus_egl_glx_Contexts.end();
    ::pthread_mutex_unlock(&eglplus_egl_Contexts_mutex);
...
EGLAPI EGLContext EGLAPIENTRY eglCreateContext(
  ...
  const EGLint* egl_attrib_list) {
    ...
    if(eglplus_egl_glXCreateContextAttribsARB == nullptr) {
        eglplus_egl_glXCreateContextAttribsARB =
        ...
        while(*tmp_attrib_list != EGL_NONE) {
            switch(*tmp_attrib_list++) {
                ...
                case 0x30FC: // EGL_CONTEXT_FLAGS_KHR
                {
                    ...
                    glx_attrib_list[glx_attrib_count++] = glx_flags;
                    break;
                }
                case EGL_CONTEXT_OPENGL_DEBUG: {
                ...
                case EGL_CONTEXT_OPENGL_PROFILE_MASK: {
                    ...
                    if(
                      (egl_flags &
                       EGL_CONTEXT_OPENGL_COMPATIBILITY_PROFILE_BIT) != 0)
                        glx_flags |= 0x0002;
    ...
    int empty_surf_attr[] = {GLX_PBUFFER_WIDTH,
                             0,
                             GLX_PBUFFER_HEIGHT,
                             0,
    ...
    try {
        return new eglplus_egl_glx_ContextImpl(context, empty_surf);
    } catch(...) {
        eglplus_egl_SetErrorCode(EGL_BAD_ALLOC);
        return EGL_NO_CONTEXT;
    }
  ...
  EGLContext context) {
    ...
    if(
      ::glXMakeContextCurrent(
        display->_x_open_display, glx_draw, glx_read, glx_context) != True) {
        return EGL_FALSE;
    }
...
//------------------------------------------------------------------------------
EGLAPI EGLSurface EGLAPIENTRY eglGetCurrentSurface(EGLint which) {
    switch(which) {
        case EGL_DRAW: {
