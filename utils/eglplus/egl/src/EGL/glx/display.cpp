...
int eglplus_egl_X_error_handler(
  ::Display* display, ::XErrorEvent* error_event) {
    switch(error_event->error_code) {
        ...
        default: {
            if(eglplus_egl_X_old_error_handler) {
                return eglplus_egl_X_old_error_handler(display, error_event);
            }
        }
    }
    ...
    ::XGetErrorText(display, error_event->error_code, message, sizeof(message));

    std::cerr << "Error " << error_event->error_code << " (" << message
              << "): request " << error_event->request_code << "."
              << error_event->minor_code << std::endl;
...
//------------------------------------------------------------------------------
bool eglplus_egl_glx_DisplayImpl::_init() {
    // if already open
    if(_x_open_display != nullptr)
        return false;
...
//------------------------------------------------------------------------------
bool eglplus_egl_glx_DisplayImpl::_cleanup() {
    // if not open
    if(_x_open_display == nullptr)
        return false;
...
EGLAPI EGLDisplay EGLAPIENTRY eglGetDisplay(EGLNativeDisplayType display_id) {
    ...
    if(p == eglplus_egl_Displays.end()) {
        p = eglplus_egl_Displays
              .insert(eglplus_egl_display_map::value_type(
                display_id,
                std::unique_ptr<eglplus_egl_glx_DisplayImpl>(
                  new eglplus_egl_glx_DisplayImpl(display_id))))
              .first;
    }
