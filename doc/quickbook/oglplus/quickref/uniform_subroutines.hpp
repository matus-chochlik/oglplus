...
public:
    static GLint GetLocation(
      ...
      bool active_only); /*<
                      ...
                      [^identifier] in the [^stage] of a [^program]. If
                      [^active_only] is [^true] then throws __ProgVarError if no
                      such subroutine exists or if it is not active. See [glfunc
                      GetSubroutineIndex].
                      >*/

    GLint GetLocation(
      __ProgramName program, __StrCRef identifier, bool active_only) const;

    __ShaderType Stage() const; /*<
...
public:
    static GLint GetLocation(
      ...
      bool active_only); /*<
                      ...
                      [^identifier] in the [^stage] of a [^program]. If
                      [^active_only] is [^true] then throws __ProgVarError if no
                      such uniform exists or if it is not active. See [glfunc
...
class __ProgVar<
  ...
  __tag_NoTypecheck,
  void> : public __ProgVarLoc<__tag_SubroutineUniform> {
public:
