...
#define OGLPLUS_LINK_LIBRARY                                                   \
    0 /*<                                                                      \
Compile-time switch enabling linking of some parts of OGLplus from a binary    \
library. Certain parts of OGLplus (mostly complicated functions and functions          \
where static variables or long string literals are used) can be built into a        \
separate library that can be then linked to applications. If this option is            \
set to zero, then everything is inlined. If it is set to a nonzero integer           \
value, then some functions are just declared and must be built separatelly and       \
linked to the final executable.                                                \
                                                                               \
The [^oglplus/lib.hpp] header file can be included into one of the translation \
units when building the library or the final executable to include the              \
implementations of all such functions.                                                        \
                                                                               \
