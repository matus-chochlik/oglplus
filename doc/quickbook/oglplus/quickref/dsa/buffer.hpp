...
public:
    ...
    void Resize(
      __BufferSize size, __BufferUsage usage = BufferUsage::StaticDraw)
      const; /*<
    ...
    void RawData(
      ...
      const GLvoid* data,
      __BufferUsage usage =
        BufferUsage::StaticDraw) const; /*<
                               Uploads [^size] bytes from the location pointed
                               to by [^data] to [^this] buffer taking [^usage]
                               as a hint. See [glfunc NamedBufferData].
                               >*/

    void Data(
      const __BufferData& data, __BufferUsage usage = BufferUsage::StaticDraw)
      const; /*<
    ...
    void SubData(__BufferSize offset, const __BufferData& data)
    ...
    template <typename GLtype>
    static void SubData(
      __BufferSize offset, __SizeType count, const GLtype* data);
    ...
    static void CopySubData(
      ...
      __BufferSize
        size); /*<
            ...
            starting at [^readoffset] to [^writebuffer] starting at
            [^writeoffset]. See [glfunc CopyNamedBufferSubData].
            >*/
    ...
    void ClearSubData(
      ...
      __BufferSize size,
      __PixelDataFormat format,
      const GLtype* value) const; /*< Clears a sub-range (specified by [^offset]
                         and [^size]) of [^this] buffer with the specified fixed
                         [^value], using the specified [^internal_format].
                         >*/

    void Storage(const __BufferData& data, __Bitfield<__BufferStorageBit> flags)
      const; /*<
