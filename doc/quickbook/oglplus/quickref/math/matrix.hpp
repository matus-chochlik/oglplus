...
>*/
    ...
    friend Matrix Multiplied(
      ...
      const Matrix<__T, N, __Cols>& b); /*< Matrix-by-matrix multiplication. >*/
...
template <typename T>
class ModelMatrix : public __Matrix<T, 4, 4> {
public:
    ...
    static ModelMatrix RotationA(
      const __Vector<T, 3>& axis, __Angle<T> angle); /*<
...
template <typename T>
class CameraMatrix : public __Matrix<T, 4, 4> {
public:
    ...
    static CameraMatrix Perspective(
      ...
      T z_near,
      T z_far); /*<
             Creates a new perspective projection matrix with specified
             parameters.
             >*/
    static CameraMatrix PerspectiveX(
      __Angle<T> xfov, T aspect, T z_near, T z_far); /*<
Creates a new perspective matrix from x-axis [^xfov] angle,
x/y [^aspect] ratio and z-axis [^z_near] and [^z_far] planes.
>*/
    static CameraMatrix PerspectiveY(
      __Angle<T> yfov, T aspect, T z_near, T z_far); /*<
...
>*/
    ...
    static CameraMatrix Ortho(
      ...
      T z_near,
      T z_far); /*<
             Creates an orthographic projection matrix with the specified
             parameters.
             >*/
    ...
    static CameraMatrix ScreenStretch(
...
>*/
    static CameraMatrix ScreenTile(
...
>*/
    ...
    static CameraMatrix LookingAt(
      ...
      const __Vector<T, 3>& target); /*<
                                  ...
                                  [^eye] and looking at [^target] positions.
                                  >*/
    ...
    static CameraMatrix Orbiting(
      ...
      __Angle<T>
        elevation); /*<
                 Constructs a matrix for a camera orbiting the [^target]
                 position at the distance specified by [^radius], with the
