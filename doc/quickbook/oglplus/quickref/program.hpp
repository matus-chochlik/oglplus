...
template <>
class __ObjCommonOps<__tag_Program> : public __ProgramName {
public:
    static __ProgramName Binding(); /*<
...
public:
    ...
    void TransformFeedbackVaryings(
      const GLchar* (&varyings)[N], __TransformFeedbackMode mode);

    void TransformFeedbackVaryings(
      const std::vector<__String>& varyings,
      __TransformFeedbackMode mode) const;
...
>*/
    ...
    void BindLocation(
      ...
      __StrCRef identifier); /*<
                          Binds the location of a shading language variable to a
                          vertex attribute.
                          >*/
};
//]
...
ProgAndXFBMode operator<<(ProgramOps& prog, __TransformFeedbackMode mode);

template <std::size_t N>
ProgramOps& operator<<(ProgAndXFBMode pam, const GLchar* (&varyings)[N]);
...
ProgXFBModeAndNames operator<<(ProgAndXFBMode pam, const GLchar* name);

ProgXFBModeAndNames operator<<(ProgXFBModeAndNames&& pman, const GLchar* name);
...
public:
    ShaderProgram(
      ...
      __GLSLString&& source); /*<
                           Creates a program with a single shader with specified
                           type and source. Throws __ValidationError.
                           >*/
    ...
    ShaderProgram(
      ...
      __ObjectDesc&&
        object_desc); /*<
                   Creates a program with a single shader with specified type,
                   source and description. Throws __ValidationError.
                   >*/

    ShaderProgram(
      __ShaderType shader_type,
      __GLSLStrings&& source); /*<
                            Creates a program with a single shader with
                            specified type and source. Throws __ValidationError.
                            >*/
    ...
    ShaderProgram(
      ...
      const __GLSLSource&
        glsl_source); /*<
                   Creates a program with a single shader with specified type
                   and source. Throws __ValidationError.
                   >*/
...
class QuickProgram : public __Program {
public:
