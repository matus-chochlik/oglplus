...
public:
    ...
    static GLint GetLocation(
      ...
      bool active_only); /*<
                      ...
                      by [^identifier] in a [^program]. If [^active_only] is
                      [^true] then throws if no such attribute exists or if it
                      is not active. For a non-throwing version see
                      QueryActiveLocation(). See [glfunc GetAttribLocation].
                      >*/
    ...
    static bool QueryActiveLocation(
      ...
      __VertexAttribSlot&
        location); /*<
                Queries the vertex attribute location, returns [^false] on
                failure. See [glfunc GetAttribLocation].
                >*/
    ...
    static bool QueryCommonLocation(
      ...
      __VertexAttribSlot&
        location); /*<
                Allows to query the vertex attribute [^location] in multiple
                [^programs] This function returns a temporary object that allows
                to query the [^location] of the specified [^identifier] in
                several programs. The returned object has two functions called
                [^In] and [^And] which are equivalent and take a __Program as
                the argument. Both these functions return in turn a new instance
                of the temporary which allows to check in another program, and
                so on. The temporary is also convertible to [^bool] indicating
                whether a common location was found in all programs in the
                chain. See [glfunc GetAttribLocation]. [note Never store the
                resulting object in a named variable nor use it after the call
                to this overload of QueryCommonLocation has finished. Doing this
    ...
    static __VertexAttribSlot GetCommonLocation(
      ...
      __StrCRef
        identifier); /*<
                  Returns vertex attribute location in multiple programs if it's
                  consistent. Finds the location of the input vertex attribute
                  specified by [^identifier] in every program in [^programs].
                  Throws __Error if no such attribute exists or if it is not
                  active in some of the programs or if the attribute has
                  different locations in different programs. Otherwise returns
                  the vertex attribute position. See [glfunc GetAttribLocation].
                  >*/
};
...
class VertexArrayAttrib : public __ProgVarCommonOps<__tag_VertexAttrib> {
public:
    ...
    const VertexArrayAttrib& Setup(
      GLint values_per_vertex, __DataType data_type) const;

    const VertexArrayAttrib& Setup(
      GLint values_per_vertex, __DataType data_type)
      const; /*<
...
__VertexArrayAttrib operator|(
  __ProgramName program, const GLchar (&identifier)[N]);

__VertexArrayAttrib operator|(ProgramName program, GLuint location);
