...
public:
    ...
    const BoundObjOps& GetImage(
      GLint level, __PixelDataFormat format, const __OutputData& dest) const;
    ...
    const BoundObjOps& GetCompressedImage(
      GLint level, const __OutputData& dest) const;

    const BoundObjOps& GetCompressedImage(
      GLint level, __SizeType size, GLubyte* buffer) const;
    ...
    const BoundObjOps& Image3D(
      const __images_Image& image, GLint level = 0, GLint border = 0) const;
    ...
    const BoundObjOps& SubImage1D(
      const __images_Image& image, GLint xoffs, GLint level = 0) const;
    ...
    const BoundObjOps& CopySubImage1D(
      GLint level, GLint xoffs, GLint x, GLint y, __SizeType width) const;
    ...
    const BoundObjOps& Image3DMultisample(
      ...
      __SizeType height,
      __SizeType depth,
      __Boolean fixed_sample_locations) const;
#endif
    ...
    const BoundObjOps& __Buffer(
      __PixelDataInternalFormat internal_format, __BufferName buffer) const;
#endif
    ...
    const BoundObjOps& BorderColor(__Vector<GLfloat, 4> color) const;

    __Vector<GLint, 4> BorderColor(__TypeTag<GLint>) const;

    const BoundObjOps& BorderColor(__Vector<GLint, 4> color) const;

    __Vector<GLuint, 4> BorderColor(__TypeTag<GLuint>) const;

    const BoundObjOps& BorderColor(__Vector<GLuint, 4> color) const;

    __TextureCompareMode CompareMode() const;

    const BoundObjOps& CompareMode(__TextureCompareMode mode) const;

    __CompareFunction CompareFunc() const;

    const BoundObjOps& CompareFunc(__CompareFunction func) const;

    GLfloat LODBias() const;

    const BoundObjOps& LODBias(GLfloat value) const;

    const BoundObjOps& Filter(__TextureFilter filter) const;

    __TextureMagFilter MagFilter() const;

    const BoundObjOps& MagFilter(__TextureMagFilter filter) const;

    __TextureMinFilter MinFilter() const;

    const BoundObjOps& MinFilter(__TextureMinFilter filter) const;
    ...
    const BoundObjOps& Swizzle(
      __TextureSwizzleCoord coord, __TextureSwizzle mode) const;
#endif
...
#if GL_VERSION_3_3 || GL_ARB_texture_swizzle
    const BoundObjOps& SwizzleR(__TextureSwizzle mode) const;
#endif
...
#if GL_VERSION_3_3 || GL_ARB_texture_swizzle
    const BoundObjOps& SwizzleG(__TextureSwizzle mode) const;
#endif
...
#if GL_VERSION_3_3 || GL_ARB_texture_swizzle
    const BoundObjOps& SwizzleB(__TextureSwizzle mode) const;
#endif
...
#if GL_VERSION_3_3 || GL_ARB_texture_swizzle
    const BoundObjOps& SwizzleA(__TextureSwizzle mode) const;
#endif

#if GL_VERSION_3_3 || GL_ARB_texture_swizzle
    __TextureSwizzleTuple SwizzleRGBA() const;
#endif

#if GL_VERSION_3_3 || GL_ARB_texture_swizzle
    const BoundObjOps& SwizzleRGBA(__TextureSwizzle mode) const;
#endif
    ...
    const BoundObjOps& SwizzleRGBA(
      ...
      __TextureSwizzle mode_a) const;
#endif

#if GL_VERSION_3_3 || GL_ARB_texture_swizzle
    const BoundObjOps& SwizzleRGBA(const __TextureSwizzleTuple& modes) const;
#endif

    __TextureWrap Wrap(__TextureWrapCoord coord) const;

    const BoundObjOps& Wrap(__TextureWrapCoord coord, __TextureWrap mode) const;

    __TextureWrap WrapS() const;

    const BoundObjOps& WrapS(__TextureWrap mode) const;

    __TextureWrap WrapT() const;

    const BoundObjOps& WrapT(__TextureWrap mode) const;

    __TextureWrap WrapR() const;

    const BoundObjOps& WrapR(__TextureWrap mode) const;

    const BoundObjOps& Wrap(__TextureWrap mode) const;

#if GL_VERSION_4_3
    __PixelDataFormat DepthStencilMode() const;
#endif

#if GL_VERSION_4_3
    const BoundObjOps& DepthStencilMode(__PixelDataFormat mode) const;
#endif
...
#if GL_ARB_seamless_cubemap_per_texture
    const BoundObjOps& Seamless(__Boolean enable) const;
#endif
