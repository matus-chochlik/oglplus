...
public:
    Example() {
        ...
        __Buffer::Data(__BufferTarget::Array, 9, triangle_verts);
...
int main(int argc, char* argv[]) {
    ...
    if(glewInit() == GLEW_OK)
        try {
            ...
            std::cerr << "Program build error (in " << pbe.GLFunc()
                      << "'): " << pbe.what() << " [" << pbe.SourceFile() << ":"
                      << pbe.SourceLine() << "] " << std::endl
                      << pbe.Log() << std::endl;
        } catch(oglplus::__Error& err) {
            std::cerr << "Error (in " << err.GLFunc() << "'): " << err.what()
