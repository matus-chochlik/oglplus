...
OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_init_dr_ar_triangles(
...
OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_init_dr_ar_triangle_strip(
  const DrawOperation& draw_op) {
...
OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_init_dr_ar_triangle_fan(
  const DrawOperation& draw_op) {
    assert((draw_op.count == 0) || (draw_op.count >= 3));
    GLuint i = 0;
    GLuint v = 0;
...
OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_init_dr_el_triangles(
  const DrawOperation& draw_op) {
    assert(draw_op.count % 3 == 0);
...
OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_init_dr_el_triangle_strip(
  const DrawOperation& draw_op) {
    assert((draw_op.count == 0) || (draw_op.count >= 3));
    GLuint i = 0;
    GLuint v = 0;
...
OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_init_dr_el_triangle_fan(
  const DrawOperation& draw_op) {
    assert((draw_op.count == 0) || (draw_op.count >= 3));
    GLuint i = 0;
    GLuint v = 0;
...
OGLPLUS_LIB_FUNC
GLuint ShapeAnalyzerGraphData::_guess_face_count() {
...
OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_initialize() {
    const std::vector<DrawOperation>& draw_ops = _instr.Operations();
...
OGLPLUS_LIB_FUNC
bool ShapeAnalyzerGraphData::_adjacent_faces(
  GLuint fa, GLuint ea, GLuint fb, GLuint eb) {
    return _same_va_values(fa, ea, fb, eb, _main_vpv, _main_va);
}

OGLPLUS_LIB_FUNC
bool ShapeAnalyzerGraphData::_smooth_faces(
  GLuint fa, GLuint ea, GLuint fb, GLuint eb) {
    return _same_va_values(fa, ea, fb, eb, _smooth_vpv, _smooth_va);
}

OGLPLUS_LIB_FUNC
bool ShapeAnalyzerGraphData::_contin_faces(
  GLuint fa, GLuint ea, GLuint fb, GLuint eb) {
    ...
    for(std::size_t i = 0; i != n; ++i) {
        result |=
          _same_va_values(fa, ea, fb, eb, _other_vpvs[i], _other_vas[i]);
    }
    return result;
}

OGLPLUS_LIB_FUNC
void ShapeAnalyzerGraphData::_detect_adjacent() {
            ...
            if(_face_adj_f[i] == _nil_face()) {
                ...
                while(fj != fe) {
                    ...
                    for(GLuint fje = 0; fje != fjen; ++fje) {
                        GLuint j = *fj + fje;
                        bool nadj = (_face_adj_f[j] == _nil_face());
                        bool adjf = _adjacent_faces(fifb, fie, fjfb, fje);
                        bool smtf = adjf && _smooth_faces(fifb, fie, fjfb, fje);
                        bool cntf = adjf && _contin_faces(fifb, fie, fjfb, fje);
                        if(nadj && adjf) {
                            _face_adj_f[i] = fjfb;
                            _face_adj_f[j] = fifb;
                        ...
                        if(nadj && smtf) {
                            _face_edge_flags[i] |= _flg_smooth_edge;
                            _face_edge_flags[j] |= _flg_smooth_edge;
                        }
                        if(nadj && cntf) {
                            _face_edge_flags[i] |= _flg_contin_edge;
                            _face_edge_flags[j] |= _flg_contin_edge;
                        }
                    }
                    ++fj;
                }
