...
unsigned SpiralSphere::_vertex_count() const {
    return (_bands * 2) * (_divisions + 1) * (_segments + 1) +
           (_bands * 2) * (_segments + 1);
}

template <typename T>
void SpiralSphere::_make_vectors(
  std::vector<T>& dest, unsigned& k, double sign, double radius) const {
...
void SpiralSphere::_make_tangents(
  std::vector<T>& dest, unsigned& k, double sign) const {
    double b_leap = (math::Pi()) / double(_bands);
    double b_step = b_leap / double(_divisions);
    double s_step = (math::Pi()) / double(_segments);
...
OGLPLUS_LIB_FUNC
std::vector<GLfloat> SpiralSphere::_positions() const {
...
OGLPLUS_LIB_FUNC
std::vector<GLfloat> SpiralSphere::_normals() const {
    std::vector<GLfloat> dest(_vertex_count() * 3);
    unsigned k = 0;
    //
...
OGLPLUS_LIB_FUNC
std::vector<GLfloat> SpiralSphere::_tangents() const {
    std::vector<GLfloat> dest(_vertex_count() * 3);
    unsigned k = 0;
    //
...
OGLPLUS_LIB_FUNC
std::vector<GLfloat> SpiralSphere::_bitangents() const {
    std::vector<GLfloat> dest(_vertex_count() * 3);
    unsigned k = 0;
    //
...
std::vector<GLfloat> SpiralSphere::_tex_coords() const {
...
OGLPLUS_LIB_FUNC
SpiralSphere::IndexArray SpiralSphere::Indices(SpiralSphere::Default) const {
    const unsigned m = (_bands * 2) * (_divisions * 2) * (_segments + 1) +
                       (_bands * 8) * (_segments + 1);
    //
...
OGLPLUS_LIB_FUNC
DrawingInstructions SpiralSphere::Instructions(SpiralSphere::Default) const {
