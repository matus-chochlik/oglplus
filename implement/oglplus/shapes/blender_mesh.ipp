    ...
    if(
      ...
      (opts.load_materials && face_ptr)) {
        ...
        for(std::size_t f = 0; f != n_faces; ++f) {
            ...
            GLuint fv[4] = {GLuint(face_v1_field.Get(f, 0)),
            ...
            for(std::size_t i = 0; i != f_verts; ++i) {
                ...
                if(opts.load_materials) {
                    needs_vert_copy |=
                      (_mtl_data[fi[i]] >= 0) && (_mtl_data[fi[i]] != mat_nr);
                }
            }
        ...
        for(std::size_t f = 0; f != n_faces; ++f) {
            ...
            if(needs_vertex_copy[f]) {
                for(std::size_t i = 0; i != f_verts; ++i) {
                    ...
                    ais.push_back(
                      index_offset + GLuint(n_verts) + GLuint(n_add_verts));
                    ++n_add_verts;
                }
                // primitive restart index
                ais.push_back(0);
...
void BlenderMesh::_load_object(
  ...
  GLuint& index_offset) {
    ...
    if(object_data_data.StructureName() == "Mesh") {
        ...
        _load_mesh(opts, blend_file, object_data_data, obmat, index_offset);
...
OGLPLUS_LIB_FUNC
Spheref BlenderMesh::GetBoundingSphere() const {
    ...
    return Spheref(
      c.x(), c.y(), c.z(), Distance(c, Vec3f(min_x, min_y, min_z)));
}

OGLPLUS_LIB_FUNC
DrawingInstructions BlenderMesh::Instructions(BlenderMesh::Default) const {
    assert(_mesh_offsets.size() == _mesh_n_elems.size());
