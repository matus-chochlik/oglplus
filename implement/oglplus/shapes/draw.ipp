...
OGLPLUS_LIB_FUNC
void DrawOperation::SetupPrimitiveRestart_() const {
#if GL_VERSION_3_1
...
OGLPLUS_LIB_FUNC
void DrawOperation::CleanupPrimitiveRestart_() const {
    if(restart_index != NoRestartIndex()) {
#if GL_VERSION_3_1
        OGLPLUS_GLFUNC(Disable)(GL_PRIMITIVE_RESTART);
        OGLPLUS_VERIFY_SIMPLE(Disable);
  ...
  GLuint base_inst) const {
    switch(method) {
        case Method::DrawArrays: {
            return DrawArrays_(inst_count, base_inst);
        }

        case Method::DrawElements: {
            return DrawElements_(
              indices, index_data_type, inst_count, base_inst);
        }
    }
}
...
void DrawOperation::DrawArrays_(GLuint inst_count, GLuint base_inst) const {
    if(inst_count == 1) {
        OGLPLUS_GLFUNC(DrawArrays)(GLenum(mode), GLint(first), GLsizei(count));
        OGLPLUS_CHECK_SIMPLE(DrawArrays);
    } else if(base_inst == 0) {
#if OGLPLUS_DOCUMENTATION_ONLY || GL_VERSION_3_1
        OGLPLUS_GLFUNC(DrawArraysInstanced)
        (GLenum(mode), GLint(first), GLsizei(count), GLsizei(inst_count));
...
void DrawOperation::DrawElements_(
  ...
  GLuint base_inst) const {
    ...
    if(inst_count == 1) {
        ...
        (GLenum(mode), GLsizei(count), GLenum(index_data_type), indices);
