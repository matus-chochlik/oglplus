...
std::vector<GLfloat> WickerTorus::_positions() const {
            ...
            for(unsigned s = 0; s != _sections; ++s) {
                ...
                for(unsigned p = 0; p != 3; ++p) {
                    ...
                    for(unsigned d = 0; d != 2; ++d) {
                        ...
                        double rs_angle =
                          r_angle + d_sign * rfs * (1.0 - 0.25 * vs);
    ...
    for(unsigned d = 0; d != 2; ++d) {
        ...
        for(unsigned r = 0; r != _rings; ++r) {
            ...
            for(unsigned s = 0; s != _sections; ++s) {
                const double sa[3] = {
                  s * s_step, (s + t) * s_step, (s + 1.0 - 2 * t) * s_step};
                const double rd[3] = {0.0,
                                      0.0 + ((s % 2 == 0) ? -rdt : rdt),
                                      0.0 + ((s % 2 == 0) ? -rdt : rdt)};
                for(unsigned p = 0; p != 3; ++p) {
                    double vr = std::cos(sa[p]);
                    ...
                    for(unsigned f = 0; f != 2; ++f) {
                        ...
                        dest[k++] =
                          T(vx * (r1 + r2 * (1.0 + vr) + (fdt + rd[p]) * vr));
                        dest[k++] = T(vy * (r2 + (fdt + rd[p])));
                        dest[k++] =
                          T(vz * (r1 + r2 * (1.0 + vr) + (fdt + rd[p]) * vr));
                    }
                }
            }
    ...
    for(unsigned f = 0; f != 2; ++f) {
        ...
        for(unsigned s = 0; s != _sections; ++s) {
            ...
            for(unsigned r = 0; r != _rings; ++r) {
                const double r_angle = r * r_step;
                const double ra[2] = {
                  r_angle + r_slip, r_angle + r_step - r_slip};
...
OGLPLUS_LIB_FUNC
DrawingInstructions WickerTorus::Instructions(WickerTorus::Default) const {
...
OGLPLUS_LIB_FUNC
WickerTorus::IndexArray WickerTorus::Indices(WickerTorus::Edges) const {
...
OGLPLUS_LIB_FUNC
DrawingInstructions WickerTorus::Instructions(WickerTorus::Edges) const {
    auto instructions = this->MakeInstructions();
