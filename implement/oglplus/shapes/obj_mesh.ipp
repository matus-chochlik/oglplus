  ...
  std::istream& input) {
    ...
    while(std::getline(input, line)) {
        ...
        if(*i == 'm') {
            ...
            if(std::find_end(i, e, s, s + 6) != i) {
                throw std::runtime_error(
                  "Obj file loader: Unknown tag at line: " + line);
            }
            i += 6;
            while((i != e) && std::isspace(*i))
                ++i;
            std::string::const_iterator f = i;
            while((f != e) && !std::isspace(*f))
                ++f;
        ...
        // if the line contains vertex data
            ...
            if(i == e) {
                throw std::runtime_error(
                  "Obj file loader: Unexpected end of line: " + line);
            }
            char t = *i;
            ++i;
            ...
            for(std::size_t n = 0; n != 3; ++n) {
                if(!_load_indices(vi1[n], n_attr, i, e)) {
                    throw std::runtime_error(
                      "Obj file loader: Error reading indices: " + line);
                }
                vi1[n]._mtl = curr_mtl;
            }
            ...
            if(!mesh_offsets.empty()) {
                mesh_counts.push_back(
                  GLuint(idx_data.size()) - mesh_offsets.back());
            }
...
GLuint ObjMesh::GetMeshIndex(const std::string& name) const {
    ...
    if(!QueryMeshIndex(name, result)) {
        throw std::runtime_error(
          "ObjMesh: Unable to find index of mesh '" + name + "'");
    }
    return result;
}

OGLPLUS_LIB_FUNC
Spheref ObjMesh::MakeBoundingSphere() const {
