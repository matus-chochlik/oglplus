...
bool Cloud::_apply_sphere(const Vec3f& center, GLfloat radius) {
    ...
    for(GLsizei k = GLsizei((c.z() - r) * d), ke = GLsizei((c.z() + r) * d);
        k != ke;
        ++k)
        for(GLsizei j = GLsizei((c.y() - r) * h), je = GLsizei((c.y() + r) * h);
            j != je;
            ++j)
            for(GLsizei i = GLsizei((c.x() - r) * w),
                        ie = GLsizei((c.x() + r) * w);
                i != ie;
                ++i) {
                assert(k >= 0 && k < d);
...
OGLPLUS_LIB_FUNC
GLfloat Cloud::_rand_u() {
    return GLfloat(std::rand()) / RAND_MAX;
}

OGLPLUS_LIB_FUNC
GLfloat Cloud::_rand_s() {
...
void Cloud::_make_spheres(Vec3f center, GLfloat radius) {
    ...
    if(radius < _min_radius)
        return;
    if(!_apply_sphere(center, radius))
        return;
  ...
  : Image(cloud.Width(), cloud.Height(), 1, 3, &TypeTag<GLubyte>()) {
    ...
    for(GLsizei j = 0; j != h; ++j)
        for(GLsizei i = 0; i != w; ++i) {
            ...
            GLuint avg_density = ((depth_far - depth_near) > 0)
                                   ? total_density / (depth_far - depth_near)
                                   : 0;
            assert(p != e);
            *p = depth_near;
            ++p;
