...
ObjectOps<tag::DirectState, tag::Program>::AttachShader(ShaderName shader) {
    OGLPLUS_GLFUNC(AttachShader)(_obj_name(), GetGLName(shader));
    OGLPLUS_CHECK(AttachShader, ObjectPairError, Subject(shader).Object(*this));
    return *this;
}

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>&
ObjectOps<tag::DirectState, tag::Program>::AttachShaders(
...
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>&
ObjectOps<tag::DirectState, tag::Program>::DetachShader(ShaderName shader) {
    OGLPLUS_GLFUNC(DetachShader)(_obj_name(), GetGLName(shader));
    OGLPLUS_CHECK(DetachShader, ObjectPairError, Subject(shader).Object(*this));
    return *this;
}

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>&
ObjectOps<tag::DirectState, tag::Program>::Link() {
    OGLPLUS_GLFUNC(LinkProgram)(_obj_name());
    OGLPLUS_CHECK(LinkProgram, ObjectError, Object(*this));
    OGLPLUS_HANDLE_ERROR_IF(
      !IsLinked(),
      GL_INVALID_OPERATION,
      LinkError::Message(),
      LinkError,
      Log(GetInfoLog()).Object(*this));
    return *this;
}

OGLPLUS_LIB_FUNC
Outcome<ObjectOps<tag::DirectState, tag::Program>&>
ObjectOps<tag::DirectState, tag::Program>::Link(std::nothrow_t) {
    OGLPLUS_GLFUNC(LinkProgram)(_obj_name());
    OGLPLUS_DEFERRED_CHECK(LinkProgram, ObjectError, Object(*this));
  ...
  SizeType count, const GLchar* const* paths, const GLint* lengths) {
    ...
    while(!shaders.Empty()) {
        ...
        if(!shader.IsCompiled()) {
            if(
              auto outcome = Failed(
                shader.CompileInclude(count, paths, lengths, std::nothrow))) {
                return Res(outcome.ReleaseHandler(), *this);
            }
        }
        shaders.Next();
...
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>&
ObjectOps<tag::DirectState, tag::Program>::Validate() {
    OGLPLUS_GLFUNC(ValidateProgram)(_obj_name());
    OGLPLUS_VERIFY(ValidateProgram, ObjectError, Object(*this));
    OGLPLUS_HANDLE_ERROR_IF(
      !IsValid(),
      GL_INVALID_OPERATION,
...
OGLPLUS_LIB_FUNC
Outcome<ObjectOps<tag::DirectState, tag::Program>&>
ObjectOps<tag::DirectState, tag::Program>::Validate(std::nothrow_t) {
    OGLPLUS_GLFUNC(ValidateProgram)(_obj_name());
    OGLPLUS_DEFERRED_CHECK(ValidateProgram, ObjectError, Object(*this));
    OGLPLUS_RETURN_HANDLER_IF(
      !IsValid(),
      GL_INVALID_OPERATION,
      ValidationError::Message(),
...
OGLPLUS_LIB_FUNC
String ObjectOps<tag::DirectState, tag::Program>::GetInfoLog() const {
...
OGLPLUS_LIB_FUNC
void ObjectOps<tag::DirectState, tag::Program>::TransformFeedbackVaryings(
  SizeType count, const GLchar** varyings, TransformFeedbackMode mode) {
    OGLPLUS_GLFUNC(TransformFeedbackVaryings)
    (_obj_name(), count, varyings, GLenum(mode));
    OGLPLUS_CHECK(TransformFeedbackVaryings, ObjectError, Object(*this));
}
...
void ObjectOps<tag::DirectState, tag::Program>::TransformFeedbackVaryings(
  const std::vector<String>& varyings, TransformFeedbackMode mode) const {
    ...
    OGLPLUS_GLFUNC(TransformFeedbackVaryings)
    (_obj_name(), GLsizei(tmp.size()), tmp.data(), GLenum(mode));
    OGLPLUS_CHECK(TransformFeedbackVaryings, ObjectError, Object(*this));
}
...
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>&
ObjectOps<tag::DirectState, tag::Program>::MakeSeparable(Boolean para) {
    OGLPLUS_GLFUNC(ProgramParameteri)
    (_obj_name(), GL_PROGRAM_SEPARABLE, para._get());
    OGLPLUS_CHECK(ProgramParameteri, ObjectError, Object(*this));
    return *this;
}
#endif
...
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>&
ObjectOps<tag::DirectState, tag::Program>::MakeRetrievable(Boolean para) {
    OGLPLUS_GLFUNC(ProgramParameteri)
    (_obj_name(), GL_PROGRAM_BINARY_RETRIEVABLE_HINT, para._get());
    OGLPLUS_CHECK(ProgramParameteri, ObjectError, Object(*this));
    return *this;
}

OGLPLUS_LIB_FUNC
void ObjectOps<tag::DirectState, tag::Program>::GetBinary(
  std::vector<GLubyte>& binary, GLenum& format) const {
    ...
    if(size > 0) {
        ...
        (_obj_name(), size, &len, &format, binary.data());
        OGLPLUS_CHECK(GetProgramBinary, ObjectError, Object(*this));
    }
}

OGLPLUS_LIB_FUNC
void ObjectOps<tag::DirectState, tag::Program>::Binary(
  const std::vector<GLubyte>& binary, GLenum format) {
    OGLPLUS_GLFUNC(ProgramBinary)
    (_obj_name(), format, binary.data(), GLsizei(binary.size()));
    OGLPLUS_CHECK(ProgramBinary, ObjectError, Object(*this));
}
#endif

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::ShaderIterationContext::
  ShaderIterationContext(GLuint name, GLuint count)
  : _shader_names(count) {
    OGLPLUS_GLFUNC(GetAttachedShaders)
    (name, GLsizei(_shader_names.size()), nullptr, _shader_names.data());
    OGLPLUS_CHECK(GetAttachedShaders, ObjectError, Object(ProgramName(name)));
}

#if GL_VERSION_4_3
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::InterfaceContext
ObjectOps<tag::DirectState, tag::Program>::ActiveResourceContext(
  ProgramInterface intf) const {
    // get the maximum string length of the longest identifier
    GLint length = 0;
    OGLPLUS_GLFUNC(GetProgramInterfaceiv)
    (_obj_name(), GLenum(intf), GL_MAX_NAME_LENGTH, &length);
...
ObjectOps<tag::DirectState, tag::Program>::ActiveResources(
  ProgramInterface intf) const {
    // get the count of active attributes
    GLint count = 0;
    OGLPLUS_GLFUNC(GetProgramInterfaceiv)
    (_obj_name(), GLenum(intf), GL_ACTIVE_RESOURCES, &count);
    OGLPLUS_VERIFY(
      GetProgramInterfaceiv, ObjectError, Object(*this).EnumParam(intf));
...
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::InterfaceContext
ObjectOps<tag::DirectState, tag::Program>::ActiveAttribContext() const {
...
ObjectOps<tag::DirectState, tag::Program>::ActiveAttribs() const {
...
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::InterfaceContext
ObjectOps<tag::DirectState, tag::Program>::ActiveUniformContext() const {
    return InterfaceContext(
      _obj_name(),
...
ObjectOps<tag::DirectState, tag::Program>::ActiveUniforms() const {
...
OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::InterfaceContext
ObjectOps<tag::DirectState, tag::Program>::ActiveSubroutineContext(
  ShaderType stage) const {
    return InterfaceContext(
      _obj_name(),
      GetStageUIntParam(GLenum(stage), GL_ACTIVE_SUBROUTINE_MAX_LENGTH),
      GLenum(stage));
}

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::ActiveSubroutineRange
ObjectOps<tag::DirectState, tag::Program>::ActiveSubroutines(
  ShaderType stage) const {
    return ActiveSubroutineRange(
      ActiveSubroutineContext(stage),
      GetStageUIntParam(GLenum(stage), GL_ACTIVE_SUBROUTINES));
}

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::InterfaceContext
ObjectOps<tag::DirectState, tag::Program>::ActiveSubroutineUniformContext(
  ShaderType stage) const {
    return InterfaceContext(
      _obj_name(),
      GetStageUIntParam(GLenum(stage), GL_ACTIVE_SUBROUTINE_UNIFORM_MAX_LENGTH),
      GLenum(stage));
}

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::ActiveSubroutineUniformRange
ObjectOps<tag::DirectState, tag::Program>::ActiveSubroutineUniforms(
  ShaderType stage) const {
    return ActiveSubroutineUniformRange(
      ActiveSubroutineUniformContext(stage),
      GetStageUIntParam(GLenum(stage), GL_ACTIVE_SUBROUTINE_UNIFORMS));
}
#endif

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::InterfaceContext
ObjectOps<tag::DirectState, tag::Program>::TransformFeedbackVaryingContext()
  const {
    return InterfaceContext(
      _obj_name(),
...
ObjectOps<tag::DirectState, tag::Program>::TransformFeedbackVaryings() const {
...
ObjectOps<tag::DirectState, tag::Program>::AttachedShaders() const {
    GLuint count = GetUIntParam(ProgramParameter(GL_ATTACHED_SHADERS));
    return ShaderRange(ShaderIterationContext(_obj_name(), count), 0, count);
}

OGLPLUS_LIB_FUNC
ObjectOps<tag::DirectState, tag::Program>::ActiveUniformBlockRange
ObjectOps<tag::DirectState, tag::Program>::ActiveUniformBlocks() const {
    ...
    return ActiveUniformBlockRange(
      aux::ProgramInterfaceContext(_obj_name(), length), 0, count);
}
...
ProgramName ShaderProgram::_make(
  ShaderType shader_type, GLsizei count, const GLchar* const* strings) {
    GLuint program = OGLPLUS_GLFUNC(CreateShaderProgramv)(
      GLenum(shader_type), count, const_cast<const GLchar**>(strings));
    OGLPLUS_CHECK(CreateShaderProgramv, Error, EnumParam(shader_type));
    return ProgramName(program);
}

OGLPLUS_LIB_FUNC
void ShaderProgram::_check() {
    OGLPLUS_HANDLE_ERROR_IF(
      !IsValid(),
      GL_INVALID_OPERATION,
      ValidationError::Message(),
