...
OGLPLUS_LIB_FUNC
const char* ProgVarLocOps<tag::Uniform>::MsgGettingInactive() {
    return "Getting the location of inactive program uniform variable";
}

OGLPLUS_LIB_FUNC
const char* ProgVarLocOps<tag::Uniform>::MsgUsingInactive() {
    return "Using inactive program uniform variable";
}

OGLPLUS_LIB_FUNC
GLenum ProgVarTypeOps<tag::Uniform>::GetType(
  ProgramName program, GLint /*location*/, StrCRef identifier) {
    ...
    (GetGLName(program), GL_ACTIVE_UNIFORMS, &active_uniforms);
    OGLPLUS_VERIFY(
      GetProgramiv, ProgVarError, Program(program).Identifier(identifier));
    ...
    for(GLint index = 0; index < active_uniforms; ++index) {
        ...
        if(GLsizei(identifier.size()) == length) {
            if(
              std::strncmp(
                identifier.c_str(), buffer.data(), std::size_t(length)) == 0) {
                result = type;
                break;
            }
        }
