...
OGLPLUS_LIB_FUNC
bool BitmapGlyphPager::_frames_consistent() const {
    _gpu_page_map.Bind(Buffer::Target::Uniform);
    BufferTypedMap<gpu_frame_t> page_frame_map(
      BufferTarget::Uniform, BufferMapAccess::Read);
...
void BitmapGlyphPager::_replace_page(GLuint frame, GLuint page) {
    ...
    _gpu_page_map.Bind(Buffer::Target::Uniform);
    BufferTypedMap<gpu_frame_t> page_frame_map(
      BufferTarget::Uniform, BufferMapAccess::Write);
    if(previous >= 0) {
...
BitmapGlyphPager::BitmapGlyphPager(
  ...
  , _pg_map_tex_unit(pg_map_tex_unit) {
    ...
    std::vector<gpu_frame_t> data(
      BitmapGlyphPlaneCount(_parent) * BitmapGlyphPagesPerPlane(_parent),
      _invalid_gpu_frame());
    ...
    Texture::Buffer(
      Texture::Target::Buffer, PixelDataInternalFormat::R8UI, _gpu_page_map);

    assert(_is_ok());
}

OGLPLUS_LIB_FUNC
GLuint BitmapGlyphPager::FindFrame() {
    assert(_is_ok());
