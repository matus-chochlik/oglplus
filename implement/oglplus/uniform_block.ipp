...
OGLPLUS_LIB_FUNC
const char* ProgVarLocOps<tag::UniformBlock>::MsgGettingInactive() {
    return "Getting the location of inactive program uniform block";
}

OGLPLUS_LIB_FUNC
const char* ProgVarLocOps<tag::UniformBlock>::MsgUsingInactive() {
    return "Using inactive program uniform block";
}

OGLPLUS_LIB_FUNC
GLenum ProgVarCommonOps<tag::UniformBlock>::_translate_ref(
...
OGLPLUS_LIB_FUNC
GLenum ProgVarCommonOps<tag::UniformBlock>::_translate_max(
  ShaderType shader_type) {
    switch(shader_type) {
#ifdef GL_VERTEX_SHADER
        case ShaderType::Vertex:
