	...
	ShapeProgram(void)
	  ...
	  , colors(prog(), "Colors") {
		...
		fs.Source(
		  ...
		  "	DepthDiff =  clamp(pow(mix(1.0, 0.0, DepthDiff*4.0), 3.0), 0.0, "
		  "1.0);"
...
private:
	...
	static shapes::ShapeWrapper load_monkeys(void) {
		...
		return shapes::ShapeWrapper(
		  ...
		  shapes::BlenderMesh(blend_file,
			List("Monkey1")("Monkey2")("Monkey3").Get(),
			shapes::BlenderMesh::LoadingOptions()
			  .Nothing()
			  .Normals()
			  .Materials()));
	}
...
public:
	...
	void Reshape(GLuint vp_width, GLuint vp_height) {
		...
		Texture::Image2D(Texture::Target::Rectangle,
		  0,
		...
		auto projection =
		  CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 20);
	...
	void Render(double time) {
		...
		auto camera = CamMatrixf::Orbiting(monkeys.BoundingSphere().Center(),
		  7.0,
		...
		const Vec3f colors[3][3] = {{Vec3f(1.0f, 0.3f, 0.3f),
		...
		const Mat4f pos_matrices[3] = {ModelMatrixf::RotationY(Degrees(120))
		...
		monkeys.Draw(
		  [this, &rot_matrices, &pos_matrices](GLuint phase) -> bool {
			  this->depth_prog.model_matrix.Set(
				pos_matrices[phase] * rot_matrices[phase]);
			  return true;
		  });

		Texture::CopyImage2D(Texture::Target::Rectangle,
		  0,
		  PixelDataInternalFormat::DepthComponent,
		  0,
		  0,
		...
		monkeys.Draw(
		  [this, &colors, &rot_matrices, &pos_matrices](GLuint phase) -> bool {
			  ...
			  this->draw_prog.model_matrix.Set(
				pos_matrices[phase] * rot_matrices[phase]);
			  return true;
		  });
	}
