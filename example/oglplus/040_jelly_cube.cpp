...
private:
    ...
    void CopyFeedback(Buffer& src, Buffer& dst) {
        ...
        Buffer::CopySubData(
          se::CopyRead(), se::CopyWrite(), 0, 0, 2 * 4 * sizeof(GLfloat));
    }
...
public:
    ...
    void UpdatePhysics(CameraDriveProgram& cam_prog, double interval) {
        ...
        BufferTypedMap<Vec4f> map(
          BufferTarget::TransformFeedback, BufferMapAccess::Read);
...
private:
    ...
    void InitPositions(
      ...
      const Mat4f& transform) {
        ...
        Uniform<GLfloat>(phys_prog, "SpringBLength")
          .Set(GLfloat(sqrt(2.0f) * s_n));
        Uniform<GLfloat>(phys_prog, "SpringCLength")
          .Set(GLfloat(sqrt(3.0f) * s_n));
    ...
    int FaceIndex(
      const GLuint n, const GLuint n_1, const GLuint f, int i, int j) const {
        if(i < 0) {
            i += n_1;
            assert(i <= int(n_1));
            assert(j >= 0);
            ...
            switch(f) {
                // +x -> +z
                case 0:
                    return FaceIndex(n, n_1, 4, i, j);
                // -x -> -z
                case 1:
                    return FaceIndex(n, n_1, 5, i, j);
                // +y -> -x
                case 2:
                    return FaceIndex(n, n_1, 1, n_1 - j, i);
                // -y -> -x
                case 3:
                    return FaceIndex(n, n_1, 1, j, n_1 - i);
                // +z -> -x
                case 4:
                    return FaceIndex(n, n_1, 1, i, j);
                // -z -> +x
                case 5:
                    return FaceIndex(n, n_1, 0, i, j);
            }
        } else if(i > int(n_1)) {
            i -= n_1;
            assert(i <= int(n_1));
            ...
            switch(f) {
                ...
                // +y -> +x
                case 2:
                    return FaceIndex(n, n_1, 0, j, n_1 - i);
                // -y -> +x
                case 3:
                    return FaceIndex(n, n_1, 0, n_1 - j, i);
                // +z -> +x
                case 4:
                    return FaceIndex(n, n_1, 0, i, j);
                // -z -> -x
            ...
            switch(f) {
                // +x -> -y
                case 0:
                    return FaceIndex(n, n_1, 3, j, n_1 - i);
                // -x -> -y
                case 1:
                    return FaceIndex(n, n_1, 3, n_1 - j, i);
                // +y -> +z
                case 2:
                    return FaceIndex(n, n_1, 4, i, j);
                // -y -> -z
                case 3:
                    return FaceIndex(n, n_1, 5, n_1 - i, n_1 - j);
                // +z -> -y
                case 4:
                    return FaceIndex(n, n_1, 3, i, j);
                // -z -> -y
                case 5:
                    return FaceIndex(n, n_1, 3, n_1 - i, n_1 - j);
            }
        } else if(j > int(n_1)) {
            j -= n_1;
            assert(j <= int(n_1));
            ...
            switch(f) {
                // +x -> +y
                case 0:
                    return FaceIndex(n, n_1, 2, n_1 - j, i);
                // -x -> +y
                case 1:
                    return FaceIndex(n, n_1, 2, j, n_1 - i);
                // +y -> -z
                case 2:
                    return FaceIndex(n, n_1, 5, n_1 - i, n_1 - j);
                // -y -> +z
                ...
                // +z -> +y
                case 4:
                    return FaceIndex(n, n_1, 2, i, j);
                // -z -> +y
                case 5:
                    return FaceIndex(n, n_1, 2, n_1 - i, n_1 - j);
            }
        }
...
public:
        ...
        if(impulse_strength != 0.0) {
            if(prev_impulse_strength == 0.0) {
                phys_prog.impulse_center.Set(Vec3f(
                  ...
                  +4.5f - float(std::rand()) / float(RAND_MAX) * 9.0f +
                    pos.z()));
            }
    ...
    void Render(ExampleClock& clock) {
        ...
        bt_trg.UpdateAndStartIf(
          ...
          camera.Target().y() < 1.0f && clock.RealTime().Second() < 20,
          1.5);
