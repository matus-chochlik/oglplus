...
inline void example_print_std_error_common(
  std::exception& error, std::ostream& errstr) {
    errstr << "Message: '" << error.what() << "'" << std::endl;
}

inline void example_print_error_common(Error& error, std::ostream& errstr) {
    if(error.SourceFile()) {
        errstr << "Source file: '" << error.SourceFile() << "'" << std::endl;
    }

    if(error.SourceLine()) {
        errstr << "Source line: " << error.SourceLine() << std::endl;
    }

    if(error.SourceFunc()) {
        errstr << "Source function: '" << error.SourceFunc() << "'"
               << std::endl;
    }
    ...
    if(error.GLFunc()) {
        ...
        errstr << error.GLFunc() << "'" << std::endl;
    }

    if(error.EnumParam() || error.EnumParamName()) {
        errstr << "GL constant: ";
        if(error.EnumParamName()) {
            errstr << "'" << error.EnumParamName() << "'";
        } else {
            errstr << "(0x" << std::hex << error.EnumParam() << ")";
        }
        errstr << std::endl;
    }
    ...
    if(error.BindTarget() || error.TargetName()) {
        ...
        if(error.TargetName()) {
            errstr << "'" << error.TargetName() << "'";
        } else {
            errstr << "(0x" << std::hex << error.BindTarget() << ")";
        }
        errstr << std::endl;
    }
    ...
    if(error.ObjectTypeName() || error.ObjectType()) {
        ...
        if(error.ObjectTypeName()) {
            ...
            errstr << "(0x" << std::hex << error.ObjectType() << ")";
        }
        errstr << std::endl;
    }
    ...
    if(error.SubjectTypeName() || error.SubjectType()) {
        ...
        if(error.SubjectTypeName()) {
            errstr << "'" << error.SubjectTypeName() << "'";
        } else {
            errstr << "(0x" << std::hex << error.SubjectType() << ")";
        }
        errstr << std::endl;
    }
    ...
    if((!error.SubjectDesc().empty()) || (error.SubjectName() >= 0)) {
        ...
        if(!error.SubjectDesc().empty()) {
            errstr << "'" << error.SubjectDesc() << "'";
        } else {
            errstr << "(" << error.SubjectName() << ")";
        }
        errstr << std::endl;
    }

    if(error.Identifier()) {
        errstr << "Identifier: '" << error.Identifier() << "'" << std::endl;
    }

    if(error.Index() >= 0) {
        errstr << "Index: (" << error.Index() << ")" << std::endl;
    }

    if(error.Value() != 0) {
        errstr << "Value: (" << error.Value() << ")" << std::endl;
    }

    if(error.Limit() != 0) {
        errstr << "Limit: (" << error.Limit() << ")" << std::endl;
    }

    if(!error.Log().empty()) {
        errstr << "Log:" << std::endl << error.Log() << std::endl;
    }
}
