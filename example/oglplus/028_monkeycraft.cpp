...
private:
    static Program make() {
        ...
        fs.Source(
            ...
            "	float ambi = 0.5;"
            "	float diff = max(dot(geomNormal, normalize(geomLightDir)), "
            "0)*0.6;"
...
class AmbiOcclMaps : DSATextureEXT {
public:
...
public:
    Field(const images::Image& map, const Program& prog)
      : gl() {
        ...
        fademap.BindMulti(2);
        fademap.Image3D(
          images::RandomRedUByte(map.Width(), map.Height(), map.Depth()));
...
public:
    CubeExample()
      ...
      , field(
          images::FlipImageAxes<GLubyte, 1>(
            images::LoadTexture("suzanne_vol"), 0, 2, -1),
          prog)
      , shadowmaps(images::LoadTexture("oglcraft_ao"), prog) {
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        prog.projection_matrix.Set(CamMatrixf::PerspectiveX(
          Degrees(60), GLfloat(width) / height, 1, field.Radius() * 3));
    }
    ...
    ExampleTimePeriod ScreenshotTime() const {
        return ExampleTimePeriod::Seconds(28.0);
    }
};
