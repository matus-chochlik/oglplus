...
public:
    MetalFloor(const Program& metal_prog)
      : shapes::ShapeWrapper(
        ...
        metal_tex << TextureTarget::_2D << TextureMinFilter::LinearMipmapLinear
...
public:
    BarGridBarData(
      ...
      , index_count((9 * 2 + 1) + 4 * (4 + 1 + 3 + 1) + (4)) {
        ...
        *ii++ = 18;
        ...
        positions << Buffer::Target::Array << pos_data;

        normals << Buffer::Target::Array << nml_data;

        indices << Buffer::Target::ElementArray << idx_data;
    ...
    void InitVAO(const Program& prog, const VertexArray& vao) {
        ...
        VertexArrayAttrib(prog, "PosAndOffs").Setup<Vec4f>().Enable();

        gl.Bind(Buffer::Target::Array, normals);
        VertexArrayAttrib(prog, "Normal").Setup<Vec3f>().Enable();
...
class BarGridDrawProgram : public Program {
private:
    static Program make() {
        Program prog;
        prog.AttachShader(VertexShader(
...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        auto projection =
          CamMatrixf::PerspectiveX(Degrees(65), width, height, 1.0, 100.0);
