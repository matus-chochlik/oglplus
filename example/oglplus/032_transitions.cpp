...
public:
    DrawFramebuffer(
      TextureUnitSelector color_unit, TextureUnitSelector depth_unit)
...
class ClearProgram : public Program {
private:
    static Program make() {
        Program prog;
...
class TransitionProgram : public Program {
private:
    static Program make() {
        Program prog;
        ...
        prog.AttachShader(FragmentShader(
          ...
          StrCRef("#version 400\n"
                  ...
                  "subroutine vec4 TransitionFunc();"
                  "subroutine uniform TransitionFunc Transition;"

                  "in vec2 vertPosition, vertTexCoord;"
                  "out vec4 fragColor;"

                  "int Frame0() { return Direction?1:0; }"
                  "int Frame1() { return Direction?0:1; }"

                  "float Factor() { return Direction?1.0-MixFactor:MixFactor; }"

                  "vec4 Texel0()"
                  "{"
                  "	return texelFetch(FrameTex, ivec3(gl_FragCoord.xy, "
                  "Frame0()), 0);"
                  "}"
                  ...
                  "	float cv = length(vec2(int(fc.x) % s - a, int(fc.y) % s - "
                  "a)/r);"
    ...
    Screen(const Program& prog1, const Program& prog2) {
        ...
        corners.Bind(Buffer::Target::Array);
        {
            ...
            VertexArrayAttrib attr(VertexArrayAttrib::GetCommonLocation(
              MakeGroup(prog1, prog2), "Position"));
            attr.Setup<Vec2f>();
            attr.Enable();
        }
    }
...
private:
    ...
    static GLuint torus_vert_attr_count() {
        return 4;
    }
    static const GLchar** torus_vert_attr_names() {
        static const GLchar* attrs[] = {
...
public:
    TransitionExample()
      ...
      , screen(clear_prog, trans_prog) {
        ...
        ProgramUniform<Vec3f>(draw_prog, "LightPosition")
    ...
    void Reshape(GLuint vp_width, GLuint vp_height) {
        ...
        projection_0 =
          CamMatrixf::PerspectiveX(Degrees(70), width, height, 1, 20);
        projection_1 =
          CamMatrixf::PerspectiveX(Degrees(24), width, height, 1, 40);
    }
