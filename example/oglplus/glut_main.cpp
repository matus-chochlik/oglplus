...
class SingleExample {
private:
    static SingleExample*& SingleInstance() {
...
public:
    ...
    void Display() {
        ...
        if(_primitive_query) {
            try {
                auto query_exec = _primitive_query->Execute(
                  Query::Target::PrimitivesGenerated, primitives_per_frame);
        ...
        if(this_interval >= fps_interval) {
            ...
            std::cout << _frame_no << " frames in " << std::fixed
                      << this_interval << " seconds = " << std::fixed
                      << _frame_no / this_interval << " FPS; "
                      << std::scientific << _prim_count / this_interval
                      << " PPS; " << std::scientific << _prim_count / _frame_no
                      << " PPF; " << std::endl;
            _frame_no = 0;
            _prim_count = 0.0;
        }
    ...
    void Screenshot() {
        ...
        if(_clock.Now() >= _example->ScreenshotTime()) {
            ...
            glReadPixels(
              0, 0, _width, _height, GL_RGB, GL_UNSIGNED_BYTE, pixels.data());
