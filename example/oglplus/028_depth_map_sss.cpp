...
public:
    DepthBuffer(const Program& prog, GLuint tex_unit, GLuint tex_side)
      : side(tex_side) {
        ...
        tex << Texture::Target::_2D << TextureFilter::Nearest
        ...
        fbo << Framebuffer::Target::Draw << FramebufferAttachment::Color << rbo
            << FramebufferAttachment::Depth << tex << FramebufferComplete();
    }
...
public:
    ...
    void Render(double time) {
        ...
        GLfloat lgt_tgt_dist =
          Distance(shape.BoundingSphere().Center(), light.Position());

        auto lgt_proj = CamMatrixf::PerspectiveX(
