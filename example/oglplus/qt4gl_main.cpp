...
QGLFormat OGLplusExampleGLWidget::getGLFormat() {
    QGLFormat result;
...
OGLplusExampleGLWidget::OGLplusExampleGLWidget(
  QWidget* parent_widget, int argn, char** args, const char* ss_path)
...
QSize OGLplusExampleGLWidget::minimumSizeHint() const {
    return QSize(128, 128);
}

QSize OGLplusExampleGLWidget::sizeHint() const {
...
void OGLplusExampleGLWidget::initializeGL() {
    emit updatedGL();
...
void OGLplusExampleGLWidget::paintGL() {
    assert(example);
    if(screenshot_path) {
        ...
        if(clock.Now() >= example->ScreenshotTime()) {
            ...
            glReadPixels(
              0, 0, width(), height(), GL_RGB, GL_UNSIGNED_BYTE, pixels.data());
        ...
        if(!example->Continue(clock))
            ...
            example->Render(clock);
        emit timeUpdated(clock.Now().Seconds(), clock.RealTime().Seconds());
    }
    update();
}
...
void OGLplusExampleGLWidget::mouseMoveEvent(QMouseEvent* mouse_event) {
    ...
    assert(example);
    example->MouseMove(
      mouse_event->x(), height() - mouse_event->y(), width(), height());
    emit mouseMoved(mouse_event->x(), height() - mouse_event->y());
}

OGLplusExampleWindow::OGLplusExampleWindow(
  int argc, char** argv, const char* screenshot_path) {
    ...
    connect(
      ...
      this,
      SLOT(updateMouseInfo(int, int)));
    connect(
      gl_widget,
      SIGNAL(timeUpdated(double, double)),
      this,
      SLOT(updateTimeInfo(double, double)));
...
void OGLplusExampleWindow::updateGLInfo() {
...
int qtgl_example_main(int argc, char** argv) {
    ...
    OGLplusExampleWindow example_window(argc, argv, screenshot_path);
    example_window.show();
    return app.exec();
}
