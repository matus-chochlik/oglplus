...
class ShadowProg : public Program {
public:
    static Program make() {
        Program result;
...
class DisplayProg : public Program {
public:
    static Program make() {
        Program result;
        ...
        vs.Source(
            ...
            "uniform mat4 ProjectionMatrix, CameraMatrix, LightProjMatrix, "
            "LightMatrix;"
...
public:
    ...
    void AddShape(const ShapeGenerator& shape_gen) {
        shapes.emplace_back(List("Position").Get(), shape_gen, prog);
        if(shapes.size() == 1) {
            ishape = shapes.begin();
        }
    }
    ...
    double Draw(double time) {
        ...
        projection_matrix.Set(CamMatrixf::PerspectiveX(
          Degrees(45),
          1.0f,
          1.0f + dist,
        ...
        model_matrix.Set(
          ModelMatrixf::RotationA(Vec3f(1, 1, 1), FullCircles(time / -37.0)));
...
public:
    BarDisplay()
      ...
      , shadow_vao(cube.VAOForProgram(shadow_prog)) {
        ...
        auto light_proj =
          CamMatrixf::PerspectiveX(Degrees(74), 1.0, 1, 3 * side);
    ...
    void Reshape(GLuint vpw, GLuint vph) {
        ...
        display_prog.projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(65), width, height, 1, 3 * side));
    }
