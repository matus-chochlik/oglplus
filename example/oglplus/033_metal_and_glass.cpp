...
public:
	ShadowFragmentShader(void)
	  : FragmentShader(ObjectDesc("Shadow fragment shader"),
		  "#version 150\n"
		  "in vec3 vertNormal;"
		  "in vec3 vertTangent;"
...
public:
	GlassFragmentShader(void)
	  : FragmentShader(ObjectDesc("Metal fragment shader"),
		  "#version 150\n"
...
public:
	GlassAndMetalExample(void)
	  ...
	  , shadow_tex_side(1024) {
		...
		gl.Bound(Framebuffer::Target::Draw, frame_shadow_fbo)
		  .AttachTexture(FramebufferAttachment::Depth, frame_shadow_tex, 0);
		...
		gl.Bound(Framebuffer::Target::Draw, glass_shadow_fbo)
		  .AttachTexture(FramebufferAttachment::Color, glass_shadow_tex, 0);
	...
	void RenderGlassShadowMap(const Vec3f& light_position,
	  ...
	  const Mat4f& light_proj_matrix) {
		...
		Planef clip_plane = Planef::FromPointAndNormal(
		  torus_center, Normalized(light_position - torus_center));
		...
		for(int c = 0; c != 2; ++c) {
			...
			for(int p = 3; p >= 0; --p) {
				...
				torus.Draw([&p](GLuint phase) -> bool {
					if(p == 0 || p == 3) {
						return (phase == 4);
					} else
						return (phase > 4);
	...
	void RenderFrameShadowMap(const Vec3f& light_position,
	  const Mat4f& torus_matrix,
	  const Mat4f& light_proj_matrix) {
		...
		torus.Draw([](GLuint phase) -> bool { return (phase <= 3); });
	...
	void RenderImage(double time,
	  ...
	  const Mat4f& light_proj_matrix) {
		...
		Mat4f perspective =
		  CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 60);
		...
		// setup the view clipping plane
		Planef clip_plane = Planef::FromPointAndNormal(
		  torus_center, Normalized(camera_position - torus_center));

		metal_pp.Bind();

		// Render the plane
		transf_prog.model_matrix = ModelMatrixf();
		transf_prog.texture_matrix.Set(
		  Mat2f(Vec2f(9.0f, 0.0f), Vec2f(0.0f, 9.0f)));
		...
		transf_prog.model_matrix.Set(torus_matrix);
		transf_prog.texture_matrix.Set(
		  Mat2f(Vec2f(16.0f, 0.0f), Vec2f(0.0f, 4.0f)));
		metal_prog.metal_tex.Set(0);
	...
	void Render(double time) {
		...
		RenderFrameShadowMap(light_position, torus_matrix, light_proj_matrix);
		RenderGlassShadowMap(
		  light_position, torus_center, torus_matrix, light_proj_matrix);
