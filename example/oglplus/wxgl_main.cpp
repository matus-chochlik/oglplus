...
struct ExampleInfoDisplay {
    ...
    virtual void MouseMove(unsigned mouse_x, unsigned mouse_y) = 0;

    virtual void Resize(unsigned width, unsigned height) = 0;

    virtual void FrameUpdate(
      unsigned frame_no, double sim_time, double real_time) = 0;

    virtual void RenderStats(double fps, double pps, double ppf) = 0;
};

// configuration options for wxGL
static inline int* GLConfig() {
...
inline wxString MakeCommonErrorMessage(oglplus::Error& error) {
    ...
    if(error.SourceLine()) {
        message.Append(
          wxString::Format(wxT("Source line: '%d'\r\n"), error.SourceLine()));
    }
...
inline void HandleError(oglplus::Error& error, wxWindow* parent = nullptr) {
    wxMessageBox(
...
inline void HandleError(
  const std::exception& error, wxWindow* parent = nullptr) {
    wxMessageBox(
...
private:
    ...
    class ExampleGLCanvas : public wxGLCanvas {
    private:
    ...
    void Update() {
        ...
        example_clock.Update(
          oglplus::ExampleTimePeriod::Seconds(os_clock.seconds()));
    ...
    void HandleMouseMove(wxPoint position) {
        ...
        if(position.y < 0)
            position.y = 0;
        if(position.x > size.GetWidth())
            position.x = size.GetWidth();
        if(position.y > size.GetHeight())
            position.y = size.GetHeight();
        example->MouseMove(
          position.x,
          size.GetHeight() - position.y,
          size.GetWidth(),
          size.GetHeight());
        info_display->MouseMove(position.x, size.GetHeight() - position.y);
    }
