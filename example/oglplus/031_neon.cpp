...
class PhysicsProg : public Program {
private:
    static Program make() {
        Program prog;
        ...
        xfbs
          .Source(
            ...
            "	xfbPosition = vec4(Position + xfbVelocity.xyz * Interval, "
            "Time);"
            "}")
          .Compile();

        prog << xfbs << TransformFeedbackMode::SeparateAttribs << "xfbPosition"
             << "xfbVelocity";
...
public:
      ...
      , velocities(2) {
        ...
        for(GLuint i = 0; i != particle_count; ++i) {
            Vec3f pos(
              ...
              (std::rand() / float(RAND_MAX) - 0.5f));
            pos = Normalized(pos) *
                  (1.0f + (std::rand() / float(RAND_MAX) - 0.5f) * 0.1);
            if(i < 10)
        ...
        for(GLuint i = 0; i != particle_count; ++i) {
            ...
            for(GLuint c = 0; c != 3; ++c)
                vel_data[k++] =
                  v * v * (1.0f * std::rand() / float(RAND_MAX) - 0.5f);
            vel_data[k++] = 0.0f;
        }
        ...
        for(GLuint b = 0; b != 2; ++b) {
            ...
            positions[b] << Buffer::Target::Array << BufferUsage::DynamicDraw
                         << pos_data;
            (prog | "Position").Setup<Vec4f>().Enable();

            velocities[b] << Buffer::Target::Array << BufferUsage::DynamicDraw
                          << vel_data;
        ...
        for(GLuint b = 0; b != 2; ++b) {
            ...
            positions[nb] << BufferIndexedTarget::TransformFeedback << 0
                          << BufferTarget::TransformFeedback;

            velocities[nb] << BufferIndexedTarget::TransformFeedback << 1
                           << BufferTarget::TransformFeedback;
        }
        ...
        forces << BufferIndexedTarget::Uniform << 0 << BufferTarget::Uniform
               << force_data;
    ...
    GLuint ParticleCount() const {
        return particle_count;
    }

    Reference<Buffer> Offsets() const {
        return positions[0];
    }
...
class ParticleProg : public Program {
private:
    static Program make() {
    ...
    Trails(const Program& program)
      ...
      , curr_trail_point(0) {
        ...
        std::vector<GLfloat> pos_data(
          electron_count * max_trail_points * 4, 0.0);
        ...
        for(GLuint e = 0; e != electron_count; ++e) {
            idx_data[k++] =
              GLshort(e + electron_count * (max_trail_points - 1));
...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        auto perspective =
          CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 1000);
