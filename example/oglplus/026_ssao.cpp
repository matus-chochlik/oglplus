...
class DataProgram : public Program {
private:
...
private:
    ...
    static Program make() {
        ...
        fs.Source(
            ...
            "	{"
            "		vec2 SampleCoord = "
            "gl_FragCoord.xy+SampleSpread*SampleOffs[s];"
...
class Shape : public shapes::ShapeWrapper {
private:
    static shapes::ShapeWrapper load(const char* name, const Program& prog) {
        ...
        return shapes::ShapeWrapper(
          List("Position").Get(),
          shapes::ObjMesh(input, shapes::ObjMesh::LoadingOptions(false)),
          prog);
    }
...
public:
    DataBuffer(
      const Program& prog, GLuint tex_unit, GLuint width, GLuint height) {
        Texture::Active(tex_unit);
        ProgramUniformSampler(prog, "DataMap").Set(tex_unit);

        tex << Texture::Target::Rectangle << TextureFilter::Linear
        ...
        fbo << Framebuffer::Target::Draw << FramebufferAttachment::Color << tex
            << FramebufferAttachment::Depth << rbo << FramebufferComplete();
    }
...
public:
    ...
    void Render(double time) {
        ...
        GLfloat cam_tgt_dist =
          Distance(shape.BoundingSphere().Center(), camera.Position());

        auto cam_proj = CamMatrixf::PerspectiveX(
          Degrees(45),
          width,
          height,
          cam_tgt_dist - bs_rad,
    ...
    ExampleTimePeriod ScreenshotTime() const {
        return ExampleTimePeriod::Seconds(29.0);
    }
};
