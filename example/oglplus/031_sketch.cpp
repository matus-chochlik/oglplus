...
public:
	LineGeomShader(void)
	  : GeometryShader(ObjectDesc("Line geometry shader"),
		  "#version 150\n"
		  "layout(lines) in;"
...
public:
	LineFragShader(void)
	  : FragmentShader(ObjectDesc("Line fragment shader"),
		  "#version 150\n"
...
public:
	SketchFragShader(void)
	  : FragmentShader(ObjectDesc("Sketch fragment shader"),
		  "#version 150\n"
...
public:
	Shape(const Program& prog, const ShapeBuilder& builder)
	  ...
	  , vbos(3) {
		...
		const GLuint nva = 3;
		const GLchar* vert_attr_name[nva] = {"Position", "Normal", "TexCoord"};
...
public:
	SketchExample(void)
	  ...
	  , sketch_prog()
	  , plane(transf_prog,
		  shapes::Plane(Vec3f(0, 0, 0), Vec3f(9, 0, 0), Vec3f(0, 0, -9), 9, 9))
	  ...
	  , shadow_tex_side(1024) {
		...
		sketch_prog.sketch_tex.Set(0);
		{
			...
			for(GLuint i = 0; i < sketch_tex_layers; ++i) {
				auto image = images::BrushedMetalUByte(512,
				  512,
				  64 + i * 128,
				  -(2 + i * 4),
				  +(2 + i * 4),
		...
		gl.Bound(Framebuffer::Target::Draw, frame_shadow_fbo)
		  .AttachTexture(FramebufferAttachment::Depth, shadow_tex, 0);
	...
	void RenderShadowMap(const Vec3f& light_position,
	...
	void RenderImage(
	  double time, const Mat4f& torus_matrix, const Mat4f& light_proj_matrix) {
	...
	void Render(double time) {
		...
		const Mat4f torus_matrix =
		  ...
		  * ModelMatrixf::RotationA(Vec3f(1, 2, 1), FullCircles(time / 17.0));
		...
		RenderShadowMap(light_position, torus_matrix, light_proj_matrix);
