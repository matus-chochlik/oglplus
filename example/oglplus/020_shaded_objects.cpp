...
public:
    ...
    void Render(const Vec3f& light, const Mat4f& camera, const Mat4f& model) {
...
private:
    ...
    static const GLchar* fs_prologue() {
        return "#version 140\n"
    ...
    static const GLchar* fs_epilogue() {
        return "	fragColor = vec4(Color * Intensity, 1.0);"
               "}";
    }

    // The part calculating the color for the black/white checker shader
    static const GLchar* fs_bw_checker() {
        return "	float c = ("
               "		1 +"
    ...
    static const GLchar* fs_yb_strips() {
        return "	float m = int((vertTexCoord.x+vertTexCoord.y)*16) % 2;"
               "	vec3 Color = mix("
               "		vec3(0.0, 0.0, 0.0),"
               "		vec3(1.0, 1.0, 0.0),"
               "		m"
    ...
    static const GLchar* fs_wo_vstrips() {
        return "	float m = int(vertTexCoord.x*8) % 2;"
               "	vec3 Color = mix("
               "		vec3(1.0, 0.6, 0.1),"
               "		vec3(1.0, 0.9, 0.8),"
               "		m"
    ...
    static const GLchar* fs_br_circles() {
        return "	vec2  center = vertTexCoord - vec2(0.5, 0.5);"
    ...
    static const GLchar* fs_wg_spirals() {
        return "	vec2  center = (vertTexCoord - vec2(0.5, 0.5)) * 16.0;"
...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        auto projection =
          CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 50);
    ...
    void Render(double time) {
        ...
        auto camera = CamMatrixf::Orbiting(
          Vec3f(), 6.0, Degrees(time * 15), Degrees(SineWave(time / 6.0) * 45));
        // render the shapes
        ...
        torus.Render(
          ...
          ModelMatrixf::Translation(-1.0f, -1.0f, -1.0f) *
            ModelMatrixf::RotationA(
              Vec3f(1.0f, 1.0f, 1.0f), Degrees(time * 45)) *
