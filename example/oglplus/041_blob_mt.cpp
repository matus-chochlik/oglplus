...
SurfaceDrawData::SurfaceDrawData(GLuint pln, GLuint wrkr, GLuint cap)
  ...
  , plane_to_draw(workers) {
    ...
    for(GLuint p = 0; p != planes; ++p) {
        ...
        Buffer::Data(
          Buffer::Target::Array, init_data, BufferUsage::DynamicCopy);
...
SurfacePolyXFBData::SurfacePolyXFBData(SurfaceDrawData& draw_data, GLuint id)
  ...
  , plane_to_draw(draw_data.plane_to_draw[id]) {
    ...
    for(GLuint i = 0; i != planes; ++i) {
        ...
        positions[i].BindRange(
          Buffer::IndexedTarget::TransformFeedback, 0, offs, size);

        normals[i].Bind(Buffer::Target::TransformFeedback);
        normals[i].BindRange(
          Buffer::IndexedTarget::TransformFeedback, 1, offs, size);
    }
...
Program SurfacePolygonizerCommon::MakeProg() {
    ...
    prog.TransformFeedbackVaryings(
      xfb_varyings, TransformFeedbackMode::SeparateAttribs);
...
void SurfacePolygonizerCommon::InitCellConfigs(SurfaceExample& parent) {
    ...
    UniformSampler(prog, "Configurations").Set(tex_unit);
    {
        ...
        const GLubyte tex_data[] = {
          x, x, x, x, x, x, x, x, a, c, b, x, d, d, d, x, b, d, a, x, c, c,
          c, x, b, b, a, a, c, d, c, d, a, d, c, x, b, b, b, x, a, a, c, c,
          b, d, b, d, a, a, d, d, c, b, c, b, a, a, a, x, b, d, c, x, c, d,
          b, x, a, a, a, x, c, c, b, b, a, d, a, d, b, d, b, d, c, c, a, a,
          b, b, b, x, c, d, a, x, c, d, c, d, a, a, b, b, c, c, c, x, a, d,
          b, x, d, d, d, x, a, b, c, x, x, x, x, x, x, x, x, x};
...
void SurfacePolygonizerCommon::UpdateMetaballs(
  SurfaceExample& parent, double time_diff) {
...
SurfaceLightMapRenderer::SurfaceLightMapRenderer(SurfaceExample& parent)
  ...
  , get_intensity_background(
      prog, ShaderType::Fragment, "GetIntensityBackground")
  , surface_vao(surface_data.planes)
...
SurfaceExample::SurfaceExample(const ExampleParams& /*params*/)
  ...
  , screen(List("Position").Get(), shapes::Screen(), screen_prog) {
    surface_data.light_projection =
      CamMatrixf::Perspective(-1, 1, -1, 1, 18.0, 22.0);
...
void SurfaceExample::Reshape(GLuint width, GLuint height) {
    ...
    surface_data.camera_projection =
      CamMatrixf::PerspectiveX(Degrees(75), float(width) / height, 1, 80);
}
...
void SurfaceExample::Render(ExampleClock& clock) {
    ...
    Vec4f tmp_light_pos(
      (surface_data.camera_projection * surface_data.camera *
...
void setupExample(ExampleParams& params) {
    if(params.max_threads > 0) {
        ...
        */
        ...
        throw std::runtime_error("This example requires multi-threading");
}

std::unique_ptr<ExampleThread> makeExampleThread(
  Example& example, unsigned thread_id, const ExampleParams& params) {
    if(thread_id < params.num_threads - 3) {
        return std::unique_ptr<ExampleThread>(new SurfacePolygonizer(
          dynamic_cast<SurfaceExample&>(example), thread_id));
    } else if(thread_id < params.num_threads - 2) {
        return std::unique_ptr<ExampleThread>(
          new SurfaceDepthMapRenderer(dynamic_cast<SurfaceExample&>(example)));
    } else if(thread_id < params.num_threads - 1) {
        return std::unique_ptr<ExampleThread>(
          new SurfaceColorMapRenderer(dynamic_cast<SurfaceExample&>(example)));
    } else if(thread_id < params.num_threads) {
        return std::unique_ptr<ExampleThread>(
          new SurfaceLightMapRenderer(dynamic_cast<SurfaceExample&>(example)));
    }
    return std::unique_ptr<ExampleThread>();
}
