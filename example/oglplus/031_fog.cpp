...
class Instances {
private:
    static std::vector<Vec3f> make_positions() {
    ...
    static std::vector<Vec3f> make_normals() {
...
public:
      ...
      , path_nml(make_normals(), 0.25) {
        ...
        for(GLuint i = 0; i != n; ++i) {
            ...
            Vec3f nml = Normalized(
              Dot(tmp, tgt) != 0.0 ? tmp - tgt * Dot(tmp, tgt) : tmp);
            Vec3f btg = Cross(nml, tgt);

            for(GLuint j = 0; j != 2; ++j) {
                const GLfloat s[2] = {-1.0f, 1.0f};
                Vec3f offs = pos + btg * s[j] * (3 + 4 * nrand()) +
                             tgt * 2 * nrand() + nml * 6 * nrand();
                ...
                p = std::copy(Data(matrix), Data(matrix) + Size(matrix), p);
            }
        }
        assert(p == matrix_data.end());

        matrix_buffer << BufferTarget::Uniform << BufferUsage::StaticDraw
                      << matrix_data;
    }
    ...
    Mat4f MakeMatrix(double t, double dt) const {
        ...
        return Transposed(
          Mat4f(Vec4f(btg, 0), Vec4f(nml, 0), Vec4f(tgt, 0), Vec4f(pos, 1)));
    }
...
private:
    static Program make() {
        ...
        fs.Source(
            ...
            "{"
            "	float l = 0.5 + "
            "sqrt(max(dot(vertLightDir,vertNormal)+0.1,0.0));"
...
public:
    FogBuffers() {
        Texture::Active(1);
        noise << Texture::Target::Rectangle << TextureMinFilter::Linear
              << TextureMagFilter::Linear << TextureWrap::ClampToEdge;

        Texture::Active(2);
        color << Texture::Target::Rectangle << TextureMinFilter::Linear
              << TextureMagFilter::Linear << TextureWrap::ClampToEdge;

        Texture::Active(3);
        depth << Texture::Target::Rectangle << TextureMinFilter::Linear
              << TextureMagFilter::Linear << TextureWrap::ClampToEdge;

        sky_fbo << Framebuffer::Target::Draw << FramebufferAttachment::Color
                << noise;

        fog_fbo << Framebuffer::Target::Draw << FramebufferAttachment::Color
                << color << FramebufferAttachment::Depth << depth;
    }
...
public:
    FogExample()
      ...
      , torus(
          ...
          draw_prog)
      , screen(List("Position")("TexCoord").Get(), shapes::Screen(), fog_prog) {
        sky_prog.Use();
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        sky_prog.projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(75), width, height, 1, 2000));

        draw_prog.Use();
        draw_prog.projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(75), width, height, 1, 200));
