...
private:
    ...
    bool SpawnParticle(
      double time, Vec3f& position, Vec3f& direction, float& age, int& id) {
...
public:
    ...
    void Update(ExampleClock& clock) {
        ...
        while(
          SpawnParticle(clock.Now().Seconds(), position, direction, age, id)) {
    ...
    void Upload(
      std::vector<Vec3f>& pos, std::vector<float>& age, std::vector<int>& id) {
...
public:
    SmokeExample(void)
      ...
      , light_cam_pos(prog, "LightCamPos") {
        ...
        gl.Bound(Texture::Target::_2D, smoke_tex)
          ...
          .Image2D(images::Cloud2D(
            images::Cloud(128, 128, 128, Vec3f(0.1f, -0.5f, 0.3f), 0.5f)))
          .GenerateMipmap();
        //
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(75), float(width) / height, 1, 100));
    }
