            ...
            for(GLuint e = 0; e != ne; ++e) {
                ...
                if(na == 1) {
                    ...
                    for(GLuint a = 0; a != na; ++a) {
                        float azim =
                          float(si[s] * ao[e] + a * a_step * math::TwoPi());
...
public:
    SphereExample()
      ...
      , hole_diameter(0.30f) {
        ...
        prog_tfb.TransformFeedbackVaryings(
          1, &var_name, TransformFeedbackMode::InterleavedAttribs);
        prog_tfb.Link();
        prog_tfb.Use();
        ...
        transf_hole_verts.BindBase(Buffer::IndexedTarget::TransformFeedback, 0);
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        Uniform<Mat4f>(prog, "ProjectionMatrix")
          .Set(CamMatrixf::PerspectiveX(
            Degrees(75), float(width) / height, 1, 20));
    }
    ...
    void Render(double time) {
        ...
        // model matrix
        auto model = ModelMatrixf::Translation(
                       0.0f, GLfloat(sqrt(1.0f + SineWave(time / 2.0))), 0.0f) *
        ...
        BufferTypedMap<GLfloat> transf_hole_verts_map(
          Buffer::Target::TransformFeedback, BufferMapAccess::Read);
        // use the values stored in the buffer as the input
        // for the fragment shader, that will use them to
        // calculate the bump map
        Uniform<Vec3f>(prog, "TransfHole")
          .SetValues(
            transf_hole_verts_map.Count() * 3, transf_hole_verts_map.Data());
