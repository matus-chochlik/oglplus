...
class FBTexThread : public ExampleThread {
private:
    Context gl;
...
public:
    FBTexThread(FBTexExample& example)
      ...
      , parent_ready(example.parent_ready) {
        ...
        Texture::Active(0);
        example.tex << TextureTarget::_2D << TextureFilter::Linear
                    << TextureWrap::Repeat
                    << images::ImageSpec(
                         tex_side,
                         tex_side,
        ...
        rbo << RenderbufferTarget::Renderbuffer
            << images::ImageSpec(
                 tex_side, tex_side, PixelDataInternalFormat::StencilIndex8);

        fbo << FramebufferTarget::Draw << FramebufferAttachment::Color
            << example.tex << FramebufferAttachment::Stencil << rbo;
        ...
        dsa.ProjectionMatrix().LoadIdentity().Ortho(
          0, tex_side, 0, tex_side, -1, 1);
    ...
    void Render(double time) {
        ...
        GLfloat shape_color_gen_coeffs[9] = {
          0.0f, 0.0f, 0.2f, 0.2f, 0.0f, 0.2f, 0.0f, 0.0f, 0.2f};
        npr.ColorGen(
          PathNVColor::Primary,
        ...
        GLfloat text_color_gen_coeffs[9] = {
          -0.3f, 0.0f, 0.3f, 0.0f, 0.2f, 0.6f, 0.3f, 0.0f, 0.0f};
        npr.ColorGen(
          PathNVColor::Primary,
          PathNVGenMode::ObjectBoundingBox,
          PathNVColorFormat::RGB,
        ...
        // Time
        dsa.ModelviewMatrix().LoadIdentity().Translate(border, border, 0);
        text << "Time: " << std::setw(7) << std::setprecision(2) << std::fixed
             << time << "[s]";
        RenderText(text.str());
        text.str(std::string());
        ...
        text << "Frame: " << std::setw(7) << std::fixed << frame_no;
        RenderText(text.str());
        text.str(std::string());
        ...
        if(time < 1)
            ...
            text << std::setw(7) << std::fixed << frame_no / time;
        RenderText(text.str());
        text.str(std::string());
...
std::unique_ptr<ExampleThread> makeExampleThread(
  Example& example, unsigned thread_id, const ExampleParams& /*params*/
    ...
    return std::unique_ptr<ExampleThread>(
      new FBTexThread(dynamic_cast<FBTexExample&>(example)));
}
