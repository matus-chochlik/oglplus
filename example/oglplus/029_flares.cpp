 ...
 *  @brief Shows how to use queries to influence rendering of effects like
 * flares
 *
...
public:
    FlareExample(void)
      ...
      , queries(n_flares) {
        ...
        metal_texture << TextureTarget::_2D
                      << TextureMinFilter::LinearMipmapLinear
                      << TextureMagFilter::Linear << TextureWrap::Repeat
                      << images::BrushedMetalUByte(
                           512, 512, 5120, -12, +12, 32, 64)
                      << TextureMipmap();

        Texture::Active(1);
        UniformSampler(flare_prog, "FlareTex").Set(1);
        flare_texture << TextureTarget::_2D
                      << TextureMinFilter::LinearMipmapLinear
                      << TextureMagFilter::Linear
                      << images::LoadTexture("flare_1") << TextureMipmap();
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        auto projection =
          CamMatrixf::PerspectiveX(Degrees(65), float(width) / height, 1, 20);
