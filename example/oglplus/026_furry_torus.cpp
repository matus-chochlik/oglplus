...
class FurVertShader : public VertexShader {
public:
    FurVertShader()
      : VertexShader(
...
class FurGeomShader : public GeometryShader {
public:
    FurGeomShader()
      : GeometryShader(
...
class FurFragShader : public FragmentShader {
public:
    FurFragShader()
      : FragmentShader(
    ...
    Fur(const Program& prog)
      ...
      , hair_count(96 * 1024) {
        ...
        for(GLuint h = 0; h != hair_count; ++h) {
            ...
            GLfloat v =
              z + GLfloat(std::pow(std::sin(2.0 * M_PI * z) / (4.0 * M_PI), 3));
...
class TorusVertShader : public VertexShader {
public:
    TorusVertShader()
      : VertexShader(
...
class TorusFragShader : public FragmentShader {
public:
    TorusFragShader()
      : FragmentShader(
...
public:
    FurExample()
      ...
      , curr_vel(0.0) {
        Texture::Active(0);
        fur_tex << Texture::Target::_2D << TextureMinFilter::LinearMipmapLinear
                << TextureMagFilter::Linear << TextureWrap::Repeat
                << images::LoadTexture("zebra_fur") << TextureMipmap();
    ...
    void Render(ExampleClock& clock) {
        ...
        switch(int(0.25 * clock.Time()) % 8) {
            case 0:
                accel += (clock.Interval().Seconds()) * 0.01;
                break;
            case 4:
                accel -= (clock.Interval().Seconds()) * 0.01;
                break;
