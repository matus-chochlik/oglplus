...
class NoiseTexture : public DSATextureEXT {
public:
...
class CloudTexture : public DSATextureEXT {
public:
    const GLuint tex_unit;
    ...
    CloudTexture(GLuint unit)
      : tex_unit(unit) {
        ...
        this->BindMulti(tex_unit);
        this->Image2D(
          images::Cloud2D(images::Cloud(128, 128, 128, Vec3f(), 0.5f)));
        this->GenerateMipmap();
...
private:
    static Program make() {
        ...
        const GLchar* variable_names[2] = {
          "xfbPositionAndId", "xfbVelocityAndAge"};
        prog.TransformFeedbackVaryings(
          variable_names, TransformFeedbackMode::SeparateAttribs);
...
private:
    static Program make() {
        ...
        gs.Source(
            ...
            "		geomGFTCoord = OcclMapMatrix * InvCameraMatrix * "
            "gl_Position;"
...
public:
    ...
    void Render(const Mat4f& projection, const Mat4f& camera, double time) {
        ...
        const auto drawing_driver = [&drawing_fan,
                                     &mm_nomotion,
                                     &mm_rotation,
                                     &pmodel_matrix](GLuint phase) -> bool {
            if(phase == drawing_fan)
                pmodel_matrix->Set(mm_rotation);
            else
                pmodel_matrix->Set(mm_nomotion);
            return true;
        };
...
public:
    ...
    OcclusionMap(GLuint unit, GLsizei side)
      ...
      , camera(CamMatrixf::LookingAt(Vec3f(0, 0, -7), Vec3f())) {
        tex << (tex_unit | TextureTarget::_2D) << TextureFilter::Nearest
        ...
        fbo << FramebufferTarget::Draw << FramebufferAttachment::Depth << tex
            << FramebufferAttachment::Color << rbo << FramebufferComplete();
    }
...
public:
    ...
    void Update(double time_diff, GLuint frame_no) {
        ...
        pp_vao[vao_i].Bind();
        pos_and_id_buf[xfb_i].BindBase(
          Buffer::IndexedTarget::TransformFeedback, 0);
        vel_and_age_buf[xfb_i].BindBase(
          Buffer::IndexedTarget::TransformFeedback, 1);
...
public:
    ParticlesExample()
      ...
      , screen(
          List("Position")("TexCoord").Get(), shapes::Screen(), screen_prog)
      , prev_time(0.0)
      , frame_no(0)
      , width(400)
      , height(300) {
        ...
        depth_rbo << RenderbufferTarget::Renderbuffer;

        Reshape(width, height);

        geom_fbo << FramebufferTarget::Draw << FramebufferAttachment::Color
                 << geom_tex << FramebufferAttachment::Depth << depth_rbo
                 << FramebufferComplete();

        volm_fbo << FramebufferTarget::Draw << FramebufferAttachment::Color
                 << volm_tex << FramebufferAttachment::Depth << depth_rbo
                 << FramebufferComplete();
    ...
    void Reshape(GLuint vp_width, GLuint vp_height) {
        ...
        projection =
          CamMatrixf::PerspectiveX(Degrees(60), width, height, 1, 100);

        geom_tex << images::ImageSpec(
          width,
          height,
          Format::RGB,
          InternalFormat::RGB,
        ...
        volm_tex << images::ImageSpec(
          width, height, Format::RG, InternalFormat::RG32F, DataType::Float);

        depth_rbo << images::ImageSpec(
          width, height, InternalFormat::DepthComponent32);
    }
    ...
    void Render(double time) {
        ...
        auto camera =
          CamMatrixf::Roll(
            Degrees(SineWave(time / 11.0) * 9 + SineWave(time / 13.0) * 7)) *
          CamMatrixf::Orbiting(
            Vec3f(),
            40.0f,
            Degrees(
              SineWave(time / 23.0) * 35 + CosineWave(time / 31.0) * 45 - 90),
            Degrees(SineWave(time / 19.0) * 35 + SineWave(time / 26.0) * 45));
        ...
        steam.Render(
          projection, camera, occl_map.projection * occl_map.camera, frame_no);
