...
void example_thread_main(ExampleThreadData& data) {
    ...
    bool debugging = true;
    bool compatibility = common.example_params.compat_context_threads.count(
                           data.thread_index) != 0;

    glx::Context ctx(display, fbc, common.ctx, 3, 3, debugging, compatibility);
    ...
    if(common.failure) {
        ...
        try {
            ...
            std::unique_ptr<ExampleThread> example_thread(makeExampleThread(
              *common.example, data.thread_index, common.example_params));
            ...
            while(!common.done && !common.failure) {
                ...
                double comp = 0.0;
                do {
                    comp = example_thread->RenderPart(part_no++, common.clock);
                    glFlush();
                } while(comp < 1.0);
            }
...
void do_run_example_loop(
  ...
  ExampleClock& clock,
  ExampleOptions& opts) {
    win.SelectInput(StructureNotifyMask | PointerMotionMask | KeyPressMask);
                ...
                switch(event.type) {
                    ...
                    case MotionNotify:
                        example->MouseMove(
                          ...
                          opts.height - GLuint(event.xmotion.y),
                          opts.width,
                          opts.height);
                        break;
                    case KeyPress:
  ...
  ExampleOptions& opts) {
    ...
    if(dbg.Available()) {
        ...
        dbg.InsertMessage(
          ...
          DebugOutputARBSeverity::Low,
          "Starting main loop");
        do_run_example_loop(display, win, ctx, example, common, clock, opts);
    } else
...
void run_framedump_loop(
  ...
  const ExampleOptions& opts) {
    ...
    if(std::strcmp(opts.framedump_prefix, txtbuf.data()) != 0)
        return;
    ...
    while(!done) {
        ...
        if(mouse_y < 0.0f)
            mouse_y = 0.0f;
        if(mouse_x > opts.width)
            mouse_x = opts.width;
        if(mouse_y > opts.height)
            mouse_y = opts.height;

        example->MouseMove(
          GLuint(mouse_x), GLuint(mouse_y), opts.width, opts.height);
        ...
        filename << opts.framedump_prefix << std::setfill('0') << std::setw(6)
                 << frame_no << ".rgba";
        {
        ...
        if(
          std::strncmp(filename.str().c_str(), txtbuf.data(), txtbuf.size()) !=
          0)
            break;
    }
    while(display.NextEvent(event))
...
void run_example(
  const x11::Display& display, ExampleOptions& opts, int argc, char** argv) {
    ...
    params.num_gpus =
      unsigned(screen_names.size()); // TODO: something more reliable
    setupExample(params);
    params.Check();
    ...
    try {
        ...
        for(unsigned t = 0; t != params.num_threads; ++t) {
            ExampleThreadData example_thread_data = {
              t, nullptr, std::string(), &example_thread_common_data};
        ...
        for(unsigned t = 0; t != params.num_threads; ++t) {
            threads.emplace_back(
              call_example_thread_main, std::ref(thread_data[t]));
            ...
            if(!thread_data[t].error_message.empty()) {
                ...
                throw std::runtime_error(thread_data[t].error_message);
            }
        }
        ...
        example->Reshape(opts.width, opts.height);
        example->MouseMove(
          opts.width / 2, opts.height / 2, opts.width, opts.height);

        if(opts.screenshot_path) {
            make_screenshot(display, win, ctx, example, clock, opts);
        } else if(opts.framedump_prefix) {
            run_framedump_loop(display, win, ctx, example, clock, opts);
        } else {
            run_example_loop(
              display,
              win,
...
int glx_example_main(int argc, char** argv) {
    ...
    while(a < argc) {
        ...
        if((std::strcmp(argv[a], "--screenshot")) == 0 && (a + 1 < argc)) {
          ...
          (std::strcmp(argv[a], "--frame-dump")) == 0 && (a + 1 < argc)) {
