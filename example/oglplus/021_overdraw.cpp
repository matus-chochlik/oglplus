...
public:
    CubeExample(void)
      ...
      , cube(List("Position").Get(), shapes::Cube(), draw_prog) {
        ...
        cube_pos << BufferIndexedTarget::Uniform << 0 << BufferUsage::StaticDraw
                 << OffsetData(n);

        ProgramUniformSampler(screen_prog, "Palette").Set(0);
        Texture::Active(0);
        palette << Texture::Target::_1D << TextureFilter::Nearest
        ...
        Texture::Active(1);
        tex
          << Texture::Target::Rectangle << TextureMinFilter::Nearest
          << TextureMagFilter::Nearest << TextureWrap::ClampToEdge
          << images::ImageSpec(
               64, 64, Format::Red, InternalFormat::R8, DataType::UnsignedByte);
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        draw_prog.projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(70), width, height, 1, 100));
    ...
    void RenderOffscreen(double time) {
        ...
        draw_prog.Use();
        draw_prog.camera_matrix.Set(CamMatrixf::Orbiting(
          Vec3f(),
          27.0,
          Degrees(time * 23),
