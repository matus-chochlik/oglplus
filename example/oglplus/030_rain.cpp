...
class RippleTexProg : public Program {
private:
    static Program make() {
        Program prog;
...
public:
    RippleTexHolder(GLuint ripple_tex_size, GLuint tex_unit)
      ...
      , curr(0) {
        ...
        for(GLuint i = 0; i != nhm; ++i) {
            ...
            height_maps[i] << Texture::Target::_2D << TextureFilter::Nearest
                           << TextureWrap::Repeat
                           << images::ImageSpec(
                                ripple_tex_size,
                                ripple_tex_size,
        ...
        bump_map << Texture::Target::_2D << TextureFilter::Linear
                 << TextureWrap::Repeat
                 << images::ImageSpec(
                      ripple_tex_size,
                      ripple_tex_size,
    ...
    GLuint TexUnit1() const {
        return first_tex_unit + (curr + 0) % nhm;
    }

    GLuint TexUnit2() const {
...
public:
    RippleGenerator(GLuint first_tex_unit)
      ...
      , holder(size, first_tex_unit) {
        ...
        Framebuffer::AttachColorTexture(
          Framebuffer::Target::Draw, 0, holder.BumpMap(), 0);
    ...
    void Update() {
        ...
        fbo.Bind(Framebuffer::Target::Draw);
        Framebuffer::AttachColorTexture(
          Framebuffer::Target::Draw, 1, holder.CurrentHeightMap(), 0);
        Context::ColorBuffer draw_buffs[2] = {
          FramebufferColorAttachment::_0, FramebufferColorAttachment::_1};
...
class WaterProg : public Program {
private:
    static Program make() {
        Program prog;
        prog.AttachShader(VertexShader(
        ...
        prog.AttachShader(FragmentShader(
          ...
          "	float Specular = clamp(pow(dot(fragViewDir, fragLightRefl)+0.1, "
          "64.0), 0.0, 2.0);"
...
public:
    EnvMap(GLuint tex_unit)
      : _tex_unit(tex_unit) {
        ...
        *this << Texture::Target::CubeMap << TextureFilter::Linear
...
public:
    ...
    void Reshape(GLuint vp_width, GLuint vp_height) {
        ...
        water_prog.projection_matrix.Set(CamMatrixf::PerspectiveX(
          Degrees(60), float(width) / float(height), 1.0, 100.0));
    }
