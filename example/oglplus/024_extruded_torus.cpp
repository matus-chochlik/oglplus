...
private:
    ...
    Program transf_prog;
    static Program make_transf_prog() {
        VertexShader vs;
        vs.Source(
          "#version 150\n"
        ...
        gs.Source(
          ...
          "		vertTexCoord[2] "
          "	);"
          "	float Offs = (sin((FaceCoord.s + Time/10.0)* 3.14 * 2.0 * 10)*0.5 "
          "+ 0.5)*0.4;"
    ...
    Program frame_prog;
    static Program make_frame_prog() {
        FragmentShader fs;
        fs.Source(
          "#version 150\n"
          "out vec4 fragColor;"
...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        projection_matrix =
          CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 30);
    }
    ...
    void Render(double time) {
        ...
        auto model = ModelMatrixf::RotationY(FullCircles(time * 0.25)) *
