...
public:
    ...
    void RenderInfo(
      ...
      int inst_id) {
        ...
        ss << std::setw(5) << std::setprecision(1) << std::fixed << dist
           << " [m]";
        s = ss.str();
    ...
    void FinishTracking(const text::Font2D& font, HUDOverlay& hud_overlay) {
        ...
        for(GLuint i = 0; i != count; ++i) {
            ...
            hud_overlay.RenderInfo(
              font, ndc_x, ndc_y, ndc_z, dist, name, inst_id);
        }
    }
...
private:
    ...
    void AddInstancedObject(
      ...
      GLuint count) {
        ...
        objects.push_back(InstancedShape(
          shape_name,
          point,
          shapes::ShapeWrapper(vert_attrs, shape_builder, draw_prog),
          count));
    }
...
public:
    ObjectTrackingExample()
      : gl()
      , path()
      , point(List("Position").Get(), shapes::PointAndVector(), track_prog)
      , screen(
          List("Position")("TexCoord").Get(), shapes::Screen(), overlay_prog)
      , overlay_tex_unit(0)
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        auto projection =
          CamMatrixf::PerspectiveX(Degrees(75), float(width) / height, 1, 1000);
