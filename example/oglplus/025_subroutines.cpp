...
class CubeVertShader : public VertexShader {
public:
    CubeVertShader()
      : VertexShader(
...
class CubeFragShader : public FragmentShader {
public:
    CubeFragShader()
      : FragmentShader(
...
public:
    ...
    CubeProgram()
      ...
      , frag_shiny_checker(frag_subroutines.Assign(pixel_light_func, shiny)
                             .Assign(pixel_color_func, checker)
                             .Save())
      , frag_shiny_texture(
          frag_subroutines.Assign(pixel_color_func, texture_rgb).Save())
      , frag_shiny_strips(
          frag_subroutines.Assign(pixel_color_func, strips).Save())
      , frag_shiny_spiral(
          frag_subroutines.Assign(pixel_color_func, spiral).Save()) {}
};
...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        cube_prog.projection_matrix =
          CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 25);
    }
    ...
    void Render(double time) {
        ...
        cube_prog.frag_subroutines
          .Assign(cube_prog.pixel_light_func, cube_prog.dull)
          .Assign(cube_prog.pixel_color_func, cube_prog.strips)
          .Apply();
        ...
        // dull textured cube
        cube_prog.frag_subroutines
          .Assign(cube_prog.pixel_color_func, cube_prog.texture_bgr)
          .Apply();
