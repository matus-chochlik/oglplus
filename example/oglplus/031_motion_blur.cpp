...
public:
    ...
    Instances(GLuint n = 256)
      ...
      , path_nml(make_normals(), 0.25) {
        ...
        Buffer::Data(
          Buffer::Target::Uniform, matrix_data, BufferUsage::StaticDraw);
    }
...
private:
    static Program make() {
        ...
        fs.Source(
            ...
            "	float l = pow(max(dot(vertLightDir, "
            "vertNormal)+0.1,0.0)*1.6,2.0);"
            "	fragColor = mix("
            "		vec3(0, 0, 0),"
            "		vertColor,"
            "		mix(c, 1.0, 0.2)*(l+0.3)"
...
public:
    MotionBlurBuffers()
      : tex(2) {
        ...
        Framebuffer::AttachTexture(
          fbo_tgt, FramebufferAttachment::Color, tex[0], 0);

        Renderbuffer::Target rbo_tgt = Renderbuffer::Target::Renderbuffer;
        rbo.Bind(rbo_tgt);
        Framebuffer::AttachRenderbuffer(
          fbo_tgt, FramebufferAttachment::Depth, rbo);
    }
    ...
    void Resize(GLuint new_width, GLuint new_height) {
        ...
        Renderbuffer::Storage(
          rbo_tgt, PixelDataInternalFormat::DepthComponent, width, height);
    }
...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        auto projection =
          CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 300);
    ...
    void Render(ExampleClock& clock) {
        ...
        for(int s = 0; s != samples; ++s) {
            ...
            draw_prog.Use();
            draw_prog.camera_matrix.Set(CamMatrixf::LookingAt(
            ...
            draw_prog.model_matrix.Set(instances.MakeMatrix(
              pos - 0.007 + SineWave(time / 13.0) * 0.007, 0.001));
            arrow.Draw();

            draw_prog.model_matrix.Set(instances.MakeMatrix(
              pos + 0.013 + SineWave(time / 7.0) * 0.007, 0.001));
            arrow.Draw();
