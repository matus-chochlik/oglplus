...
private:
    ...
    static std::vector<Vec3f> make_positions() {
        const Vec3f _positions[3] = {Vec3f(-1.4f, -0.3f, -0.7f),
    ...
    static std::vector<GLfloat> make_sizes() {
    ...
    static std::vector<Vec3f> make_light_path_cps() {
        const Vec3f _cps[6] = {Vec3f(-3.0f, -3.0f, -3.0f),
...
public:
    CloudExample(const ExampleParams& params)
      ...
      , light_path(make_light_path_cps()) {
        ...
        for(std::size_t i = 0, n = positions.size(); i != n; ++i) {
            cloud_tex[i] << Texture::Target::_3D
                         << images::Cloud(
                              128, 128, 128, Vec3f(0.1f, -0.5f, 0.3f), 0.5f)
                         << TextureMipmap()
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        Mat4f perspective =
          CamMatrixf::PerspectiveX(Degrees(65), width, height, 1, 40);
    ...
    void Render(double time) {
        ...
        auto cameraMatrix = CamMatrixf::Orbiting(
          Vec3f(), 4.5f, Degrees(0), Degrees(SineWave(time / 20.0) * 80));
        ...
        for(std::size_t i = 0, n = positions.size(); i != n; ++i) {
            ...
            gl.DrawArraysInstanced(PrimitiveType::Points, GLint(i), 1, samples);
        }
    }
