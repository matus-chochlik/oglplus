...
private:
    ...
    static std::vector<shapes::Plane> make_plane_builders(void) {
        ...
        result.push_back(
          shapes::Plane(Vec3f(2.0f, 0.0f, 0.0f), Vec3f(0.0f, 0.0f, -2.0f)));
        result.push_back(
          shapes::Plane(Vec3f(2.0f, 0.0f, 0.0f), Vec3f(0.0f, 2.0f, 0.0f)));
        return result;
    }
    ...
    Lazy<ProgramUniform<Mat4f>> plane_camera_matrix, torus_camera_matrix,
      torus_model_matrix;
