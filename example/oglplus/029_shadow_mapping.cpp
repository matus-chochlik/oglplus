...
private:
    ...
    static std::vector<Vec3f> MakeCubeOffsets(
      GLfloat distance, GLint cubes_per_side) {
...
public:
    ShadowMapExample(const ExampleParams& params)
      ...
      , light_paths(
          List(CubicBezierLoop<Vec3f, double>(
            List(Vec3f(-9.0f, 5.0f, 8.0f))(Vec3f(0.0f, 6.0f, -9.0f))(
              Vec3f(9.0f, 5.0f, 9.0f))(Vec3f(0.0f, 15.0f, 0.0f))
              .Get()))(
            CubicBezierLoop<Vec3f, double>(
              List(Vec3f(-8.0f, 5.0f, 9.0f))(Vec3f(-9.0f, 9.0f, -9.0f))(
                Vec3f(9.0f, 8.0f, -9.0f))(Vec3f(9.0f, 4.0f, 9.0f))
                .Get()))(
            CubicBezierLoop<Vec3f, double>(
              List(Vec3f(-9.0f, 7.0f, 9.0f))(Vec3f(9.0f, 5.0f, 9.0f))(
                Vec3f(9.0f, 5.0f, -9.0f))(Vec3f(-9.0f, 8.0f, -9.0f))
                .Get()))
            .AsVector())
      , light_positions(light_paths.size())
      , light_proj_matrices(light_paths.size())
      , light_colors(List(Vec3f(1.0f, 0.1f, 0.01f))(Vec3f(0.1f, 1.0f, 0.1f))(
      ...
      , fbo(light_paths.size()) {
        ...
        draw_fs.Source(List("#version 150\n")(max_lights_def.str().c_str())(
        ...
        for(std::size_t i = 0, n = light_paths.size(); i != n; ++i) {
            ...
            gl.Bound(Framebuffer::Target::Draw, fbo[i])
              .AttachTexture(FramebufferAttachment::Depth, smap[i], 0);
        }
    ...
    void Render(double time) {
        ...
        const auto light_persp_matrix =
          CamMatrixf::PerspectiveX(Degrees(70), 1.0, 1.0, 80.0);
        ...
        for(std::size_t i = 0, n = light_positions.size(); i != n; ++i) {
            light_positions[i] = light_paths[i].Position((i + 1) * time / 12.0);

            const auto light_matrix =
              CamMatrixf::LookingAt(light_positions[i], Vec3f());

            light_proj_matrices[i] = light_persp_matrix * light_matrix;
        ...
        ProgramUniform<Vec3f>(vert_prog, "LightPositions").Set(light_positions);

        ProgramUniform<Mat4f>(vert_prog, "TexProjectionMatrices")
          .Set(light_proj_matrices);

        ProgramUniform<GLint>(vert_prog, "LightCount")
          .Set(GLint(light_positions.size()));

        projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(70), float(width) / height, 1, 80));
