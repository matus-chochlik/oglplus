...
class SortNWTex : public Texture {
private:
    GLuint pass_count;
    Texture& self() {
        return *this;
...
public:
    ...
    Particles(GLuint n)
      ...
      , sort_nw(n) {
        ...
        gl.Bound(BufferTarget::Uniform, distances)
          .Data<GLfloat>(count, nullptr, BufferUsage::DynamicDraw);
        gl.Bound(BufferTarget::CopyRead, indices_f)
          .Data<GLuint>(count, nullptr, BufferUsage::DynamicRead);
        gl.Bound(BufferTarget::CopyWrite, indices_d)
          .Data<GLuint>(count, nullptr, BufferUsage::DynamicDraw);
    }
...
private:
    static Program make() {
        ...
        prog << TransformFeedbackMode::SeparateAttribs << "xfbIndex"
             << "xfbDistance";
...
public:
    ...
    void operator()(const Particles& particles) {
        ...
        gl.Use(prog);
        gl.DrawElements(
          PrimitiveType::Points, particles.Count(), DataType::UnsignedInt);
...
public:
    ...
    Mat4f ModelMat(double time) {
        return ModelMatrixf::RotationA(Vec3f(1), FullCircles(time / 13.0));
    }
