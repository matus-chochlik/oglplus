...
private:
    ...
    bool SpawnParticle(
      double time, Vec3f& position, Vec3f& direction, float& age) {
    ...
    void Update(ExampleClock& clock) {
        ...
        for(std::size_t i = 0, n = positions.size(); i != n; ++i) {
            ...
            if(ages[i] > 1.0) {
                ...
                SpawnParticle(
                  clock.Now().Seconds(), positions[i], directions[i], ages[i]);
            } else {
...
public:
    SmokeExample()
      ...
      , camera_matrix(prog, "CameraMatrix") {
        emitters.push_back(ParticleSystem(
          ListOf<Vec3f>(Vec3f(-20.0f, -10.0f, 10.0f))(
            Vec3f(20.0f, 0.0f, -20.0f))(Vec3f(20.0f, 10.0f, 20.0f))(
            Vec3f(-20.0f, 0.0f, -10.0f))
            .As<std::vector<Vec3f>>(),
          5.0,
          200.0));
        emitters.push_back(ParticleSystem(
          ListOf<Vec3f>(Vec3f(30.0f, 0.0f, 0.0f))(Vec3f(-30.0f, 0.0f, 0.0f))(
            Vec3f(-20.0f, 20.0f, 0.0f))(Vec3f(20.0f, -10.0f, 0.0f))
            .As<std::vector<Vec3f>>(),
          3.0,
          200.0));
        emitters.push_back(ParticleSystem(
          ListOf<Vec3f>(Vec3f(5.0f, 20.0f, 20.0f))(Vec3f(-5.0f, 20.0f, -20.0f))(
            Vec3f(5.0f, -20.0f, -20.0f))(Vec3f(-5.0f, -20.0f, 20.0f))
            .As<std::vector<Vec3f>>(),
          20.0,
          100.0));
        ...
        {
            Buffer::Data(
              Buffer::Target::Array, positions, BufferUsage::DynamicDraw);
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(60), float(width) / height, 1, 100));
    }
    ...
    void Render(ExampleClock& clock) {
        ...
        std::sort(
          indices.begin(), indices.end(), [&depths](GLuint i, GLuint j) {
    ...
    ExampleTimePeriod ScreenshotTime() const {
        return ExampleTimePeriod::Seconds(4.5);
    }
