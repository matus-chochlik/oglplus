...
private:
    static Program make(const VertexShader& vs) {
        ...
        fs.Source(
            ...
            "	NSampl = min(NSampl, int(min(ScreenSize.x, "
            "ScreenSize.y)*0.25));"
...
private:
    ...
    void RenderShadowMap(GLuint size) {
        ...
        Framebuffer::AttachTexture(
          fbo_tgt, FramebufferAttachment::Depth, shadow_map, 0);
        ...
        Renderbuffer::Storage(
          rbo_tgt, PixelDataInternalFormat::RGBA, size, size);
        Framebuffer::AttachRenderbuffer(
          fbo_tgt, FramebufferAttachment::Color, rbo);
    ...
    void SetupLightMask(void) {
        ...
        Framebuffer::AttachTexture(
          fbo_tgt, FramebufferAttachment::Color, light_mask, 0);
...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        projection =
          CamMatrixf::PerspectiveX(Degrees(70), width, height, 1, 200);
    ...
    void Render(double time) {
        auto camera =
          CamMatrixf::Roll(
            Degrees(SineWave(time / 11.0) * 7 + SineWave(time / 13.0) * 5)) *
          CamMatrixf::Orbiting(
            Vec3f(),
            40.0f,
            Degrees(
              SineWave(time / 11.0) * 10 + CosineWave(time / 19.0) * 10 - 90),
        ...
        auto drawing_driver = [&model_matrix,
                               &mm_identity,
                               &mm_rotation,
                               &drawing_fan](GLuint phase) -> bool {
            if(phase == drawing_fan)
                model_matrix->Set(mm_rotation);
            else
                model_matrix->Set(mm_identity);
            return true;
        };
