...
private:
    static Program make() {
        ...
        fs.Source(
          ...
          "	{"
          "		vec3 SampleCoord = "
          "((ShadowOffs[s]*f+vertShadowCoord.xyz)*inv_w) * 0.5 + 0.5;"
          ...
          "	vec3 fragNormal = Tangent*NAdj.x + Bitangent*NAdj.y + "
          "Normal*NAdj.z;"
          "	vec3 LightRefl = reflect(-LightDir, fragNormal);"

          "	float Ambi = sqrt(max(dot(fragNormal, vec3(0.0, 1.0, 0.0))+0.2, "
          "0.0))*0.6 + 0.4;"
          "	float Diff = sqrt(max(dot(fragNormal, LightDir), 0.0))*Shadow;"
          "	float Spec = pow(max(dot(LightRefl, ViewDir)+0.15, 0.0), "
          "0.5+1.5*LMap.b)*Shadow;"
...
public:
    ...
    ProgramUniform<Mat4f> projection_matrix, camera_matrix, light_matrix,
      model_matrix;
...
private:
    ...
    static shapes::ShapeWrapper load_hydrant() {
        std::ifstream input;
...
public:
    BlenderMeshExample()
      ...
      , tex(ntex)
      , smap_side(1024)
      , light_path(ListOf<Vec3f>(Vec3f(-15.0f, 10.0f, -12.5f))(
                     Vec3f(-15.0f, 14.0f, 0.5f))(Vec3f(-12.0f, 20.0f, 17.0f))(
                     Vec3f(0.0f, 14.0f, 12.0f))(Vec3f(11.0f, 11.0f, 11.5f))(
                     Vec3f(7.2f, 11.0f, -10.0f))
                     .Get())
      , camera_path(
          ListOf<Vec3f>(Vec3f(6.2f, 12.0f, -7.0f))(Vec3f(4.0f, 15.0f, 0.0f))(
            Vec3f(5.0f, 12.0f, 6.5f))(Vec3f(-5.0f, 13.0f, 7.0f))(
            Vec3f(-6.0f, 11.0f, 0.0f))(Vec3f(-4.0f, 14.0f, -6.5f))
            .Get())
      , target_path(ListOf<Vec3f>(Vec3f(3.2f, 2.0f, -0.1f))(
                      Vec3f(-3.2f, 3.0f, -0.3f))(Vec3f(0.3f, 1.0f, 1.3f))
                      .Get()) {
        const char* tex_filenames[3] = {
          "hydrant_normal", "hydrant_light", "hydrant_color"};
        ...
        }
        {
            ...
            gl.Bound(Renderbuffer::Target::Renderbuffer, smap_rbo)
              .Storage(PixelDataInternalFormat::RGBA, smap_side, smap_side);
        ...
        //
        auto light_proj_mat =
          CamMatrixf::PerspectiveX(Degrees(40), 1.0f, 1.0f, 100.0f);
        ProgramUniform<Mat4f>(shadow_prog, "LightProjMatrix")
          .Set(light_proj_mat);
    ...
    void Reshape(GLuint vp_width, GLuint vp_height) {
        ...
        draw_prog.projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(75), width, height, 1, 40));
    }
    ...
    void Render(double time) {
        ...
        auto light_matrix =
          CamMatrixf::LookingAt(light_pos, Vec3f(0.0, 2.0, 0.0));

        auto camera_matrix = CamMatrixf::LookingAt(
          camera_path.Position(time / 19.0), target_path.Position(time / 11.0));
        auto model_1 = ModelMatrixf::Translation(3.0f, 0.0f, 0.0f) *
                       ModelMatrixf::RotationY(Degrees(45));
        auto model_2 = ModelMatrixf::Translation(-4.0f, 0.0f, 0.0f) *
