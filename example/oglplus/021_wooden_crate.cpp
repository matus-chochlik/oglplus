...
public:
    ...
    void Reshape(GLuint width, GLuint height) {
        ...
        projection_matrix.Set(
          CamMatrixf::PerspectiveX(Degrees(70), float(width) / height, 1, 15));
    }
    ...
    void Render(double time) {
        ...
        light_pos.Set(Vec3f(Cos(lightAzimuth), 1.0f, Sin(lightAzimuth)) * 2.0f);
        //
        camera_matrix.Set(CamMatrixf::Orbiting(
          Vec3f(), 3.0f, Degrees(-45), Degrees(SineWave(time / 30.0) * 70)));

        // set the model matrix
        model_matrix.Set(ModelMatrixf::RotationY(FullCircles(time * 0.05)));
