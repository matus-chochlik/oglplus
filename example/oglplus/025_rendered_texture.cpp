...
private:
    ...
    Uniform<Mat4f> torus_projection_matrix, torus_camera_matrix,
      torus_model_matrix, cube_projection_matrix, cube_camera_matrix,
      cube_model_matrix;
...
public:
    FBTexExample()
      ...
      , height(tex_side) {
        ...
        rbo.Storage(
          PixelDataInternalFormat::DepthComponent, tex_side, tex_side);
    ...
    void Render(double time) {
        ...
        torus_camera_matrix.Set(CamMatrixf::Orbiting(
          Vec3f(),
          3.5,
          Degrees(time * 25),
          Degrees(SineWave(time / 30.0) * 90)));

        torus_model_matrix.Set(ModelMatrixf::RotationA(
          Vec3f(1.0f, 1.0f, 1.0f), FullCircles(time * 0.5)));
        ...
        cube_camera_matrix.Set(CamMatrixf::Orbiting(
          Vec3f(),
          3.0,
          Degrees(time * 35),
