...
void example_thread_main(ExampleThreadData& data) {
    ...
    eglplus::Surface surface = eglplus::Surface::Pbuffer(
      display, common.config, common.surface_attribs.Get());

    eglplus::BindAPI(eglplus::RenderingAPI::OpenGL);

    eglplus::Context context(
      display, common.config, common.context, common.context_attribs.Get());
...
void run_framedump_loop(
  ...
  const ExampleOptions& opts) {
    ...
    for(std::size_t p = 0; p != mouse_path_pts; ++p) {
        mouse_path_pos[p] =
          Vec2f(std::rand() % opts.width, std::rand() % opts.height);
...
void make_screenshot(
  ...
  const ExampleOptions& opts) {
    ...
    glReadPixels(
      0, 0, opts.width, opts.height, GL_RGB, GL_UNSIGNED_BYTE, pixels.data());
  ...
  char** argv) {
    eglplus::Configs configs(
      ...
      eglplus::ConfigAttribs()
        ...
        .Add(
          eglplus::ConfigAttrib::Samples, opts.samples > 0 ? opts.samples : 0)
    ...
    eglplus::SurfaceAttribs surface_attribs =
      eglplus::SurfaceAttribs()
        .Add(eglplus::SurfaceAttrib::Width, GLint(opts.width))
        .Add(eglplus::SurfaceAttrib::Height, GLint(opts.height));

    eglplus::Surface surface =
      eglplus::Surface::Pbuffer(display, config, surface_attribs.Get());
    ...
    eglplus::Context context(display, config, context_attribs.Get());
...
int egl_example_main(int argc, char** argv) {
    ...
    if(!(opts.screenshot_path || opts.framedump_prefix)) {
        std::cout << "--screenshot or --framedump option "
                     "must be specified"
                  << std::endl;
        return 1;
    }
