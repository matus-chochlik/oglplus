...
bool AppData::ParseArgs(int argc, char** argv) {
    ...
    // verbose
    auto& verbose_opt =
      parser.AddOpt("-v", "--verbose", verbose)
        .AddDesc(
    ...
    parser.AddArg("-vl", "--verbosity", verbosity)
      .AddDesc("Sets the verbosity level to the specified value.");
    ...
    parser.AddArg("-w", "--raytrace-width", raytrace_width)
      .AddVar(render_width)
      .AddDesc("Sets the width of the raytrace output image.");

    // output height
    parser.AddArg("-h", "--raytrace-height", raytrace_height)
      .AddVar(render_height)
      .AddDesc("Sets the height of the raytrace output image.");
    ...
    parser.AddArg("-cc", "--cloud-count", cloud_count)
      .SetMax(1024)
      .AddDesc("Sets the number of clouds that should be generated.");
    ...
    parser.AddArg("-lx", "--light-x", light_x)
      .AddDesc("The X coordinate value of the light source.");

    // light y coord
    parser.AddArg("-ly", "--light-y", light_y)
      .AddDesc("The Y coordinate value of the light source.");

    // light z coord
    parser.AddArg("-lz", "--light-z", light_z)
      .AddDesc("The Z coordinate value of the light source.");

    // high light
    parser.AddArg("-hl", "--high-light", high_light)
      .AddDesc("The high light value.");

    // ambient light
    parser.AddArg("-al", "--ambi-light", ambi_light)
      .AddDesc("The ambient (lowest) light value.");

    // crepuscular ray samples
    parser.AddArg("-crs", "--crep-ray-sam", crep_ray_sam)
      .AddDesc("Number of samples for crepuscular rays.");
    ...
    parser.AddArg("-o", "--output-prefix", output_prefix)
      .AddDesc("The path prefix for the cube map image output files.");

    // output file suffix
    parser.AddArg("-s", "--output-suffix", output_suffix)
      .AddDesc("The suffix for the cube map image output files.");

    // the X+ face id
    parser.AddArg("-of0", "--output-name-0", output_face_id[0])
      .AddDesc("The name of the positive X cube map image output file.");

    // the X- face id
    parser.AddArg("-of1", "--output-name-1", output_face_id[1])
      .AddDesc("The name of the negative X cube map image output file.");

    // the Y+ face id
    parser.AddArg("-of2", "--output-name-2", output_face_id[2])
      .AddDesc("The name of the positive Y cube map image output file.");

    // the Y- face id
    parser.AddArg("-of3", "--output-name-3", output_face_id[3])
      .AddDesc("The name of the negative Y cube map image output file.");

    // the Z+ face id
    parser.AddArg("-of4", "--output-name-4", output_face_id[4])
      .AddDesc("The name of the positive Z cube map image output file.");

    // the Z- face id
    parser.AddArg("-of5", "--output-name-5", output_face_id[5])
      .AddDesc("The name of the negative Z cube map image output file.");
    ...
    auto& single_face_arg =
      parser.AddArg("-f", "--single-face", single_face)
        .AddDesc(
...
void AppData::LogInfo() const {
    if(verbosity > 0) {
        if(render_offscreen) {
            logstr() << "Rendering offscreen" << std::endl;
        }
    }
    if(verbosity > 3) {
        logstr() << "Raytrace width: " << raytrace_width << std::endl;
        logstr() << "Raytrace height: " << raytrace_height << std::endl;
        logstr() << "Raytrace columns: " << cols() << std::endl;
        logstr() << "Raytrace rows: " << rows() << std::endl;
        logstr() << "Raytrace tiles: " << tiles() << std::endl;
    }
    if(verbosity > 1) {
        logstr() << "Random generator seed: " << rand_seed << std::endl;
    }
}
