...
images::Image CloudVolume::Load(const AppData& app_data) {
    ...
    if(cip.rfind(".r8") == cip.size() - 3) {
        ...
        try {
            ...
            storage.resize(
              app_data.cloud_res * app_data.cloud_res * app_data.cloud_res);
...
void CloudVolume::Save(const AppData& app_data) {
    ...
    try {
        output.write((const char*)image.RawData(), image.DataSize());
    } catch(std::ifstream::failure& f) {
        std::string message;
...
CloudTexture::CloudTexture(
  const AppData&, const CloudVolume& cloud_vol, ResourceAllocator& alloc)
  : tex_unit(alloc.GetNextTexUnit()) {
    Texture::Active(tex_unit);

    *this << TextureTarget::_3D << TextureWrap::ClampToBorder
          << TextureFilter::Linear << cloud_vol.image;
}

void CloudTexture::Use() {
    Texture::Active(tex_unit);
