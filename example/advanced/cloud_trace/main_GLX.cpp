    ...
    void SkipFaces(const AppData& app_data) {
        while((face < 6) && (app_data.skip_face[face])) {
            if(app_data.verbosity > 2) {
                app_data.logstr() << "Skipping cube face " << face << std::endl;
            }
            ++face;
        }
    }
...
public:
    ...
    void LogProgress(const AppData& app_data) {
        ...
        app_data.logstr() << "Rendering face tile " << tile << " out of "
                          << app_data.tiles() << " (" << (prom / 10) << "."
                          << (prom % 10) << " %)" << std::endl;
    }
};
...
void thread_loop(
  ...
  glx::Context& context) {
    ...
    RaytraceCopier::Params rtc_params(display, context, common.context);
        ...
        while(common.NextFaceTile(tile)) {
            ...
            if(bl_begin + bl_interval < now) {
                ...
                for(unsigned bl_tile : backlog) {
                    rt_copier.Copy(app_data, rtc_params, raytracer, bl_tile);
                }
                lock.unlock();
                backlog.clear();
                bl_begin = now;
  ...
  std::size_t n_threads) {
    ...
    while(!common.Done()) {
        if(app_data.verbosity > 0) {
            app_data.logstr()
              << "Rendering cube face " << common.Face() << std::endl;
        }
...
void window_loop(
  ...
  std::size_t n_threads) {
    ...
    while(!common.Done()) {
        ...
        if(app_data.verbosity > 1) {
            app_data.logstr()
              << "Finished cube face " << common.Face() << std::endl;
        }
...
void main_thread(
  AppData& app_data, CommonData& common, const std::string& screen_name) {
    ...
    if(!common.Done()) {
        try {
            thread_loop(app_data, common, display, context);
        } catch(...) {
...
void call_drawable_loop(
  ...
  DrawLoop drawable_loop) {
    ...
    CommonData common(app_data, display, context, rt_data, rt_target);
    ...
    try {
        ...
        for(auto& param : app_data.raytracer_params) {
            if(app_data.verbosity > 2) {
                app_data.logstr() << "Spawning raytracing thread on ";
                if(param.empty()) {
                    app_data.logstr() << "the current X screen";
                } else {
                    app_data.logstr() << "X screen " << param;
                }
    ...
    try {
        ...
        drawable_loop(drawable, app_data, common, rt_target, threads.size());
        for(auto& t : threads)
            t.join();
    } catch(...) {
        common.Stop();
...
int main_GLX(AppData& app_data) {
    ...
    if(app_data.render_offscreen) {
        const int pbuffer_attribs[] = {GLX_PBUFFER_WIDTH,
                                       int(app_data.render_width),
                                       GLX_PBUFFER_HEIGHT,
                                       int(app_data.render_height),
                                       None};
        ...
        call_drawable_loop(app_data, display, context, pbuffer, pbuffer_loop);
    } else {
        x11::Window window(
          display,
          vi,
        ...
        call_drawable_loop(app_data, display, context, window, window_loop);
    }
