...
template <typename T>
void ParseAndSetValue(const char* name, const char* str, std::vector<T*> vars) {
    T value;
...
void ParseAndSetValue(
  const char* name, const char* str, std::vector<std::vector<T>*>& vars) {
    T value;
    ParseValue(name, str, value);
...
bool ArgInfo<bool>::Parse(int& arg, int, char** argv) {
    bool result = (std::strcmp(argv[arg], long_name.c_str()) == 0) ||
...
ArgInfo<bool>& ArgParser::AddOpt(
  std::string&& short_name, std::string&& long_name, bool& var) {
    options.push_back(
      ArgInfo<bool>(std::move(short_name), std::move(long_name), var));
    all_arg_refs.push_back(&options.back());
    return options.back();
}

ArgInfo<unsigned>& ArgParser::AddArg(
  std::string&& short_name, std::string&& long_name, unsigned& var) {
    uint_args.push_back(
      ArgInfo<unsigned>(std::move(short_name), std::move(long_name), var));
    all_arg_refs.push_back(&uint_args.back());
    return uint_args.back();
}

ArgInfo<float>& ArgParser::AddArg(
  std::string&& short_name, std::string&& long_name, float& var) {
    float_args.push_back(
      ArgInfo<float>(std::move(short_name), std::move(long_name), var));
    all_arg_refs.push_back(&float_args.back());
    return float_args.back();
}

ArgInfo<std::string>& ArgParser::AddArg(
  std::string&& short_name, std::string&& long_name, std::string& var) {
    str_args.push_back(
      ArgInfo<std::string>(std::move(short_name), std::move(long_name), var));
...
ArgInfo<std::vector<std::string>>& ArgParser::AddArg(
  ...
  std::vector<std::string>& var) {
    str_list_args.push_back(ArgInfo<std::vector<std::string>>(
      std::move(short_name), std::move(long_name), var));
...
bool ArgParser::ParseOne(
  int& arg, int argc, char** argv, std::list<ArgInfo<T>>& arg_infos) {
