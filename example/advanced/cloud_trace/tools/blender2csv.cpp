...
bool find_blend_obj_prop(
  ...
  double& value) {
    ...
    if(prop_ptr) {
        ...
        if(
          std::strcmp(prop.Field<std::string>("name").Get().c_str(), name) ==
          0) {
            if(prop.Field<char>("type").Get() == 0x8) {
                try {
                    int data[2] = {prop.Field<int>("data.val").Get(),
        ...
        while(*child_name) {
            ...
            if(prop_ptr) {
                if(find_blend_obj_prop(blend_file, prop_ptr, name, value))
                    return true;
            }
            ++child_name;
        }
...
void print_usage(std::ostream& output) {
    output << "Usage: blender2csv "
           << "[input-file.blend] [output-file.csv]" << std::endl;
    output << std::endl;
}
