...
RaytraceProg::RaytraceProg(const AppData& app_data)
  ...
  , cloud_block(self(), "CloudBlock") {
    ProgramUniform<Vec3f>(self(), "LightPos")
      .Set(app_data.light_x, app_data.light_y, app_data.light_z);
    ...
    ProgramUniform<GLfloat>(self(), "UnitAttenuation")
      .Set(app_data.unit_attenuation);

    Optional<Uniform<GLfloat>>(self(), "Near").TrySet(app_data.cam_near);
    Optional<Uniform<GLfloat>>(self(), "Far").TrySet(app_data.cam_far);
    Optional<Uniform<GLfloat>>(self(), "CrepRayFar")
      .TrySet(app_data.crep_ray_far);
    Optional<Uniform<GLuint>>(self(), "CrepRaySam")
      .TrySet(app_data.crep_ray_sam);
...
Program RenderProg::make(const AppData& app_data) {
    ...
    Optional<Uniform<Vec3f>>(prog, "LightPos")
      .TrySet(Vec3f(app_data.light_x, app_data.light_y, app_data.light_z));
    ...
    Optional<Uniform<GLfloat>>(prog, "PlanetRadius")
      .TrySet(app_data.planet_radius);
    Optional<Uniform<GLfloat>>(prog, "AtmThickness")
      .TrySet(app_data.atm_thickness);

    return prog;
}

Program& RenderProg::self() {
    return *this;
}
