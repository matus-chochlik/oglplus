...
class SpectraDefaultMatrixTransf : public SpectraCalculator {
protected:
...
template <typename ValueType, typename Preprocessor, typename Extractor>
std::size_t
SpectraDefaultMatrixTransf<ValueType, Preprocessor, Extractor>::InputSize()
  const {
    return in_size;
}
...
template <typename ValueType, typename Preprocessor, typename Extractor>
std::size_t
SpectraDefaultMatrixTransf<ValueType, Preprocessor, Extractor>::OutputSize()
  const {
    return out_size;
}
...
template <typename ValueType, typename Preprocessor, typename Extractor>
const char*
SpectraDefaultMatrixTransf<ValueType, Preprocessor, Extractor>::Name() const {
    return name.c_str();
}

template <typename ValueType, typename Preprocessor, typename Extractor>
unsigned SpectraDefaultMatrixTransf<ValueType, Preprocessor, Extractor>::
  MaxConcurrentTransforms() const {
    return 1;
}
...
template <typename ValueType, typename Preprocessor, typename Extractor>
unsigned
SpectraDefaultMatrixTransf<ValueType, Preprocessor, Extractor>::BeginTransform(
  const float* input,
  std::size_t inbufsize,
  float* output,
  std::size_t outbufsize) {
...
std::shared_ptr<SpectraCalculator> SpectraGetDefaultCPUFourierTransf(
