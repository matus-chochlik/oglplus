...
class SpectraDefaultGPUMatrixTransf : public SpectraCalculator {
private:
...
public:
    SpectraDefaultGPUMatrixTransf(
      ...
      const std::function<std::complex<double>(
        std::size_t, std::size_t, std::size_t, std::size_t)>& gen);
    ...
    unsigned MaxConcurrentTransforms() const;
    ...
    void FinishTransform(unsigned tid, float* output, std::size_t outbufsize);
};
...
SpectraDefaultGPUMatrixTransf::SpectraDefaultGPUMatrixTransf(
  ...
  , name(transf_name)
  , transf_prog(
      shared_objects.BuildProgramWithXFB("xfb_matrix_transf.prog", "Output"))
  ...
  , queries(max_transforms) {
    ...
    Texture::Buffer(
      Texture::Target::Buffer, PixelDataInternalFormat::RG32F, matrix_buf);
    ...
    Texture::Buffer(
      Texture::Target::Buffer, PixelDataInternalFormat::R32F, input_buf);
...
std::size_t SpectraDefaultGPUMatrixTransf::InputSize() const {
    return in_size;
}

std::size_t SpectraDefaultGPUMatrixTransf::OutputSize() const {
    return out_size;
}

const char* SpectraDefaultGPUMatrixTransf::Name() const {
    return name.c_str();
}

unsigned SpectraDefaultGPUMatrixTransf::MaxConcurrentTransforms() const {
...
void SpectraDefaultGPUMatrixTransf::FinishBatch() {
    using namespace oglplus;
    Context gl;
...
void SpectraDefaultGPUMatrixTransf::FinishTransform(
  unsigned tid, float* output, std::size_t outbufsize) {
    using namespace oglplus;
