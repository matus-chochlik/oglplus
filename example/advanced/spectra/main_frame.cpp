...
class SpectraMainFrameDocumentLoader : public SpectraCoroutine {
private:
...
public:
    SpectraMainFrameDocumentLoader(
      SpectraMainFrame* frame, const std::shared_ptr<SpectraDocument>& doc);
...
wxString SpectraMainFrameDocumentLoader::Description() const {
    return wxGetTranslation(wxT("Loading document"));
}

void SpectraMainFrameDocumentLoader::Cancel() {}

bool SpectraMainFrameDocumentLoader::DoWork() {
...
int SpectraMainFrameDocumentLoader::PercentDone() const {
    return document->PercentLoaded();
}

//-- SpectraMainFrame --

static const int SpectraMainFrameID_MenuFile = wxID_HIGHEST + 1;
static const int SpectraMainFrameID_GenerateDoc =
  SpectraMainFrameID_MenuFile + 1;

static const int SpectraMainFrameID_MenuView = wxID_HIGHEST + 100;
static const int SpectraMainFrameID_DefaultRenderer =
  SpectraMainFrameID_MenuView + 1;
static const int SpectraMainFrameID_XSectionRenderer =
  SpectraMainFrameID_MenuView + 2;
...
wxStaticText* AddKeyValue(
  ...
  const wxString& value) {
    wxStaticText* name_label = new wxStaticText(
      parent, wxID_ANY, label, wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
    ...
    wxStaticText* value_label = new wxStaticText(
      parent, wxID_ANY, value, wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT);
    sizer->Add(value_label, 3, wxEXPAND);
    return value_label;
}

void SpectraMainFrame::InitMainMenu() {
    ...
    view_menu->AppendSubMenu(
      renderer_menu, wxGetTranslation(wxT("&Renderer"), wxT("Menu")));

    main_menu->Append(view_menu, wxGetTranslation(wxT("&View"), wxT("Menu")));
...
void SpectraMainFrame::ConnectEventHandlers() {
    Connect(
      wxID_EXIT,
    ...
    Connect(wxEVT_CLOSE_WINDOW, wxCloseEventHandler(SpectraMainFrame::OnClose));
    SetExtraStyle(wxWS_EX_PROCESS_IDLE);
    wxIdleEvent::SetMode(wxIDLE_PROCESS_SPECIFIED);
    Connect(wxEVT_IDLE, wxIdleEventHandler(SpectraMainFrame::OnIdle));
}

void SpectraMainFrame::InitComponents() {
    const int em = 5;
    const int em_2 = em / 2;
...
void SpectraMainFrame::DoShowAboutDialog(wxCommandEvent&) {
    ...
    info.SetDescription(
      wxGetTranslation(wxT("3D viewer of signal Fourier spectra.")));
...
void SpectraMainFrame::DoOpenDocument(wxCommandEvent&) {
    wxString doc_path = wxLoadFileSelector(
      ...
      wxT(".wav"),
      (const wxChar*)0,
      this); // TODO a customized dialog
    try {
        StartCoroutine(
          std::make_shared<SpectraMainFrameDocumentLoader>(
            this, SpectraLoadDocFromFile(doc_path)),
          false);
    } catch(oglplus::Error& err) {
        parent_app.HandleError(err);
    } catch(const std::exception& se) {
        parent_app.HandleError(se);
    }
}
    ...
    try {
        struct TestSignal {
            float operator()(float x) const {
                return float(
                  sin(3.1415 * 10.0 * (sin(x) + 2.0 + x * x)) * std::rand() /
                  float(RAND_MAX) * std::rand() / float(RAND_MAX));
            }
        };

        StartCoroutine(
          std::make_shared<SpectraMainFrameDocumentLoader>(
            this, SpectraOpenTestDoc(TestSignal(), 11000, 4.71f)),
          false);
    } catch(oglplus::Error& err) {
        parent_app.HandleError(err);
    } catch(const std::exception& se) {
...
void SpectraMainFrame::StartCoroutine(
  const std::shared_ptr<SpectraCoroutine>& coroutine) {
    coroutine_exec->Start(
      coroutine, false); // TODO: get value of quiet from settings
}

void SpectraMainFrame::StartCoroutine(
  const std::shared_ptr<SpectraCoroutine>& coroutine, bool quiet) {
    coroutine_exec->Start(coroutine, quiet);
}

void SpectraMainFrame::OpenLoadedDocument(
  const std::shared_ptr<SpectraDocument>& document) {
    struct DocVisMaker {
        ...
        std::shared_ptr<SpectraVisualisation> operator()(
          ...
          SpectraMainFrame* main_frame,
          wxGLCanvas* canvas,
          wxGLContext* parent_context) const {
            return std::make_shared<SpectraVisualisation>(
              main_frame, canvas, parent_context, calc, doc);
        }
    } doc_vis_maker = {// let the use pick the sliding window size
                       // or spectrum with and the spectrum calculator
                       shared_objects->SpectrumCalculator(128),
                       document};

    RegisterDocumentFrame(new SpectraDocumentFrame(
      this->parent_app,
      this,
      &this->gl_context,
      doc_vis_maker,
  ...
  wxGLCanvas* canvas) {
    ...
    if(renderer_menu->IsChecked(SpectraMainFrameID_XSectionRenderer)) {
        result = SpectraMakeXSectionRenderer(
          app, main_frame->shared_objects, doc_vis, canvas);
    } else {
...
SpectraMainFrame::RendererGetter SpectraMainFrame::LazyRendererPicker() {
        ...
        std::shared_ptr<SpectraRenderer> operator()(
          ...
          wxGLCanvas* canvas) {
            ...
            return main_frame->PickRenderer(
              main_app, main_frame, doc_vis, canvas);
        }
    };
...
SpectraMainFrame::SpectraMainFrame(SpectraApp& app)
  ...
  , coroutine_exec() {
    ...
    gl_vendor->SetLabel(
      wxString((const char*)glGetString(GL_VENDOR), wxConvUTF8));
    gl_renderer->SetLabel(
      wxString((const char*)glGetString(GL_RENDERER), wxConvUTF8));
    gl_version->SetLabel(
      wxString((const char*)glGetString(GL_VERSION), wxConvUTF8));
    sl_version->SetLabel(wxString(
      (const char*)glGetString(GL_SHADING_LANGUAGE_VERSION), wxConvUTF8));

    SetStatus(wxGetTranslation(wxT("Creating shared objects"), wxT("Status")));

    shared_objects =
      std::make_shared<SpectraSharedObjects>(&gl_context, gl_canvas);
...
SpectraMainFrame::~SpectraMainFrame() {
    gl_context.SetCurrent(*gl_canvas);
