...
std::complex<double> SpectraFourierMatrixGen::operator()(
  std::size_t i, std::size_t k, std::size_t /*n*/, std::size_t /*m*/
  ) const {
...
extern std::shared_ptr<SpectraCalculator> SpectraGetDefaultCPUFourierTransf(
  SpectraSharedObjects&, std::size_t spectrum_size);

extern std::shared_ptr<SpectraCalculator> SpectraGetDefaultGPUFourierTransf(
  SpectraSharedObjects&, std::size_t spectrum_size);

std::shared_ptr<SpectraCalculator> SpectraGetDefaultSignalTransform(
  SpectraSharedObjects& shared_objects, std::size_t spectrum_size) {
    try {
        return SpectraGetDefaultGPUFourierTransf(shared_objects, spectrum_size);
    } catch(...) {
        return SpectraGetDefaultCPUFourierTransf(shared_objects, spectrum_size);
    }
}
