  ...
  bool separate_attribs) {
    ...
    while(prog_file.getline(line, linesize).good()) {
        ...
        if(types.Empty()) {
            wxString message = wxString::Format(
              wxGetTranslation(wxT("Invalid shader type in program '%s' "
                                   "on line %d: '%s'")),
              wxString(prog_name, wxConvUTF8).c_str(),
              line_no,
              wxString(line, wxConvUTF8).c_str());
            throw std::runtime_error((const char*)message.mb_str(wxConvUTF8));
        }
    ...
    if(xfb_varyings && xfb_var_count) {
        prog.TransformFeedbackVaryings(
          xfb_var_count,
          xfb_varyings,
          separate_attribs ? TransformFeedbackMode::SeparateAttribs
...
oglplus::Program SpectraSharedObjects::BuildProgramWithXFB(
  const char* prog_name, const char* xfb_varying, bool separate_attribs) {
    return BuildProgramWithXFB(prog_name, &xfb_varying, 1, separate_attribs);
}
...
SpectraSharedObjects::SpectraSharedObjects(
  wxGLContext* context, wxGLCanvas* canvas)
  : gl_context(context)
  , gl_canvas(canvas)
  , ground_unit_grid(
...
wxGLContext* SpectraSharedObjects::GLContext() {
    return gl_context;
}

wxGLCanvas* SpectraSharedObjects::GLCanvas() {
    return gl_canvas;
}

const oglplus::shapes::ShapeWrapper& SpectraSharedObjects::GroundUnitGrid() {
    return ground_unit_grid;
}

const oglplus::shapes::ShapeWrapper& SpectraSharedObjects::WallUnitGrid() {
...
const oglplus::shapes::ShapeWrapper& SpectraSharedObjects::SpectrumPlane(
  GLuint samples_per_sec, GLuint spectrum_size) {
    ...
    if(pos == spectrum_planes.end()) {
        pos = spectrum_planes
                .insert(spectrum_plane_map::value_type(
                  key,
...
std::shared_ptr<SpectraCalculator> SpectraSharedObjects::SpectrumCalculator(
  std::size_t spectrum_size) {
    spectrum_calculator_map::iterator pos =
      spectrum_calculators.find(spectrum_size);
    if(pos == spectrum_calculators.end()) {
        pos = spectrum_calculators
                .insert(spectrum_calculator_map::value_type(
                  spectrum_size,
                  SpectraGetDefaultSignalTransform(*this, spectrum_size)))
                .first;
    }
    return pos->second;
}
