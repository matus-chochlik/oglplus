...
static const int SpectraDocFrameID_MenuFile = wxID_HIGHEST + 1001;
static const int SpectraDocFrameID_Play = SpectraDocFrameID_MenuFile + 1;

void SpectraDocumentFrame::InitMainMenu() {
    wxMenu* file_menu = new wxMenu();
    file_menu->Append(wxID_ADD, wxGetTranslation(wxT("New &view\tCtrl-N")));
    file_menu->AppendSeparator();
    play_menu_item = file_menu->Append(
      SpectraDocFrameID_Play, wxGetTranslation(wxT("&Play\tCtrl-P")));
    file_menu->AppendSeparator();
...
void SpectraDocumentFrame::ConnectEventHandlers() {
    Connect(
      wxID_CLOSE,
    ...
    Connect(
      wxEVT_CLOSE_WINDOW, wxCloseEventHandler(SpectraDocumentFrame::OnClose));
    Connect(
    ...
    Connect(wxEVT_SIZE, wxSizeEventHandler(SpectraDocumentFrame::OnResize));
...
void SpectraDocumentFrame::InitComponents() {
    ...
    SetSize(
      ...
      main_menu->GetSize().GetHeight() + 1 + main_panel->GetSize().GetHeight() +
        1 + status_bar->GetSize().GetHeight() + 1);
    SetSizer(main_sizer);
    Layout();
}
...
void SpectraDocumentFrame::DoDuplicate(wxCommandEvent&) {
    struct DocVisGetter {
        ...
        std::shared_ptr<SpectraVisualisation> operator()(
          SpectraApp&, SpectraMainFrame*, wxGLCanvas*, wxGLContext*) const {
            return doc_vis;
        }
    } doc_vis_getter = {document_vis};

    main_frame->RegisterDocumentFrame(new SpectraDocumentFrame(
      parent_app,
      main_frame,
      parent_context,
      doc_vis_getter,
...
void SpectraDocumentFrame::HandleResize() {
...
void SpectraDocumentFrame::OnResize(wxSizeEvent& event) {
    try {
        ...
        parent_app.HandleError(err, this);
    } catch(const std::exception& se) {
        parent_app.HandleError(se, this);
    }
}

void SpectraDocumentFrame::HandleSysColorChange() {
...
void SpectraDocumentFrame::HandleMouseWheel(const wxMouseEvent& event) {
    document_view.Zoom(
      GLfloat(event.GetLinesPerAction()) * GLfloat(event.GetWheelRotation()) /
...
void SpectraDocumentFrame::Update() {
...
SpectraDocumentFrame::SpectraDocumentFrame(
  ...
  , gl_canvas(
      new SpectraDocumentCanvas((wxEvtHandler*)this, (wxWindow*)main_panel))
  , document_vis()
  , renderer()
  , idle_call_count(0) {
    assert(gl_canvas);
    ...
    document_vis =
      get_doc_vis(parent_app, main_frame, gl_canvas, parent_context);
    assert(document_vis);
    document_vis->AddCanvas(gl_canvas);
    assert(play_menu_item);
    play_menu_item->Enable(document_vis->Document().CanPlay());

    renderer = get_renderer(parent_app, main_frame, document_vis, gl_canvas);
    assert(renderer);
