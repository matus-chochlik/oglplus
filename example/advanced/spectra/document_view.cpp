...
void SpectraDocumentView::Clamp(
  GLfloat& value, GLfloat min, GLfloat max, GLfloat s) {
    if(value < min * s)
        value = min * s;
    if(value > max * s)
        value = max * s;
...
SpectraDocumentView::SpectraDocumentView()
  : max_time(1.0f)
  , width(64)
  , height(64)
...
GLfloat SpectraDocumentView::MaxTargetX() {
    return max_time + 1.5f;
}

GLuint SpectraDocumentView::SegmentCount() {
...
void SpectraDocumentView::Slide(
  GLint new_x, GLint new_y, GLint old_x, GLint old_y) {
    oglplus::Vec3f dir =
      PickOnGround(new_x, new_y) - PickOnGround(old_x, old_y);
    target_x -= dir.x();
...
void SpectraDocumentView::Elevate(
  GLint new_x, GLint new_y, GLint old_x, GLint old_y) {
...
void SpectraDocumentView::Orbit(
  GLint new_x, GLint new_y, GLint old_x, GLint old_y) {
    ...
    if(Length(ns) > 0.0f && Length(os) > 0.0f) {
        camera_azimuth += oglplus::Radians(
          std::atan2(-ns.z(), ns.x()) - std::atan2(-os.z(), os.x()));
...
void SpectraDocumentView::Scale(
  GLint new_x, GLint new_y, GLint old_x, GLint old_y) {
...
void SpectraDocumentView::StretchDomain(
  GLint new_x, GLint new_y, GLint old_x, GLint old_y) {
    oglplus::Vec3f ng = PickOnGround(new_x, new_y);
    oglplus::Vec3f og = PickOnGround(old_x, old_y);
...
void SpectraDocumentView::StretchRange(
  GLint new_x, GLint new_y, GLint old_x, GLint old_y) {
...
void SpectraDocumentView::RecalcProjection() {
    projection_matrix = oglplus::CamMatrixf::PerspectiveX(
      camera_xfov, float(width) / height, 0.1f, 10000.0f);
    RecalcTransf();
}

void SpectraDocumentView::RecalcCamera() {
...
void SpectraDocumentView::RecalcStretch() {
    stretch_matrix =
      oglplus::ModelMatrixf::Scale(stretch_x, stretch_y, stretch_z);
    RecalcTransf();
}

void SpectraDocumentView::RecalcTransf() {
    transf_matrix = projection_matrix * camera_matrix * stretch_matrix;
    inv_view_matrix = Inverse(projection_matrix * camera_matrix);
}

float SpectraDocumentView::TimeStretch() const {
    return stretch_x;
}

const oglplus::Mat4f& SpectraDocumentView::ProjectionMatrix() const {
    return projection_matrix;
}

const oglplus::Mat4f& SpectraDocumentView::CameraMatrix() const {
    return camera_matrix;
}

const oglplus::Mat4f& SpectraDocumentView::StretchMatrix() const {
    return stretch_matrix;
}

const oglplus::Mat4f& SpectraDocumentView::TransfMatrix() const {
