...
public:
    SpectraCoroutineExecution(
      wxWindow* parent, const std::shared_ptr<SpectraCoroutine>& coro);
...
wxString SpectraCoroutineExecution::Description() const {
    return coroutine->Description();
}

int SpectraCoroutineExecution::PercentDone() const {
    return coroutine->PercentDone();
}

void SpectraCoroutineExecution::Cancel() {
    coroutine->Cancel();
}

bool SpectraCoroutineExecution::DoWork() {
...
SpectraCoroutineExecutor::SpectraCoroutineExecutor(
  SpectraApp& app, wxWindow* win)
  : parent_app(app)
  , parent_window(win) {}
  ...
  const std::shared_ptr<SpectraCoroutine>& coroutine, bool quiet_exec) {
    try {
        if(quiet_exec) {
            ...
            coroutines.push(std::make_shared<SpectraCoroutineExecution>(
              parent_window, coroutine));
        }
...
bool SpectraCoroutineExecutor::UpdateSingle(
...
bool SpectraCoroutineExecutor::DoWork() {
    if(!coroutines.empty()) {
