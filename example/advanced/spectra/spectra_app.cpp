...
wxString SpectraApp::MakeCommonErrorMessage(oglplus::Error& err) {
    ...
    message.Append(newline);
    message.Append(wxString::Format(
      wxGetTranslation(wxT("Line: '%u'"), wxT("Error")), err.SourceLine()));
    message.Append(newline);

    return message;
}

void SpectraApp::HandleError(oglplus::Error& err, wxWindow* parent) {
    wxMessageBox(
...
void SpectraApp::HandleError(const std::exception& err, wxWindow* parent) {
    wxMessageBox(
