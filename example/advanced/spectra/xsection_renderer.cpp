...
class SpectraXSectionRenderer : public SpectraRenderer {
private:
    GLfloat clear_r, clear_g, clear_b;
    GLfloat color_r, color_g, color_b;

    oglplus::Program xsection_prog;
    oglplus::Optional<oglplus::Uniform<oglplus::Mat4f>>
      xsection_projection_matrix;
...
SpectraXSectionRenderer::SpectraXSectionRenderer(
  ...
  , spectrum_plane_wrap(Common().SpectrumPlane(
      GLuint(DocVis().GridSamples()), GLuint(DocVis().SignalSpectrumSize())))
  , spectrum_plane_vao(spectrum_plane_wrap.VAOForProgram(xsection_prog)) {
    CacheBgColor();
}

void SpectraXSectionRenderer::CacheBgColor() {
...
void SpectraXSectionRenderer::RenderSpectrum(SpectraDocumentView& view) {
    ...
    xsection_projection_matrix.TrySet(
      oglplus::CamMatrixf::Ortho(-0.5f, 0.5f, -0.5f, 0.5f, +1.0f, 1.01f));
    xsection_camera_matrix.TrySet(oglplus::CamMatrixf::LookingAt(
...
void SpectraXSectionRenderer::ReinitStyle() {
    CacheBgColor();
}

bool SpectraXSectionRenderer::Interactive() {
    return false;
}

void SpectraXSectionRenderer::Render(
  SpectraDocumentView& view, wxGLCanvas* canvas) {
    assert(canvas);
