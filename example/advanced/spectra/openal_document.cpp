...
class SpectraOpenALDocument : public SpectraDocument {
private:
...
bool SpectraOpenALDocument::FinishLoading() {
    context.MakeCurrent();
...
int SpectraOpenALDocument::PercentLoaded() const {
    return samples.empty() ? 0 : 100;
}

std::size_t SpectraOpenALDocument::SamplesPerSecond() const {
    return std::size_t(frequency);
}

std::size_t SpectraOpenALDocument::SignalSampleCount() const {
    return samples.size();
}

float SpectraOpenALDocument::MaxTime() const {
    return samples.size() / frequency;
}

wxString SpectraOpenALDocument::Name() const {
    return file_path;
}
...
bool SpectraOpenALDocument::CanPlay() const {
    return true;
}
