...
class SpectraVisDataUploader : public SpectraCoroutine {
private:
  ...
  std::size_t end_row) {
    ...
    for(std::size_t i = 0; i != n; ++i) {
        spectra_calc->FinishTransform(
          transform_ids[i % m], buffer + i * spectrum_size, spectrum_size);
        std::size_t j = i + m;
        if(j < n) {
...
bool SpectraVisDataUploader::SetCurrent() {
    assert(gl_canvases);
    if(gl_canvases->empty())
        return false;
...
wxString SpectraVisDataUploader::Description() const {
    return wxGetTranslation(wxT("Uploading spectrum data to GPU"));
}

void SpectraVisDataUploader::Cancel() {
    current_row = row_count;
}

bool SpectraVisDataUploader::DoWork() {
    if((current_row < row_count) && SetCurrent()) {
        ...
        this->QuerySpectrumValues(
          data_buf.data(), data_buf.size(), current_row, current_row + n);
...
int SpectraVisDataUploader::PercentDone() const {
    return int((current_row * 100) / row_count);
}

// SpectraGLContext
SpectraGLContext::SpectraGLContext(wxGLCanvas* canvas, wxGLContext* parent_ctxt)
  ...
  , signal_samples_per_grid(1) {
    ...
    std::shared_ptr<SpectraVisDataUploader> data_uploader(
      std::make_shared<SpectraVisDataUploader>(
        &gl_context, gl_canvases, calculator, document, spectrum_data));
...
wxGLContext& SpectraVisualisation::GLContext() {
    return gl_context;
}

oglplus::Reference<oglplus::Texture> SpectraVisualisation::SpectrumTex() {
    return spectrum_tex;
}

wxString SpectraVisualisation::Name() {
    return Document().Name();
}

const SpectraDocument& SpectraVisualisation::Document() const {
    assert(document);
    return *document;
}

void SpectraVisualisation::Play() {
    assert(document);
    document->Play(SelectionBegin(), SelectionEnd());
}

std::size_t SpectraVisualisation::SignalSpectrumSize() const {
    return spectrum_size;
}

std::size_t SpectraVisualisation::SignalSamplesPerGrid() const {
    return Document().SamplesPerSecond();
}

std::size_t SpectraVisualisation::SignalSamplesPerGridPatch() const {
    return signal_samples_per_grid;
}

std::size_t SpectraVisualisation::GridSamples() const {
...
float SpectraVisualisation::SelectedTime() const {
...
float SpectraVisualisation::SelectionBegin() const {
    return selection_begin;
}

float SpectraVisualisation::SelectionEnd() const {
