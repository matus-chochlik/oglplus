...
class TextExample : public oglplus::StandaloneExample {
private:
    oglplus::Context gl;
...
public:
    ...
    void RenderText(GLfloat x, GLfloat y, const oglplus::String& text) {
        ...
        dsa.ModelviewMatrix().LoadIdentity().Translate(x, y, 0);
    ...
    void Render() {
        ...
        }
        {
            ...
            s << std::fixed << std::setprecision(2);
            s << "Current FPS: "
              << (FrameDuration() > 0.0 ? CurrentFPS() : 0.0);
            RenderText(l, h - font_height * i++, s.str());
        }
        {
            std::stringstream s;
...
int main(int argc, char* argv[]) {
    return oglplus::GlutGlewMain<TextExample>(
      "Example of text rendering with NV_path_rendering", argc, argv);
}
