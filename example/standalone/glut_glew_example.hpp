...
public:
    GlutGlewExampleApp(
      ...
      int& argc,
      char* argv[])
      : GlutInit(xpos, ypos, width, height, window_title, argc, argv)
      , GlewInit()
    ...
    try {
        std::cout << "Started" << std::endl;
        {
            ...
            GlutGlewExampleApp<Example> app(
              100, 100, width, height, title, argc, argv);
        ...
        std::cerr << "Program build error (in "
                  << (pbe.GLFunc() ? pbe.GLFunc() : "N/A") << ", "
                  << pbe.ObjectTypeName() << ": (" << pbe.ObjectName() << ") '"
                  << pbe.ObjectDesc() << "'): " << pbe.what() << ": "
                  << pbe.Log() << std::endl;
    } catch(oglplus::LimitError& le) {
        std::cerr << "Limit error: (" << le.Value() << ") exceeds ("
                  << le.EnumParamName() << " == " << le.Limit() << ") ["
                  << le.SourceFile() << ":" << le.SourceLine() << "] "
                  << std::endl;
    } catch(oglplus::ObjectError& obe) {
        std::cerr << "Object error (in "
                  << (obe.GLFunc() ? obe.GLFunc() : "N/A") << ", "
                  << obe.ObjectTypeName() << ": (" << obe.ObjectName() << ") '"
                  << obe.ObjectDesc() << "') [" << obe.SourceFile() << ":"
                  << obe.SourceLine() << "]: " << obe.what() << std::endl;
