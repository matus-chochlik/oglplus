...
class FontTexture : public oglplus::DSATextureEXT {
public:
...
private:
    ...
    void LoadGlyph(std::istream& bfm, char* buf, const GLuint size) {
...
public:
    ...
    const std::vector<GLfloat>& LogData() const {
        return log_data;
    }

    const std::vector<GLfloat>& InkData() const {
        return ink_data;
    }

    const std::vector<GLfloat>& TexData() const {
        return tex_data;
    }
...
class BitmapFontProgram : public oglplus::Program {
private:
    static oglplus::Program make() {
        using namespace oglplus;
        Program prog;
    ...
    void Render(
      ...
      const std::vector<GLubyte>& indices) {
        ...
        Context().DrawElements(
          PrimitiveType::Points, indices.size(), indices.data());
    }
};
...
class BitmapFontExample : public oglplus::StandaloneExample {
private:
    oglplus::Context gl;
...
public:
    BitmapFontExample(int argc, const char** argv)
      ...
      , frame_no(0) {
        ...
        gl.BlendFunc(BlendFunction::SrcAlpha, BlendFunction::DstAlpha);
    }
    ...
    void Render() {
        ...
        font_prog.camera_matrix.Set(CamMatrixf::Orbiting(
          Vec3f(),
        ...
        // the "OGLplus" string
        font_prog.layout_matrix.Set(ModelMatrixf::Translation(
          -4.0f, GLfloat(3.0 + SineWave(t / 0.5) * 0.1), 0.1f));
        ...
        font_prog.layout_matrix.Set(
          ...
          ModelMatrixf::Translation(
            -2.0f, GLfloat(1.0 + SineWave(t / 0.7) * 0.2), 0.0f) *
        ...
        font_prog.layout_matrix.Set(
          ...
          ModelMatrixf::Scale(
            0.6f, GLfloat(0.6 + CosineWave(t / 0.6) * 0.2), 1.0f));
        txt << "Frame time: " << t << " [s]";
        font.Render(font_prog, txt.str());
        txt.str(std::string());
        ...
        font_prog.layout_matrix.Set(
          ...
          ModelMatrixf::Scale(
            GLfloat(0.6 + SineWave(t / 0.8) * 0.1), 0.6f, 1.0f));
...
int main(int argc, char* argv[]) {
    return oglplus::GlutGlewMain<BitmapFontExample>(
      "Simple example of bitmap font text rendering", argc, argv);
}
