...
struct value_visitor : BlendFileVisitor {
    ...
    void VisitStr(const std::string& value) {
        ...
        for(char c : value) {
            if(c == '\0')
                ...
                _output << "\\" << std::setw(3) << std::setfill('0') << std::oct
                        << (c & 0xFF) << std::dec;
            }
        }
        _output << "'</em>";
    ...
    void VisitPtr(BlendFilePointer ptr) {
        ...
        _output << std::hex << pv << std::dec;
        if(pv != ptr.Value()) {
            _output << "+" << std::hex << (ptr.Value() - pv) << std::dec;
        }
        if(found_block) {
            _output << "</a>";
        }
    ...
    void VisitChr(char c) {
        _output << "0x" << std::hex << (c & 0xFF) << std::dec;
    }
...
void blend_to_html(const std::string& filename, const std::string& work_dir) {
    ...
    index << "<tr><th>Property</th><th>Value</th></tr>" << std::endl;
    index << "<tr><th>Pointer size</th><td>" << fi.PointerSize() << "</td></tr>"
          << std::endl;
    index << "<tr><th>Byte order</th><td>"
          << (fi.ByteOrder() == Endian::Big ? "Big" : "Little")
          << " endian</td></tr>" << std::endl;
    index << "<tr><th>Version</th><td>" << fi.VersionMajor() << "."
          << fi.VersionMinor() << "</td></tr>" << std::endl;
    ...
    while(!sr.Empty()) {
        ...
        filepath << work_dir << "/struct-" << s.Name() << ".html";
        ...
        output << "<h2>Structure <code>" << s.Name() << "</code></h2>"
               << std::endl;

        output << "<table>" << std::endl;

        output << "<tr><th>Property</th><th>Value</th></tr>" << std::endl;
        output << "<tr><th>Size</th><td>" << s.Size() << "</td></tr>"
               << std::endl;
        ...
        output << "<table>" << std::endl;
        output << "<tr><th>Type</th><th>Definition</th></tr>" << std::endl;
        ...
        while(!fr.Empty()) {
            ...
            auto bt = f.BaseType();
            if(bt.IsStructure()) {
                output << "<tr><td><a href='struct-" << bt.Name() << ".html'>"
                       << bt.Name() << "</a></td><td>" << f.Definition()
                       << "</td></tr>" << std::endl;
            } else {
                output << "<tr><td>" << bt.Name() << "</td><td>"
                       << f.Definition() << "</td></tr>" << std::endl;
            }
            fr.Next();
        }
        output << "</table>" << std::endl;
    ...
    while(!br.Empty()) {
        ...
        std::stringstream filepath;
        filepath << work_dir << "/block-0x" << std::hex << b.Pointer().Value()
                 << ".html";
        ...
        output << "<tr><th>Property</th><th>Value</th></tr>" << std::endl;
        output << "<tr><th>Code</th><td>" << b.Code().c_str() << "</td></tr>"
               << std::endl;
        output << "<tr><th>Structure</th><td><a href='struct-" << bs.Name()
               << ".html'>" << bs.Name() << "</a></td></tr>" << std::endl;
        output << "<tr><th>Element size</th><td>" << bs.Size() << "</td></tr>"
               << std::endl;
        output << "<tr><th>Element count</th><td>" << b.ElementCount()
               << "</td></tr>" << std::endl;
        output << "<tr><th>Size</th><td>" << b.Size() << "</td></tr>"
               << std::endl;
        ...
        if((b.Size() != 0) && (b.Size() == bs.Size() * b.ElementCount())) {
            ...
            for(std::size_t be = 0; be != b.ElementCount(); ++be) {
                ...
                while(!bfsfr.Empty()) {
                    ...
                    output << "<tr>" << std::endl;
                    if(bt.IsStructure()) {
                        output << "<td><a href='struct-" << bt.Name()
                               << ".html'>" << bt.Name() << "</a></td>"
                               << std::endl;
                    } else {
                        output << "<td>" << bt.Name() << "</td>" << std::endl;
                    }
                    output << "<td>" << sf.Definition() << "</td>" << std::endl;
                    output << "<td>" << ff.Name() << "</td>" << std::endl;
                    output << "<td>";
    ...
    stylesheet << "body { font-family: 'Courier New', Courier, monospace; }"
               << std::endl;
    stylesheet << "table { border-collapse: collapse; }" << std::endl;
    stylesheet << "table, th, td { border: 1px solid black; }" << std::endl;
    stylesheet << "th { background-color: navy; color: white; }" << std::endl;

    stylesheet.close();
}

void print_usage(std::ostream& output) {
    output << "Usage: 010_blender2html "
           << "[input-file.blend] [output-directory]" << std::endl;
    output << "Note: The output directory must exist and must be writable"
           << std::endl;
    output << std::endl;
}
