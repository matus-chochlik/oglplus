...
template <typename T>
void do_test_matrix_default_construction() {
...
BOOST_AUTO_TEST_CASE(Matrix_construction_2) {
    float mat_init_4[4] = {0.0f, 1.0f, 2.0f, 3.0f};
    oglplus::Matrix<float, 2, 2> mat2(mat_init_4);

    float mat_init_6[6] = {0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f};
    oglplus::Matrix<float, 2, 3> mat2x3(mat_init_6);
    oglplus::Matrix<float, 3, 2> mat3x2(mat_init_6);

    float mat_init_8[8] = {0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f};
...
BOOST_AUTO_TEST_CASE(Matrix_construction_3) {
    oglplus::Matrix<float, 2, 2> mat2(0.0f, 1.0f, 2.0f, 3.0f);
    oglplus::Matrix<float, 2, 3> mat2x3(0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f);
    oglplus::Matrix<float, 2, 4> mat2x4(
      0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f);
    oglplus::Matrix<float, 3, 2> mat3x2(0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f);
    oglplus::Matrix<float, 3, 3> mat3(
      0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f);
    oglplus::Matrix<float, 3, 4> mat3x4(
      0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.f, 11.f);
...
BOOST_AUTO_TEST_CASE(Matrix_comparison_1) {
    oglplus::Matrix<float, 2, 2> mat2(0.0f, 1.0f, 4.0f, 5.0f);
    oglplus::Matrix<float, 2, 3> mat2x3(0.0f, 1.0f, 2.0f, 4.0f, 5.0f, 6.0f);
    oglplus::Matrix<float, 2, 4> mat2x4(
      0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f);
    oglplus::Matrix<float, 3, 2> mat3x2(0.0f, 1.0f, 4.0f, 5.0f, 8.0f, 9.0f);
    oglplus::Matrix<float, 3, 3> mat3(
      0.0f, 1.0f, 2.0f, 4.0f, 5.0f, 6.0f, 8.0f, 9.0f, 10.f);
    oglplus::Matrix<float, 3, 4> mat3x4(
      0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.f, 11.f);
    oglplus::Matrix<float, 4, 2> mat4x2(
      0.0f, 1.0f, 4.0f, 5.0f, 8.0f, 9.0f, 12.f, 13.f);
    oglplus::Matrix<float, 4, 3> mat4x3(
      0.0f, 1.0f, 2.0f, 4.0f, 5.0f, 6.0f, 8.0f, 9.0f, 10.f, 12.f, 13.f, 14.f);
    oglplus::Matrix<float, 4, 4> mat4(
      0.0f,
      1.0f,
      2.0f,
...
BOOST_AUTO_TEST_CASE(Matrix_submatrix) {
    ...
    BOOST_CHECK(
      (oglplus::Submatrix<0, 1, 2, 2>(mat4) ==
       oglplus::Matrix<float, 2, 2>(1.0f, 2.0f, 5.0f, 6.0f)));

    BOOST_CHECK(
      (oglplus::Submatrix<1, 0, 2, 2>(mat4) ==
       oglplus::Matrix<float, 2, 2>(4.0f, 5.0f, 8.0f, 9.0f)));

    BOOST_CHECK(
      (oglplus::Submatrix<1, 1, 2, 2>(mat4) ==
       oglplus::Matrix<float, 2, 2>(5.0f, 6.0f, 9.0f, 10.f)));

    BOOST_CHECK(
      (oglplus::Submatrix<2, 2, 2, 2>(mat4) ==
       oglplus::Matrix<float, 2, 2>(10.f, 11.f, 14.f, 15.f)));

    BOOST_CHECK(
      (oglplus::Submatrix<1, 1, 3, 3>(mat4) ==
       oglplus::Matrix<float, 3, 3>(
         5.0f, 6.0f, 7.0f, 9.0f, 10.f, 11.f, 13.f, 14.f, 15.f)));
}
...
void do_test_matrix_addition() {
    ...
    for(unsigned y = 0; y != 1000; ++y) {
        ...
        for(std::size_t i = 0; i != R; ++i) {
            for(std::size_t j = 0; j != C; ++j) {
                BOOST_CHECK_CLOSE(
                  mat_3.At(i, j), mat_2.At(i, j) + mat_1.At(i, j), eps);
            }
        }
    }
}
...
template <typename T, std::size_t R, std::size_t C>
void do_test_matrix_subtraction() {
    const T eps = 1e-9;
    for(unsigned y = 0; y != 1000; ++y) {
        T data_1[R * C];
        T data_2[R * C];
        ...
        for(std::size_t i = 0; i != R; ++i) {
            for(std::size_t j = 0; j != C; ++j) {
                BOOST_CHECK_CLOSE(mat_4.At(i, j), mat_2.At(i, j), eps);
                BOOST_CHECK_CLOSE(mat_5.At(i, j), mat_1.At(i, j), eps);
            }
        }
    }
}
...
template <
  ...
  std::size_t K,
  std::size_t L>
void do_test_matrix_multiplication() {
    const T eps = 1e-9;
    for(unsigned y = 0; y != 1000; ++y) {
        T data_1[M * N];
        ...
        BOOST_CHECK(Close(
          mat_1 * mat_2 * mat_3 * mat_4, mat_1 * (mat_2 * mat_3) * mat_4, eps));
